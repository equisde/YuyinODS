...
Override with `YUYINODS_OAUTH_CREDENTIAL_DIRS` using the platform path separator
...
```text
http://yuyinods.local:3002/api/oauth/callback
http://localhost:3002/api/oauth/callback
http://127.0.0.1:3002/api/oauth/callback
```

If the device uses a custom `YUYINODS_DEVICE_NAME`, add the matching
