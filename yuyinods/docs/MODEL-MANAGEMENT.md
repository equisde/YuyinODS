...
docker restart yuyinods-hermes
```
...
```bash
~/yuyinods/data/models/
```

On Windows installs:

```powershell
$env:USERPROFILE\yuyinods\data\models\
```

Each model is normally a single `.gguf` file:

```bash
ls -lh ~/yuyinods/data/models/*.gguf
```

The active model is recorded in `.env`:

```bash
grep -E "^(LLM_MODEL|GGUF_FILE|CTX_SIZE|MAX_CONTEXT)=" ~/yuyinods/.env
```
...
ods restart litellm
docker restart yuyinods-hermes 2>/dev/null || true
```
