...
scripts/yuyinods-doctor.sh /tmp/custom-yuyinods-doctor.json
```
...
Diagnoses:
  BLOCKER YuyinODS-DOCKER-IMAGE-UNRESOLVED: Docker image reference could not be resolved (high confidence)
...
- **diagnoses**: Stable, evidence-ranked install/runtime root-cause diagnoses.
  Each item includes:
  - `id`: stable issue code, for example `YuyinODS-COMPOSE-CWD-MISMATCH`
...
- **runtime.inference_contract**: A compact routing contract for the active
  inference mode. It records `YUYINODS_MODE`, expected inference owner
...
- `YuyinODS-INSTALL-ENV-MISSING`: an installed-looking tree is missing its generated
  `.env`.
- `YuyinODS-COMPOSE-CWD-MISMATCH`: `logs/compose-launch.txt` says compose was launched
  from a different directory than the Doctor root.
- `YuyinODS-DOCKER-IMAGE-UNRESOLVED`: saved install logs show an image tag that Docker
  could not resolve.
- `YuyinODS-COMPOSE-ZERO-CONTAINERS`: compose completed but no managed ODS
  containers were created.
- `YuyinODS-PYTHON-PYYAML-MISSING`: the selected installer Python could not import
  PyYAML.
- `YuyinODS-WINDOWS-FILE-SHARING-PROBE-IMAGE`: Windows file-sharing probe evidence is
...
- `YuyinODS-RUNTIME-MODE-UNKNOWN`: `.env` contains an unrecognized `YUYINODS_MODE`.
- `YuyinODS-RUNTIME-CLOUD-OVERLAY-MISSING`: `YUYINODS_MODE=cloud` but cached
  `.compose-flags` does not include `docker-compose.cloud.yml`.
- `YuyinODS-RUNTIME-CLOUD-LLM-LOCAL-ROUTE`: cloud mode still has `LLM_API_URL`
  pointing at local `llama-server`.
- `YuyinODS-RUNTIME-CLOUD-HERMES-LOCAL-ROUTE`: cloud mode still has Hermes pointing
  at local `llama-server`.
- `YuyinODS-RUNTIME-CLOUD-GATEWAY-BYPASS`: cloud mode points ODS services somewhere
  other than the LiteLLM gateway.
- `YuyinODS-RUNTIME-EXTERNAL-LEMONADE-CLOUD-OVERLAY-MISSING`: external Lemonade is
  active while cached `.compose-flags` lacks the cloud overlay that profiles
  out managed local inference.
- `YuyinODS-RUNTIME-EXTERNAL-LEMONADE-OVERLAY-MISSING`: external Lemonade is active
  while cached `.compose-flags` lacks `docker-compose.lemonade-external.yml`.
- `YuyinODS-RUNTIME-EXTERNAL-LEMONADE-LOCAL-ROUTE`: external Lemonade still routes
  clients to local `llama-server`.
- `YuyinODS-RUNTIME-LOCAL-CLOUD-OVERLAY`: local mode still has the cloud overlay in
  cached `.compose-flags`.
- `YuyinODS-RUNTIME-LOCAL-LITELLM-ROUTE`: non-AMD local mode unexpectedly routes
  through LiteLLM.
...
recorded `YUYINODS_MODE`, external Lemonade flags, and AMD runtime ownership so
...
`/tmp/yuyinods-doctor-report.json`
