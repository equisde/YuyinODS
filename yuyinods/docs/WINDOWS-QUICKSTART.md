...
(or `$env:YUYINODS_HOME` if you set it before installing). That runtime directory
...
volumes will point at the wrong data directory. Use `.\yuyinods.ps1` from the
...
Manage ODS using `yuyinods.ps1` from your runtime directory:
...
.\yuyinods.ps1 start               # Start all services
.\yuyinods.ps1 stop                # Stop all services
.\yuyinods.ps1 restart             # Restart all services
.\yuyinods.ps1 logs llama-server   # Tail logs (any service name)
.\yuyinods.ps1 update              # Pull latest images and restart
.\yuyinods.ps1 report              # Generate diagnostics bundle
```

For development installs where you intentionally want the runtime files inside
your working tree, set `YUYINODS_HOME` before running the installer:

```powershell
$env:YUYINODS_HOME = "C:\path\to\ODS\ods"
.\install.ps1
```
...
| What | Where |
|------|-------|
| Install directory | `$env:USERPROFILE\ods` by default; override with `-InstallDir` or `YUYINODS_HOME` |
| Config | `$installDir\.env` |
| Models | `$installDir\data\models\` |
| Logs | `.\yuyinods.ps1 logs <service>` or `docker compose logs` after `cd $installDir` |
