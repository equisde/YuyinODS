...
3. Before or after install, run **`./scripts/linux-install-preflight.sh`** (or `./yuyinods-preflight.sh --install-env`) for a structured environment report with stable check IDs and optional `--json` output.
4. After a real install, run `./yuyinods-preflight.sh` (service health) and `scripts/yuyinods-doctor.sh` if you use them.
