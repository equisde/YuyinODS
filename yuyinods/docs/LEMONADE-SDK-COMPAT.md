...
To wrap an existing Lemonade install without YuyinODS-managed voice or image
services:
...
ODS containers on `yuyinods-network` to reach the configured Lemonade port. If
that automatic rule cannot be added, allow the `yuyinods-network` subnet to reach
...
`LITELLM_LEMONADE_API_KEY=sk-yuyinods-lemonade-*` value is only the key LiteLLM
...
Use this to distinguish Lemonade service/network issues from YuyinODS-managed
container failures.
