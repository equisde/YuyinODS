...
`/tmp/dream-fleet-heavy.lock`. Set `YUYINODS_FLEET_HOST_LOCK` to override it;
...
distrobox create --name yuyinods-test-fedora --image fedora:41
distrobox create --name yuyinods-test-arch --image archlinux:latest
distrobox create --name yuyinods-test-manjaro --image manjarolinux/base:latest
distrobox create --name yuyinods-test-cachyos --image cachyos/cachyos:latest
distrobox create --name yuyinods-test-opensuse --image opensuse/tumbleweed:latest
distrobox create --name yuyinods-test-debian --image debian:12
distrobox create --name yuyinods-test-ubuntu2204 --image ubuntu:22.04
distrobox create --name yuyinods-test-mint213 --image linuxmintd/mint21.3-amd64:latest
distrobox create --name yuyinods-test-rocky9 --image rockylinux:9
```
...
distrobox enter yuyinods-test-fedora
...
# Exit and switch
exit
distrobox enter yuyinods-test-arch
```
...
5. Open terminal:
   ```bash
   git clone --depth 1 https://github.com/equisde/YuyinODS.git
   cd ODS
   ./install.sh
   ```
6. Note what breaks
...
5. Test with Distrobox: `distrobox create --name yuyinods-test-newdistro --image newdistro:latest`
