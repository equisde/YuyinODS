...
## Docker Compose failed (during install or `yuyinods.ps1`)

If **`install-windows.ps1`** stops with **docker compose up failed**, or **`.\yuyinods.ps1`** reports a compose error on **start / stop / restart / update**, the installer and CLI print a **COMPOSE FAILURE DIAGNOSTICS** block. Please save that entire block when asking for help (Discord, GitHub, etc.).
...
2. **GitHub Issues:** https://github.com/equisde/YuyinODS/issues
