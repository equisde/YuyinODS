...
```
  Browser
     ...
     ▼
  ┌─────────────────────────────────────┐
  │  yuyinods-hermes-proxy                 │
  │    forward_auth → dashboard-api     │
  │    reverse_proxy → yuyinods-hermes     │
  └─────────────────────────────────────┘
                 │
                 │  Docker network: yuyinods-hermes:9119
                 ▼
  ┌─────────────────────────────────────┐
  │  yuyinods-hermes container             │
  │                                     │
  │   hermes gateway run                │
  │     - HERMES_DASHBOARD=1 →          │
...
- **Persona (`SOUL.md`):** a generalist YuyinODS-aware persona (see `extensions/services/hermes/SOUL.md.template`)
...
# 5. Smoke test:
#    ods restart hermes
#    docker inspect --format '{{.State.Health.Status}}' yuyinods-hermes
...
- **mDNS announcement** — register `hermes.<device>.local` in the ODS mDNS announcer. ✅ shipped as [#1167](https://github.com/equisde/YuyinODS/pull/1167) (stacked on [#1152](https://github.com/equisde/YuyinODS/pull/1152)).
...
- **YuyinODS-side status panel** — surface Hermes's session count + skill inventory in the ODS dashboard. Lower priority since Hermes has its own `AnalyticsPage`.
...
### `docker inspect yuyinods-hermes` is not healthy, or `localhost:9120` returns 502

Hermes hasn't finished bootstrapping. Watch the logs:

```bash
docker logs -f yuyinods-hermes
```
...
```bash
ods doctor
docker exec yuyinods-hermes sh -c "ps -eo pid=,etimes=,args= | grep '[t]ui_gateway[.]slash_worker'"
```
...
```bash
docker exec yuyinods-hermes curl -fs http://llama-server:8080/v1/models
```
