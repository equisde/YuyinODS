...
The Lemonade provider may be either YuyinODS-managed on platforms where that is
...
| Variable | Meaning |
|----------|---------|
| `YUYINODS_MODE=lemonade` | ODS is running with the Lemonade provider |
...
- All YuyinODS-owned clients must pass the configured provider key before key
