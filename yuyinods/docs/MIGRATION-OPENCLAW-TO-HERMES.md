...
`config/n8n/openclaw-agent-trigger.json` still ships in this release and continues to point at OpenClaw's port 7860. A `hermes-agent-trigger.json` ships alongside it for Hermes. In the default auth-gated stack, users enter through port 9120 and containers call Hermes on the internal Docker network at `yuyinods-hermes:9119`.
...
- All `ENABLE_OPENCLAW` / `--openclaw` / `--no-openclaw` references in `install-core.sh` and `yuyinods-uninstall.sh`
...
File an issue at <https://github.com/equisde/YuyinODS/issues> with the `migration` label.
