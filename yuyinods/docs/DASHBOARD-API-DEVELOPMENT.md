...
- Editing `.py` files under `extensions/services/dashboard-api/` on the host **does not** reload the running `yuyinods-dashboard-api` container. The image bakes a copy into `/app/` at build time.
...
2. The compose service mounts the host repo at `/yuyinods` (read-only) — see `docker-compose.base.yml`:

   ```yaml
   volumes:
     - ./scripts:/yuyinods/scripts:ro
     - ./config:/yuyinods/config:ro
     - ./extensions:/yuyinods/extensions:ro
     - ./.env:/yuyinods/.env:ro
     ...
   ```

   That bind-mount exists so the API can **read manifests, scripts, and config** at runtime. It is not on Python's import path. Editing `yuyinods/extensions/services/dashboard-api/routers/setup.py` on the host updates `/yuyinods/extensions/services/dashboard-api/routers/setup.py` inside the container — a path nothing imports from. The running uvicorn keeps serving the baked `/app/routers/setup.py`.
...
You can keep the rest of the stack (`llama-server`, `yuyinods-host-agent`, `qdrant`, etc.) running. Only the two dashboard containers need to step aside.

### One-time setup

```bash
# API venv
cd yuyinods/extensions/services/dashboard-api
python -m venv .venv
...
```bash
cd yuyinods/extensions/services/dashboard
npm run dev
```

Terminal 2 — native uvicorn (claims host port 3002). Set `YUYINODS_INSTALL_DIR` to the path of your installed ODS repo (the same path the container sees as `/yuyinods`):

```bash
cd yuyinods/extensions/services/dashboard-api
source .venv/bin/activate
YUYINODS_INSTALL_DIR=/path/to/yuyinods \
YUYINODS_DATA_DIR=/path/to/yuyinods/data \
YUYINODS_AGENT_HOST=127.0.0.1 \
YUYINODS_AGENT_PORT=7710 \
DASHBOARD_API_KEY="$(grep ^DASHBOARD_API_KEY /path/to/yuyinods/.env | cut -d= -f2-)" \
YUYINODS_AGENT_KEY="$(grep ^YUYINODS_AGENT_KEY /path/to/yuyinods/.env | cut -d= -f2-)" \
  uvicorn main:app --host 127.0.0.1 --port 3002 --reload
```

`YUYINODS_AGENT_HOST=127.0.0.1` is required when running uvicorn natively on the host — `config.py` defaults it to `host.docker.internal`, which Docker Desktop only injects inside containers. Without this override, any host-agent-backed dashboard-api route (extensions install/start, model download, `/v1/env/update`, etc.) fails DNS resolution and returns 502.
...
Mirror any other env vars your code path reads (`OLLAMA_URL`, `LLM_MODEL`, `KOKORO_URL`, etc.) from the values in `.env`. A small wrapper script that exports the relevant subset is a reasonable thing to keep locally. Note that `DASHBOARD_API_KEY` and `YUYINODS_AGENT_KEY` are independent secrets (since PR #979 they are no longer aliased) — if you only export one, calls in the direction that needs the other will return 401.
...
```bash
cd yuyinods/extensions/services/dashboard-api
docker cp routers/setup.py yuyinods-dashboard-api:/app/routers/setup.py
...
services:
  dashboard-api:
    command: uvicorn main:app --host 0.0.0.0 --port 3002 --reload --reload-dir /yuyinods/extensions/services/dashboard-api
```

The trap: `WORKDIR` is still `/app`. uvicorn launches in `/app`, imports `main:app` from `/app/main.py`, and watches `/yuyinods/extensions/services/dashboard-api/` for file changes. The watcher fires correctly, uvicorn reloads — and re-imports the **same baked `/app/` code**. The reload appears to work and the change appears to do nothing.

To actually pick up host edits, the overlay has to also change `WORKDIR` (or wrap the command in `bash -c "cd /yuyinods/extensions/services/dashboard-api && uvicorn ..."`) and ensure Python's import path points at the bind-mount. At that point you've effectively re-implemented Option B inside an overlay, with all of its drawbacks plus a confusing dev-only command. Running uvicorn on the host is shorter and clearer.
