...
| Mode | `LLM_API_URL` | `YUYINODS_MODE` | LiteLLM config |
|------|---------------|--------------|-----------------|
...
|----------|---------|-------------|
| `YUYINODS_MODE` | `local` | Active mode: `local`, `cloud`, or `hybrid`; `lemonade` is auto-set on AMD (not user-switchable) |
...
This sets `YUYINODS_MODE=cloud`, `LLM_API_URL=http://litellm:4000`, and auto-enables the LiteLLM extension.
...
| `.env` | Stores `YUYINODS_MODE`, `LLM_API_URL`, API keys |
