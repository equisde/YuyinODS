...
ODS announces itself on your local network so you can browse to it from any phone, tablet, or laptop without knowing the IP. The URL is `http://yuyinods.local` (or `http://<your-name>.local` if you renamed the device during setup).

## Prerequisites

The mDNS announcement publishes the device's LAN IP under `<device>.local` and the proxy-routed subdomains. For `http://yuyinods.local` to actually load chat from a phone, the `yuyinods-proxy` extension must be enabled and LAN-bound:

1. **The `yuyinods-proxy` extension is enabled** — that's the Caddy service that listens on port 80 and routes by hostname (`chat.<device>.local`, `dashboard.<device>.local`, `auth.<device>.local`, etc.) to the right backend.
2. **`YUYINODS_PROXY_BIND=0.0.0.0`** — this is the proxy default. The other ODS services can remain safely loopback-bound with `BIND_ADDRESS=127.0.0.1`; only the proxy needs to listen on the LAN.

Fresh installs without the proxy are still loopback-only. The installer's first-boot wizard offers to enable the proxy path. See `docs/YuyinODS-PROXY.md` for how the proxy routes traffic.
...
All seven names resolve to the same IP — the device's LAN address. Routing happens at the yuyinods-proxy by Host header (see `docs/YuyinODS-PROXY.md`). The bare `<device>.local` redirects to chat for the friendliest landing.
...
Edit `.env` and change `YUYINODS_DEVICE_NAME` to whatever you want (letters, digits, hyphens; max 32 chars). The mDNS service polls `.env` every 30 seconds and re-announces automatically — no restart needed.

If you want to force immediate re-announcement: `sudo systemctl restart yuyinods-mdns`.

### Running multiple ODSs on one network

Give each one a unique `YUYINODS_DEVICE_NAME`. Two devices both calling themselves `ods.local` is undefined behavior — the most recent announcement usually wins but it depends on the OS and the timing. The recommended pattern: `kitchen.local`, `office.local`, `studio.local`.

### Disabling mDNS entirely

```bash
sudo systemctl stop yuyinods-mdns
sudo systemctl disable yuyinods-mdns
```
...
Once `ods.local` resolves on your network **and the yuyinods-proxy is up on port 80** (see [Prerequisites](#prerequisites)), the Phase 1 onboarding UX works end to end:

1. User installs ODS (today: by running `install.sh`)
2. Device joins WiFi, starts announcing, and yuyinods-proxy comes up on :80
