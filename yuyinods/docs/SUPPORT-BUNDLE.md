...
`scripts/yuyinods-support-bundle.sh` creates a redacted diagnostics archive that can
...
```bash
# Create artifacts/support/yuyinods-support-<timestamp>.tar.gz
scripts/yuyinods-support-bundle.sh

# Write under a custom directory
scripts/yuyinods-support-bundle.sh --output /tmp/yuyinods-support

# Skip container logs
scripts/yuyinods-support-bundle.sh --no-logs

# Print machine-readable result JSON
scripts/yuyinods-support-bundle.sh --json
```

## What It Collects

- ODS Doctor output, when `scripts/yuyinods-doctor.sh` can run
