...
- The model file itself was viable:
  - Path: `$HOME/yuyinods/data/models/qwen3-coder-next-Q4_K_M.gguf`
...
```text
yuyinods-llama-cpp-gb10:sm121-2496f9c
```
...
Updated the live install at `$HOME/yuyinods/.env`:

```env
LLAMA_SERVER_IMAGE=yuyinods-llama-cpp-gb10:sm121-2496f9c
```

Validation after `./yuyinods-cli start llm`:

- `yuyinods-llama-server` is healthy.
