...
```bash
docker network inspect yuyinods-network
```
...
```bash
cd ~/yuyinods  # or wherever you installed
...
```bash
cd ~/yuyinods
tar -czvf yuyinods-backup-$(date +%Y%m%d).tar.gz data/
```
...
cd ~/yuyinods
docker compose down
tar -xzvf yuyinods-backup-YYYYMMDD.tar.gz
docker compose up -d
```
...
**Run manually** from your install directory (e.g. `~/yuyinods`):
...
5. **Open an issue:** https://github.com/equisde/YuyinODS/issues
