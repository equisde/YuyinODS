...
```
        ...
        │  browser opens http://yuyinods.tail-xxxxx.ts.net   │
        └──────────────────────┬──────────────────────────┘
                               │
                               │  encrypted WireGuard
                               ▼
...
1. **`yuyinods-proxy` is enabled.** It listens on port 80 and routes `/chat`, `/api/*`, `/auth/*` to the right backend. With it, tailnet clients browse to `http://yuyinods.tail-xxxxx.ts.net` (no port). Without it, only the per-service ports work (`:3000`, `:3001`, `:3002`) and only if those are LAN-bound.
...
If you ship a fresh install with Tailscale enabled but neither of the above, `tailscale status` will show the device authed but `http://yuyinods.tail-xxxxx.ts.net` will hang or refuse. Until the dashboard gets a dedicated Remote Access UI, verify those prerequisites with `.env`, `ods list`, and the status endpoint below.
...
TS_HOSTNAME=                # leave blank — defaults to YUYINODS_DEVICE_NAME
...
```bash
docker exec yuyinods-tailscale tailscale status
...
# AND with yuyinods-proxy enabled + BIND_ADDRESS=0.0.0.0 on the device:
curl http://yuyinods.tail-abcde.ts.net/api/status
# Or from a browser: http://yuyinods.tail-abcde.ts.net (lands at /chat via the proxy)

# To bypass the proxy and hit a specific service directly, use its port
# (only works when BIND_ADDRESS=0.0.0.0):
curl http://yuyinods.tail-abcde.ts.net:3001  # dashboard direct
```
