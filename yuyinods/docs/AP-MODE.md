...
```
        ...
        │  joins "YuyinODS-Setup-XXXX" AP via QR scan      │
        ...
        └─────────────────────────────────────────────┘
                             ...
                             ▼
        ┌─────────────────────────────────────────────┐
        │  yuyinods-proxy (Caddy) on 0.0.0.0:80          │
        ...
        Once the wizard completes:
          ...
          * operator (or PR-11) runs `systemctl disable --now yuyinods-ap-mode`
...
1. **`yuyinods-proxy` is enabled and running.** That's the Caddy service that listens on port 80 and routes `/setup`, `/chat`, `/api/*`, `/auth/*` to the right backend. Without it, AP clients hit an empty port 80 and the connection fails. The first-boot install flow enables it by default.
...
|---|---|---|
| `ap-mode.sh` | `scripts/ap-mode.sh` | Bring-up / tear-down / status. Reads config from `/etc/yuyinods/ap-mode.conf`. |
| `yuyinods-ap-mode.service` | `scripts/systemd/yuyinods-ap-mode.service` | systemd unit. Disabled by default — enable per-device. |
| `ap-mode.conf.example` | `scripts/ap-mode.conf.example` | Annotated example operator config. |
| `/v1/ap-mode/status` | `bin/yuyinods-host-agent.py` | Read-only status endpoint. Used by the wizard to know "am I running on the device's own AP?" |
...
sudo install -d -m 0755 /etc/yuyinods
sudo install -m 0600 /yuyinods/scripts/ap-mode.conf.example /etc/yuyinods/ap-mode.conf
sudo $EDITOR /etc/yuyinods/ap-mode.conf  # set YUYINODS_AP_SSID + YUYINODS_AP_PASSWORD

# 3. Install the systemd unit. Manual installs must render __INSTALL_DIR__
#    before copying the unit into /etc/systemd/system.
sudo cp /yuyinods/scripts/systemd/yuyinods-ap-mode.service /etc/systemd/system/
sudo sed -i 's|__INSTALL_DIR__|/yuyinods|g' /etc/systemd/system/yuyinods-ap-mode.service
sudo systemctl daemon-reload

# 4. Enable + start.
sudo systemctl enable --now yuyinods-ap-mode

# 5. Verify.
sudo systemctl status yuyinods-ap-mode
sudo /yuyinods/scripts/ap-mode.sh status
```

When the wizard finishes and the device should join the home network instead:

```bash
sudo systemctl disable --now yuyinods-ap-mode
```
...
All settings are bash variables sourced from `/etc/yuyinods/ap-mode.conf`. See `scripts/ap-mode.conf.example` for the annotated version.

| Variable | Default | Notes |
|---|---|---|
| `YUYINODS_AP_SSID` | `YuyinODS-Setup` | Network name. Include a per-unit suffix to avoid collisions. |
| `YUYINODS_AP_PASSWORD` | empty | WPA2 passphrase. Empty → open AP (allowed but warned). The example placeholder is refused so images do not ship a known password. |
| `YUYINODS_AP_INTERFACE` | `wlan0` | Must support AP mode. Check with `iw list \| grep -A4 'Supported interface modes' \| grep AP`. |
| `YUYINODS_AP_GATEWAY_IP` | `192.168.7.1` | IP that resolves every hostname (captive-portal trick). |
| `YUYINODS_AP_NETMASK` | `255.255.255.0` | |
| `YUYINODS_AP_DHCP_RANGE` | `192.168.7.10,192.168.7.50,1h` | `<start>,<end>,<lease-time>` |
| `YUYINODS_AP_CHANNEL` | `6` | 2.4 GHz only; 1 / 6 / 11 are the non-overlapping channels. |
...
- **No automatic teardown.** The wizard (PR-11) will call `systemctl disable --now yuyinods-ap-mode` after setup completes. Until that lands, the operator does it manually.

## Security notes

- The unit runs `hostapd` and `dnsmasq` as root because they bind privileged sockets. That's the standard pattern; this isn't a "YuyinODS-specific" elevation.
...
- `iptables` rules are tagged with `--comment yuyinods-ap-mode` so `ap-mode.sh down` removes exactly those rules and nothing else.
...
```bash
sudo hostapd /run/yuyinods-ap-mode/hostapd.conf
```
...
```bash
cat /run/yuyinods-ap-mode/dnsmasq.log
```
...
`systemctl disable --now yuyinods-ap-mode` and `sudo nmcli device set wlan0 managed yes`. NetworkManager should pick the interface back up within a few seconds.
