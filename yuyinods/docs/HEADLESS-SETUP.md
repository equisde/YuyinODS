...
- mDNS plus `yuyinods-proxy` provide the friendly LAN URLs. Fleet validation
...
1. The device is pre-installed with ODS and given a friendly
   `YUYINODS_DEVICE_NAME` such as `ods`, `studio`, or `kitchen-ai`.
...
| Wi-Fi setup API | [`extensions/services/dashboard-api/routers/setup.py`](../extensions/services/dashboard-api/routers/setup.py), [`bin/yuyinods-host-agent.py`](../bin/yuyinods-host-agent.py) | [`test_network_config.py`](../extensions/services/dashboard-api/tests/test_network_config.py), [`test_host_agent.py`](../extensions/services/dashboard-api/tests/test_host_agent.py) | Lets the dashboard ask the host agent to scan, connect, check status, and forget Wi-Fi profiles. |
| First-boot AP mode | [`scripts/ap-mode.sh`](../scripts/ap-mode.sh), [`scripts/systemd/yuyinods-ap-mode.service`](../scripts/systemd/yuyinods-ap-mode.service), [`scripts/ap-mode.conf.example`](../scripts/ap-mode.conf.example) | [`test-ap-mode.sh`](../tests/test-ap-mode.sh), [`AP-MODE.md`](AP-MODE.md) | Optional setup access point for devices that are not yet on the user's network. |
| LAN discovery | [`bin/yuyinods-mdns.py`](../bin/yuyinods-mdns.py), [`scripts/systemd/yuyinods-mdns.service`](../scripts/systemd/yuyinods-mdns.service) | [`MDNS.md`](MDNS.md) | Publishes `<device>.local` and service subdomains on the local network. |
| LAN reverse proxy | [`extensions/services/yuyinods-proxy/Caddyfile`](../extensions/services/yuyinods-proxy/Caddyfile), [`extensions/services/yuyinods-proxy/compose.yaml`](../extensions/services/yuyinods-proxy/compose.yaml) | [`YuyinODS-PROXY.md`](YuyinODS-PROXY.md) | Routes `chat.<device>.local`, `dashboard.<device>.local`, `talk.<device>.local`, `auth.<device>.local`, `api.<device>.local`, and `hermes.<device>.local`. |
...
2. Set a unique `YUYINODS_DEVICE_NAME` in `.env`.
3. Set `YUYINODS_SESSION_SECRET` so magic-link redemption can issue signed
   `yuyinods-session` cookies.
4. Enable the LAN entry path:
   - `yuyinods-proxy` for friendly HTTP routing;
   - `yuyinods-mdns` for local `.local` discovery where supported;
   - `hermes-proxy` if the Hermes Agent should be LAN-reachable behind auth.
5. For AP-based out-of-box setup, write `/etc/yuyinods/ap-mode.conf`, install the
   `yuyinods-ap-mode.service` unit, and generate a setup card or factory owner card.
...
- `yuyinods-proxy` answers on port 80 from another device on the LAN.
