...
by default, or in `$env:YUYINODS_HOME` if you set that variable before install.
...
cd $installDir
.\yuyinods.ps1 status
.\yuyinods.ps1 logs llama-server
```
...
For in-place development only, set `YUYINODS_HOME` before running the installer so
...
docker exec -it yuyinods-llama-server-1 nvidia-smi
```
...
| Task | Command |
|------|---------|
| Stop ODS | `cd $installDir; .\yuyinods.ps1 stop` |
| Start ODS | `cd $installDir; .\yuyinods.ps1 start` |
| View logs | `cd $installDir; .\yuyinods.ps1 logs` |
| Update | `cd $installDir; .\yuyinods.ps1 update` |
| Enable voice | Add `-Voice` flag or edit `.env` |
| Enable workflows | Add `-Workflows` flag |
| Support report | `cd $installDir; .\yuyinods.ps1 report` |
