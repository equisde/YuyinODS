...
| Build a custom downstream edition or appliance | Fork / downstream build | [BUILD-ON-YuyinODS-SERVER.md](BUILD-ON-YuyinODS-SERVER.md) |
...
- set `service.container_name` (typically `yuyinods-<id>`)
...
services:
  my-service:
    ...
    container_name: yuyinods-my-service
...
The service automatically joins `yuyinods-network` and can reach other services by Docker DNS name.
...
services:
  whisper:
    ...
    container_name: yuyinods-whisper
...
services:
  comfyui:
    ...
    container_name: yuyinods-comfyui
    restart: unless-stopped
    ports:
...
2. Locates the extension in the **extensions library** (`$YUYINODS_DATA_DIR/extensions-library/<id>/`)
...
4. Copies the extension to `$YUYINODS_DATA_DIR/user-extensions/<id>/` atomically via a temp directory on the same filesystem
...
The [host agent](HOST-AGENT-API.md) (`bin/yuyinods-host-agent.py`) runs on the host machine and exposes `/v1/extension/start`, `/v1/extension/stop`, and `/v1/extension/logs`. When the Dashboard API calls start/stop, the host agent:
