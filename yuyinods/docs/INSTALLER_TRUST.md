...
```bash
https://raw.githubusercontent.com/equisde/YuyinODS/main/yuyinods/get-yuyinods.sh
```
...
- clones `https://github.com/equisde/YuyinODS.git` with sparse
  checkout for the `yuyinods/` product tree;
- copies the runtime product files into `~/yuyinods`;
- runs `./install.sh` from that copied runtime tree.

By default the bootstrap follows `main`. To select a published release tag or
another branch, set `YUYINODS_REF` before running the script:

```bash
YUYINODS_REF=main bash get-yuyinods.sh
```
...
```bash
git clone --depth 1 --branch main https://github.com/equisde/YuyinODS.git
cd ODS/yuyinods
./install.sh
```
...
`$env:USERPROFILE\ods` by default, or `$env:YUYINODS_HOME` if set.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/equisde/YuyinODS/main/yuyinods/get-yuyinods.sh
less get-yuyinods.sh
YUYINODS_REF=main bash get-yuyinods.sh
```
