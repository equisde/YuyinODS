#!/bin/bash
# yuyinods-restore.sh - ODS Restore Utility
...
ensure_restore_space() {
    ...
    local free
    free=$(free_bytes_for_path "$YUYINODS_DIR")

    if [[ -n "$free" && "$free" -gt 0 && "$free" -lt "$need" ]]; then
        log_error "Not enough disk space to restore into: $YUYINODS_DIR"
        log_error "Need ~$(fmt_bytes "$need"), have ~$(fmt_bytes "$free")."
        log_error "Free up space or restore to a different location (set YUYINODS_DIR)."
        return 1
    fi
...
dry_run_preview() {
    ...
    if [[ "$restore_config" == "true" ]]; then
        ...
        for file in "$backup_dir"/.env "$backup_dir"/.version "$backup_dir"/docker-compose*.y*ml "$backup_dir"/yuyinods-*.sh; do
            if [[ -f "$file" ]]; then
                echo "  ✓ $(basename "$file")"
            fi
        done
...
stop_containers() {
    ...
    if ! docker compose ls --quiet 2>/dev/null | grep -q "$(basename "$YUYINODS_DIR")"; then
...
restore_user_data() {
    ...
    for dir in "${data_dirs[@]}"; do
        if [[ -d "$backup_dir/$dir" ]]; then
            restored_any=true
            mkdir -p "$YUYINODS_DIR/$(dirname "$dir")"
            ...
            rsync_with_progress "$backup_dir/$dir" "$YUYINODS_DIR/$(dirname "$dir")/" "Restoring $dir"
...
restore_config() {
    ...
    if [[ -d "$backup_dir/config" ]]; then
        ...
        if [[ -d "$YUYINODS_DIR/config" ]]; then
            rm -rf "$YUYINODS_DIR/config"
        fi
        cp -r "$backup_dir/config" "$YUYINODS_DIR/"
...
verify_restore() {
    ...
    if [[ ! -d "$YUYINODS_DIR/data/open-webui" ]]; then
...
do_restore() {
    ...
    if [[ "$force" != "true" ]]; then
        echo ""
        log_warn "This will copy backup data into: $YUYINODS_DIR"
    ...
    echo ""
    echo "Next steps:"
    echo "  1. Review restored configuration: cat $YUYINODS_DIR/.env"
    echo "  2. Start services: docker compose up -d"
    echo "  3. Check status: ./yuyinods-preflight.sh"
}
