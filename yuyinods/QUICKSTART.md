...
YuyinODS downloads a small model first so you can start chatting quickly,
...
```bash
ods status
tail -f ~/yuyinods/logs/model-upgrade.log
```
...
```bash
ods status
ods chat "Say exactly: YuyinODS is ready."
ods doctor
```
...
```bash
cd ~/yuyinods
./yuyinods-preflight.sh
./scripts/yuyinods-test.sh
```
...
curl "http://localhost:${LLM_PORT:-11434}/v1/chat/completions" \
  ...
  -d "{
    ...
    \"messages\": [{\"role\": \"user\", \"content\": \"Hello from YuyinODS\"}]
  }"
```
