...
Publishes the device on the local network as `<YUYINODS_DEVICE_NAME>.local`
...
When paired with yuyinods-proxy on port 80, this makes the "open `ods.local`
from any phone" UX work: the device boots, joins WiFi, and starts announcing
itself within seconds.

Reads `YUYINODS_DEVICE_NAME` and the service ports from `.env`. Re-publishes
...
Run via:
  python3 /opt/yuyinods/bin/yuyinods-mdns.py
or via the yuyinods-mdns.service systemd unit.
"""
...
logging.basicConfig(
    level=os.environ.get("YUYINODS_MDNS_LOG_LEVEL", "INFO"),
    format="%(asctime)s [yuyinods-mdns] %(levelname)s %(message)s",
)
logger = logging.getLogger(__name__)

INSTALL_DIR = Path(os.environ.get("YUYINODS_INSTALL_DIR", "/opt/yuyinods"))
...
POLL_INTERVAL = _safe_positive_int_env("YUYINODS_MDNS_POLL_INTERVAL", 30)

# Hostname-safe pattern matches YUYINODS_DEVICE_NAME schema in .env.schema.json.
...
def _direct_ports_lan_reachable(env: dict[str, str]) -> bool:
    ...
    yuyinods-proxy on the LAN. In that default, advertise the proxy hostnames but
...
def _build_services(env: dict[str, str], device_name: str, ip: str) -> list[ServiceInfo]:
    ...
    2. Per-subdomain A records (`chat.<device>.local`, `auth.<device>.local`,
       ...
       the device's LAN IP on port 80 — the yuyinods-proxy. The proxy
    ...
    YUYINODS_DEVICE_NAME is valid — the proxy itself decides at routing
    time whether the upstream is healthy.
    """
    addresses = [socket.inet_aton(ip)]
    proxy_port = _safe_port(env, "YUYINODS_PROXY_PORT", 80)
    ...
    subdomain_routes = (
        ...
        # phones at this hostname; yuyinods-proxy's Caddy block routes it to
...
class Announcer:
    ...
    def _config_signature(self, device_name: str, ip: str, env: dict[str, str]) -> tuple[str, str, str, int, int, int, int, int]:
        ...
        Includes YUYINODS_PROXY_PORT so that flipping the proxy to a non-
        ...
        return (
            ...
            _safe_port(env, "YUYINODS_PROXY_PORT", 80),
        )

    def refresh(self) -> None:
        env = _read_env()
        device_name = env.get("YUYINODS_DEVICE_NAME", "ods") or "ods"
        if not _HOSTNAME_RE.match(device_name):
            logger.warning(
                "YUYINODS_DEVICE_NAME %r is not hostname-safe; falling back to 'ods'",
                device_name,
            )
            device_name = "ods"
        ip = _get_local_ip()
