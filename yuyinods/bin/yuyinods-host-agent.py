...
logger = logging.getLogger("yuyinods-host-agent")
...
# Update lock/state: only one background yuyinods-update run at a time.
...
def _resolve_agent_bind_addr(env: dict, system_name: str | None = None) -> str:
    ...
    explicit = env.get("YUYINODS_AGENT_BIND", "").strip()
    if explicit:
        return explicit
    ...
    if system_name == "Linux":
        ...
        return (
            _detect_docker_network_gateway("yuyinods-network")
...
def resolve_compose_flags() -> list:
    ...
    if platform.system() == "Windows":
        ...
        env["YUYINODS_PYTHON_CMD"] = _to_bash_path(Path(sys.executable))
    cmd = [
...
def _run_post_install_hook(service_id: str, ext_dir: Path) -> tuple[bool, str]:
    ...
    hook_env = {
        ...
        "YUYINODS_VERSION": YUYINODS_VERSION,
...
def _enable_retry_work(service_id: str) -> None:
    ...
    try:
        ...
        if not isinstance(retry_service_def, dict):
            retry_service_def = {}
        container_name = retry_service_def.get("container_name") or f"yuyinods-{service_id}"
...
def _resolve_container_name(service_id: str) -> str:
    ...
    Falls back to yuyinods-{service_id} convention if label lookup fails.
    """
    try:
        result = subprocess.run(
            ["docker", "ps", "--filter",
...
def _service_has_docker_container(service_id: str) -> tuple[bool, str]:
    ...
    container_name = service_def.get("container_name", f"yuyinods-{service_id}")
...
def _find_update_script() -> Path | None:
    for candidate in (
        INSTALL_DIR / "yuyinods-update.sh",
        INSTALL_DIR / "scripts" / "yuyinods-update.sh",
        INSTALL_DIR.parent / "scripts" / "yuyinods-update.sh",
    ):
        if candidate.exists():
            return candidate
    return None
...
def _run_update_script(action: str, *args: str, timeout: int | None) -> subprocess.CompletedProcess:
    ...
    if script is None:
        raise FileNotFoundError("yuyinods-update.sh not found")
...
class AgentHandler(BaseHTTPRequestHandler):
    ...
    def _handle_tailscale_status(self):
        ...
        try:
            result = subprocess.run(
                ["docker", "exec", "yuyinods-tailscale",
                 "tailscale", "status", "--json"],
                capture_output=True, text=True, timeout=10,
            )
        except subprocess.TimeoutExpired:
    ...
    def _handle_ap_mode_status(self):
        ...
        Reads /run/yuyinods-ap-mode/state.json which ap-mode.sh writes
        ...
        if not check_auth(self):
            return
        state_path = Path("/run/yuyinods-ap-mode/state.json")
    ...
    def _handle_service_stats(self):
        """Return CPU/memory stats for all YuyinODS-managed containers."""
        if not check_auth(self):
            return
        ...
        try:
            ...
            for line in result.stdout.strip().splitlines():
                ...
                name = raw.get("name", "")
                if not name.startswith("yuyinods-"):
                    continue
                ...
                service_id = name.removeprefix("yuyinods-")
    ...
    def _handle_update_action(self):
        """Run yuyinods-update.sh from the host-agent trust boundary."""
        if not check_auth(self):
            return
    ...
    def _handle_extension_sync_config(self):
        ...
        bind-mount of /yuyinods/config is read-only, so it cannot
    ...
    def _handle_logs(self):
        ...
        try:
            ...
            container_name = f"yuyinods-{service_id}"
        ...
        def _run_install():
            try:
                ...
                if not isinstance(install_service_def, dict):
                    install_service_def = {}
                container_name = install_service_def.get("container_name") or f"yuyinods-{service_id}"
        ...
        try:
            ...
            for hermes_path in (hermes_live_config, hermes_template_config):
                try:
                    ...
                    logger.warning(
                        ...
                        "`docker restart yuyinods-hermes` to pick up the new model.",
                        hermes_path,
                    )
            ...
            if hermes_template_patched and not hermes_patched:
                logger.warning(
                    ...
                    "yuyinods-hermes restart because it would keep using the old "
            ...
            env = load_env(env_path)
            _in_container = bool(os.environ.get("YUYINODS_HOST_INSTALL_DIR"))
            ...
            # - Inside a container (YUYINODS_HOST_INSTALL_DIR set): use docker
            ...
            elif os.environ.get("YUYINODS_HOST_INSTALL_DIR"):
                llama_host = "yuyinods-llama-server"
                llama_port = "8080"
            else:
                llama_host = "127.0.0.1"
            ...
            if healthy:
                ...
                ods_mode = env.get("YUYINODS_MODE", "local")
                if ods_mode == "lemonade":
                    _write_lemonade_config(INSTALL_DIR, gguf_file)

                # Restart dependent services so they pick up the new model
                dependent_services = ["yuyinods-litellm"]
                if hermes_patched:
                    dependent_services.append("yuyinods-hermes")
...
def _restart_windows_lemonade(env: dict):
    ...
    ps_env.update({
        "YUYINODS_WIN_LEMONADE_EXE": str(exe),
        "YUYINODS_WIN_MODELS_DIR": str(INSTALL_DIR / "data" / "models"),
        "YUYINODS_WIN_PID_FILE": str(INSTALL_DIR / "data" / "llama-server.pid"),
        "YUYINODS_WIN_LEMONADE_PORT": env.get("AMD_INFERENCE_PORT", "8080") or "8080",
        "YUYINODS_WIN_BIND_ADDR": env.get("BIND_ADDRESS", "127.0.0.1") or "127.0.0.1",
        "YUYINODS_WIN_LEMONADE_TASK": "ODSLemonadeRuntime",
    })
    script = r'''
$ErrorActionPreference = "Stop"
$exe = $env:YUYINODS_WIN_LEMONADE_EXE
$modelsDir = $env:YUYINODS_WIN_MODELS_DIR
$pidPath = $env:YUYINODS_WIN_PID_FILE
$port = [int]$env:YUYINODS_WIN_LEMONADE_PORT
$bindAddr = $env:YUYINODS_WIN_BIND_ADDR
$taskName = $env:YUYINODS_WIN_LEMONADE_TASK
...
def _write_lemonade_config(install_dir: Path, gguf_file: str):
    ...
    ods_mode = env.get("YUYINODS_MODE", "lemonade")
...
def _compose_restart_llama_server(env: dict):
    ...
    if gpu_backend == "amd":
        ...
        if compose_flags:
            ...
            _run(["docker", "stop", "yuyinods-llama-server"], 120)
            _run(["docker", "start", "yuyinods-llama-server"], 300)
    else:
...
def _recreate_llama_server(env: dict, override_image: str = ""):
    ...
    """
    container = "yuyinods-llama-server"
    gguf_file = env.get("GGUF_FILE", "")
    ctx_size = env.get("CTX_SIZE", "32768")
...
def _request_server_shutdown(server, signum=None):
    ...
    threading.Thread(
        ...
        name="yuyinods-host-agent-shutdown",
        daemon=True,
    ).start()
...
def main():
    ...
    global USER_EXTENSIONS_DIR, EXTENSIONS_DIR, YUYINODS_VERSION
    ...
    elif os.environ.get("YUYINODS_HOME"):
        INSTALL_DIR = Path(os.environ["YUYINODS_HOME"]).resolve()
    else:
    ...
    env = load_env(INSTALL_DIR / ".env")
    # Prefer dedicated YUYINODS_AGENT_KEY; fall back to DASHBOARD_API_KEY for
    # existing installs that haven't generated a separate key yet.
    AGENT_API_KEY = env.get("YUYINODS_AGENT_KEY", "") or env.get("DASHBOARD_API_KEY", "")
    if not AGENT_API_KEY:
        logger.error("Neither YUYINODS_AGENT_KEY nor DASHBOARD_API_KEY set in .env")
        sys.exit(1)
    GPU_BACKEND = env.get("GPU_BACKEND", "nvidia")
    TIER = env.get("TIER", "1")
    GPU_COUNT = env.get("GPU_COUNT", "1")

    DATA_DIR = Path(env.get("YUYINODS_DATA_DIR", str(INSTALL_DIR / "data")))
    USER_EXTENSIONS_DIR = Path(env.get(
        "YUYINODS_USER_EXTENSIONS_DIR",
        str(DATA_DIR / "user-extensions"),
    ))
    EXTENSIONS_DIR = INSTALL_DIR / "extensions" / "services"
    YUYINODS_VERSION = env.get("YUYINODS_VERSION", VERSION)

    port = args.port
    env_port = env.get("YUYINODS_AGENT_PORT", "")
    if port == 7710 and env_port:
        try:
            port = int(env_port)
        except ValueError:
            logger.warning("Invalid YUYINODS_AGENT_PORT in .env: %s", env_port)
    ...
    # default. Linux prefers the yuyinods-network gateway so dashboard-api
    ...
    # restart the service after yuyinods-network exists.
    ...
    if bind_addr == "0.0.0.0":
        logger.info(
            "Bound to all interfaces. Bearer-auth (YUYINODS_AGENT_KEY) is enforced "
            "on every endpoint. To restrict to a specific interface, set "
            "YUYINODS_AGENT_BIND=<ip> in %s/.env.",
            INSTALL_DIR,
        )
