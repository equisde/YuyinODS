...
YUYINODS_APP_DIR="${YUYINODS_APP_DIR:-${YUYINODS_INSTALL_DIR:-$YUYINODS_BASE_DIR/yuyinods}}"
...
YUYINODS_BOOTSTRAP_ROOT="${YUYINODS_TEMP_DIR:-${HOME:-/tmp}}"
if ! cd "$YUYINODS_BOOTSTRAP_ROOT" 2>/dev/null; then
    mkdir -p "$YUYINODS_BOOTSTRAP_ROOT" 2>/dev/null || true
fi
if ! cd "$YUYINODS_BOOTSTRAP_ROOT" 2>/dev/null; then
    YUYINODS_BOOTSTRAP_ROOT="/tmp"
    cd "$YUYINODS_BOOTSTRAP_ROOT" 2>/dev/null || {
...
REPO_URL="${YUYINODS_REPO_URL:-https://github.com/equisde/YuyinODS.git}"
INSTALL_DIR="${YUYINODS_HOME:-${YUYINODS_INSTALL_DIR:-$YUYINODS_APP_DIR}}"
LEGACY_DREAMSERVER_DIR="${DREAMSERVER_INSTALL_DIR:-$YUYINODS_BOOTSTRAP_ROOT/dream-server}"
YUYINODS_REF="${YUYINODS_REF:-${YUYINODS_BOOTSTRAP_REF:-}}"
...
refuse_legacy_dreamserver_install() {
    is_truthy "${YUYINODS_ALLOW_DREAMSERVER_PARALLEL:-}" && return 0
    ...
    if (( ${#findings[@]} > 0 )); then
        ...
        echo "  YUYINODS_ALLOW_DREAMSERVER_PARALLEL=1 YUYINODS_INSTALL_DIR=\"$INSTALL_DIR\" bash get-yuyinods.sh"
        echo ""
        exit 1
    fi
}
...
echo -e "${BOLD}${BLUE}"
cat << 'BANNER'
  YY   YY UU   UU YY   YY III NN   NN  OOOOO  DDDD   SSSSS
   YY YY  UU   UU  YY YY  III NNN  NN OO   OO DD DD SS
    YYY   UU   UU   YYY   III NN N NN OO   OO DD DD  SSS
    YYY   UU   UU   YYY   III NN  NNN OO   OO DD DD    SS
    YYY    UUUUU    YYY   III NN   NN  OOOOO  DDDD  SSSS
BANNER
echo -e "${NC}"
...
if [[ -d "$INSTALL_DIR" ]]; then
    if [[ -f "$INSTALL_DIR/.env" ]]; then
        ...
        echo "  To update:    cd $INSTALL_DIR && ./yuyinods-cli update"
        echo ""
        exit 0
    else
...
log "Cloning ODS..."
if [[ -n "$YUYINODS_REF" ]]; then
    log "Using repository ref: $YUYINODS_REF"
fi
...
mkdir -p "$YUYINODS_TEMP_DIR"
TEMP_DIR=$(mktemp -d "$YUYINODS_TEMP_DIR/yuyinods-bootstrap.XXXXXX")
trap 'rm -rf "$TEMP_DIR"' EXIT

clone_args=(--depth 1 --filter=blob:none --sparse)
if [[ -n "$YUYINODS_REF" ]]; then
    clone_args+=(--branch "$YUYINODS_REF")
fi
...
git sparse-checkout set ods 2>/dev/null || {
    ...
    cd "$YUYINODS_BOOTSTRAP_ROOT"
    rm -rf "$TEMP_DIR/repo"
    fallback_clone_args=(--depth 1)
    if [[ -n "$YUYINODS_REF" ]]; then
        fallback_clone_args+=(--branch "$YUYINODS_REF")
    fi
...
if [[ -d "$TEMP_DIR/repo/yuyinods" ]]; then
    ...
    if command -v rsync >/dev/null 2>&1; then
        rsync -a \
            ...
            --include='LICENSE' \
            "$TEMP_DIR/repo/yuyinods/" "$INSTALL_DIR/"
    else
        # Fallback to cp if rsync not available
        cp -r "$TEMP_DIR/repo/yuyinods" "$INSTALL_DIR"
        ...
        # Keep LICENSE file
        [[ -f "$TEMP_DIR/repo/yuyinods/LICENSE" ]] && cp "$TEMP_DIR/repo/yuyinods/LICENSE" "$INSTALL_DIR/" 2>/dev/null || true
    fi
else
...
# source library now ships inside yuyinods/extensions/library/, which the
...
if [[ -d "$TEMP_DIR/repo/yuyinods/extensions/library" ]]; then
    if rm -rf "$INSTALL_DIR/extensions-library-bundle" \
        && mkdir -p "$INSTALL_DIR/extensions-library-bundle" \
        && cp -R "$TEMP_DIR/repo/yuyinods/extensions/library/." "$INSTALL_DIR/extensions-library-bundle/"; then
        success "Bundled extensions-library templates"
    else
        warn "Failed to bundle extensions-library — Extensions page may 503"
    fi
else
    warn "yuyinods/extensions/library not in clone — Extensions page will 503"
fi

# ── Make scripts executable ──────────────────────────
chmod +x "$INSTALL_DIR/install.sh" 2>/dev/null || true
chmod +x "$INSTALL_DIR/yuyinods-cli" 2>/dev/null || true
