...
sr_load() {
    ...
    # explicitly enabled YUYINODS_SR_AUTO_INSTALL_PYYAML.
    if ! "$PYTHON_CMD" -c "import yaml" 2>/dev/null; then
        if [[ "${YUYINODS_SR_AUTO_INSTALL_PYYAML:-}" == "1" ]] && declare -f pkg_install &>/dev/null && declare -f pkg_resolve &>/dev/null; then
    ...
    _SR_CACHE="$(mktemp "${TMPDIR:-/tmp}/yuyinods-service-registry.XXXXXX")" || _SR_CACHE=""
...
for service_dir in _all_service_dirs:
    ...
    try:
        ...
        container = s.get("container_name", f"yuyinods-{sid}")
...
#
# Users copy container names (e.g. `yuyinods-token-spy`) from `docker ps` and
# expect them to work as arguments to `ods restart|stop|start|update`. The
# registry loader names every container `yuyinods-<sid>` (or the manifest's
# explicit `container_name`, which by convention follows the same pattern),
# so stripping a leading `yuyinods-` recovers the alias key when the literal
# input doesn't match an alias.
sr_resolve() {
    sr_load
    local input="$1"
    if [[ -z "${SERVICE_ALIASES[$input]:-}" && "$input" == yuyinods-* ]]; then
        local _stripped="${input#yuyinods-}"
...
sr_container() {
    ...
    local sid
    sid=$(sr_resolve "$1")
    echo "${SERVICE_CONTAINERS[$sid]:-yuyinods-$sid}"
}
