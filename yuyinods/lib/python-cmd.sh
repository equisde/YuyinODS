...
ods_detect_python_cmd() {
    ...
    if [[ -n "${YUYINODS_PYTHON_CMD:-}" ]] && _ods_python_runnable "$YUYINODS_PYTHON_CMD"; then
        _ods_python_cmd_cached="$YUYINODS_PYTHON_CMD"
        printf '%s' "${_ods_python_cmd_cached}"
        return 0
    fi
    ...
    if [[ "${YUYINODS_PYTHON_PREFER_SYSTEM:-}" == "1" && -x /usr/bin/python3 ]]; then
...
ods_detect_python_cmd_with_module() {
    ...
    if [[ -n "${YUYINODS_PYTHON_CMD:-}" ]] && _ods_python_has_module "$YUYINODS_PYTHON_CMD" "$module"; then
        printf '%s' "$YUYINODS_PYTHON_CMD"
        return 0
    fi
