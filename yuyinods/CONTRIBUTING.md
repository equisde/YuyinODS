...
No CLA. No committee. No waiting for permission. If it makes YuyinODS better, send it.
...
A student with a $200 laptop and no GPU should be able to run YuyinODS. So should someone with a 96GB Strix Halo laptop. We don't care if you have a 4090 or a hand-me-down ThinkPad — YuyinODS runs on your machine or we haven't done our job.
...
An LLM in a terminal is a toy. YuyinODS becomes something people can't live without when it connects to everything they already use. This is how we build the ecosystem that makes sovereign AI actually useful.

Where to help:
- **New services** — wrap any Docker-based tool as a YuyinODS extension. Manifest, compose file, health check — that's it. Look at `extensions/services/` for examples.
- **API bridges** — connect YuyinODS to Slack, Discord, email, calendars, CRMs. n8n workflows are the fastest path.
...
- It makes YuyinODS run on hardware it didn't before
...
**Got a question?** Open an issue or start a [GitHub Discussion](https://github.com/equisde/YuyinODS/discussions). Seriously — ask before you build. It's faster for everyone.
