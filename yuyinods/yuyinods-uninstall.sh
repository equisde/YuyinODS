#!/bin/bash
# yuyinods-uninstall.sh - ODS Clean Uninstaller
# Removes all ODS components, data, and system modifications.
# Usage: ./yuyinods-uninstall.sh [--keep-models] [--keep-data] [--force]

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
INSTALL_DIR="${INSTALL_DIR:-$HOME/yuyinods}"
...
resolve_compose_flags() {
    ...
    if [[ -z "$flags" && -x "$INSTALL_DIR/scripts/resolve-compose-stack.sh" ]]; then
        flags="$("$INSTALL_DIR/scripts/resolve-compose-stack.sh" \
            ...
            --yuyinods-mode "${YUYINODS_MODE:-local}" 2>/dev/null || true)"
    fi
...
This will remove:
    ...
    - CLI symlinks (/usr/local/bin/yuyinods, ~/.local/bin/yuyinods, legacy /usr/local/bin/yuyinods-cli)
...
# Detect install dir
if [[ -d "$SCRIPT_DIR" && -f "$SCRIPT_DIR/yuyinods-cli" ]]; then
...
if command -v docker &>/dev/null; then
    ...
    # Remove any remaining yuyinods-* containers
    ods_containers=$(docker ps -a --filter "name=yuyinods-" --format "{{.Names}}" 2>/dev/null || true)
    if [[ -n "$ods_containers" ]]; then
        log_info "Removing ODS containers..."
        echo "$ods_containers" | xargs docker rm -f 2>/dev/null || true
    fi

    # Remove yuyinods-specific Docker volumes unless data preservation was requested.
    if [[ "$KEEP_DATA" == "true" ]]; then
...
for unit in opencode-web.service openclaw-session-cleanup.timer \
            ...
            yuyinods-host-agent.service; do
...
#     `yuyinods-uninstall` currently has no path that catches this.
#
...
# Remove system-mode yuyinods-host-agent unit (migrated from --user mode).
# Idempotent — no-op if the unit was never installed (e.g. older user-mode installs).
if systemctl is-enabled yuyinods-host-agent.service >/dev/null 2>&1; then
    if ! timeout 20s sudo systemctl disable --now yuyinods-host-agent.service 2>/dev/null; then
        log_warn "yuyinods-host-agent did not stop cleanly; forcing service shutdown"
        sudo systemctl kill -s SIGKILL yuyinods-host-agent.service 2>/dev/null || true
        timeout 10s sudo systemctl disable yuyinods-host-agent.service 2>/dev/null || true
    fi
fi
sudo rm -f /etc/systemd/system/yuyinods-host-agent.service 2>/dev/null || true
sudo systemctl daemon-reload 2>/dev/null || true
log_ok "Systemd services removed"

# 3. Remove CLI symlinks
_removed_cli_symlink=false
for _ods_cli_link in "/usr/local/bin/yuyinods" "$HOME/.local/bin/yuyinods" "/usr/local/bin/yuyinods-cli"; do
...
DESKTOP_FILE="$HOME/.local/share/applications/yuyinods.desktop"
...
if $KEEP_MODELS && [[ -d "$INSTALL_DIR/data/models" ]]; then
    MODELS_BACKUP="$HOME/.yuyinods-models-backup"
...
echo ""
if $KEEP_MODELS; then
    echo "Your models were saved to: $HOME/.yuyinods-models-backup"
    echo "To reuse them on reinstall, move them back to ~/yuyinods/data/models/"
fi
if $KEEP_DATA; then
