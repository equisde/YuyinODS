# YuyinODS Security Guide

Security best practices for running YuyinODS.
...
NEW_LITELLM="sk-yuyinods-$(openssl rand -hex 16)"
...
The host agent (`bin/yuyinods-host-agent.py`) has its own bind address, separate from the Docker services above. It is controlled by `YUYINODS_AGENT_BIND` in `.env`:

| Platform | Default | Behavior |
|----------|---------|----------|
| macOS / Windows | `127.0.0.1` | Docker Desktop routes container traffic via loopback — loopback is sufficient |
| Linux | auto-detected | Detects the `yuyinods-network` gateway IP (e.g. `172.18.0.1`) so containers can reach the agent; LAN devices cannot. Falls back to the default Docker bridge gateway (e.g. `172.17.0.1`) for partial/older installs, then `127.0.0.1` if detection fails. |

To override the default, set `YUYINODS_AGENT_BIND` in `.env`:

```bash
# Restrict to loopback only (e.g. no-Docker Linux or extra hardening)
YUYINODS_AGENT_BIND=127.0.0.1

# Bind to a Docker network gateway only (explicit Linux default)
YUYINODS_AGENT_BIND=172.17.0.1

# Bind to all interfaces — exposes the host agent API on LAN (not recommended)
YUYINODS_AGENT_BIND=0.0.0.0
```

> **Note:** If you bind to `0.0.0.0`, ensure `YUYINODS_AGENT_KEY` is set in `.env` — it protects the extension management endpoints with Bearer token authentication.
...
tar -cz data/ | gpg -c > yuyinods-backup-$(date +%Y%m%d).tar.gz.gpg

# Restore
gpg -d yuyinods-backup-YYYYMMDD.tar.gz.gpg | tar -xz
```
