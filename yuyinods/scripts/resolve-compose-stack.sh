...
"$PYTHON_CMD" - "$SCRIPT_DIR" "$TIER" "$GPU_BACKEND" "$PROFILE_OVERLAYS" "$ENV_MODE" "$SKIP_BROKEN" "$GPU_COUNT" "$YUYINODS_MODE" <<'PY'
import os
import pathlib
import platform
import sys
...
ods_mode = (sys.argv[8] or os.environ.get("YUYINODS_MODE", "local")).lower()
lemonade_external = (
            ...
            try:
                ...
                if ods_mode in ("local", "hybrid", "lemonade") and tier != "CLOUD" and gpu_backend != "apple" and not lemonade_external:
                    ...
                    if local_mode_overlay.exists():
                        ...
                        # and reach the host since YUYINODS_MODE defaults to "local".
