...
# Configuration
YUYINODS_DIR="${YUYINODS_DIR:-$HOME/yuyinods}"
MODELS_DIR="${MODELS_DIR:-$YUYINODS_DIR/data/models}"
STATE_FILE="$YUYINODS_DIR/model-state.json"
BACKUP_FILE="$YUYINODS_DIR/model-state.backup.json"
LOG_FILE="$YUYINODS_DIR/upgrade-model.log"

OLLAMA_PORT="${OLLAMA_PORT:-${LLAMA_SERVER_PORT:-11434}}"
LLAMA_SERVER_CONTAINER="${LLAMA_SERVER_CONTAINER:-yuyinods-llama-server}"
...
detect_compose_file() {
    COMPOSE_FILE_ARGS=()
    if [[ -f "$YUYINODS_DIR/docker-compose.base.yml" && -f "$YUYINODS_DIR/docker-compose.amd.yml" ]]; then
        COMPOSE_FILE_ARGS=(-f "$YUYINODS_DIR/docker-compose.base.yml" -f "$YUYINODS_DIR/docker-compose.amd.yml")
    elif [[ -f "$YUYINODS_DIR/docker-compose.base.yml" && -f "$YUYINODS_DIR/docker-compose.nvidia.yml" ]]; then
        COMPOSE_FILE_ARGS=(-f "$YUYINODS_DIR/docker-compose.base.yml" -f "$YUYINODS_DIR/docker-compose.nvidia.yml")
    elif [[ -f "$YUYINODS_DIR/docker-compose.yml" ]]; then
        COMPOSE_FILE_ARGS=(-f "$YUYINODS_DIR/docker-compose.yml")
    fi
}
...
ensure_dirs() {
    mkdir -p "$YUYINODS_DIR" "$MODELS_DIR"
    touch "$LOG_FILE"
}
...
start_llm() {
    ...
    local env_file="$YUYINODS_DIR/.env"
    if [[ -f "$env_file" ]]; then
...
Environment Variables:
  ...
  LLAMA_SERVER_CONTAINER Docker container name (default: yuyinods-llama-server)
