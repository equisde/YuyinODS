...
# Test hooks: YUYINODS_NV_TEGRA_RELEASE, YUYINODS_DEVICE_TREE_COMPATIBLE,
#             YUYINODS_DEVICE_TREE_MODEL, YUYINODS_GPU0_SYSFS, YUYINODS_UNAME_M
detect_jetson() {
    [[ "${YUYINODS_ENABLE_EXPERIMENTAL_JETSON:-0}" == "1" ]] || return 1

    local tegra_release="${YUYINODS_NV_TEGRA_RELEASE:-/etc/nv_tegra_release}"
    local dt_compat="${YUYINODS_DEVICE_TREE_COMPATIBLE:-/proc/device-tree/compatible}"
    local dt_model="${YUYINODS_DEVICE_TREE_MODEL:-/proc/device-tree/model}"
    local gpu0_sysfs="${YUYINODS_GPU0_SYSFS:-/sys/devices/gpu.0}"
    local uname_m="${YUYINODS_UNAME_M:-$(uname -m)}"
