#!/bin/bash
#=============================================================================
# yuyinods-test.sh - ODS Validation Suite (M8)
#
...
#
# Usage:
#   ./yuyinods-test.sh                  # Run all tests
#   ./yuyinods-test.sh --quick          # Fast mode (~30s, no inference)
#   ./yuyinods-test.sh --json           # JSON output for automation
#   ./yuyinods-test.sh --service llm     # Test specific service
#
# Exit codes:
...
YUYINODS_DIR="${YUYINODS_DIR:-$HOME/.ods}"
ENV_FILE="${ENV_FILE:-$YUYINODS_DIR/.env}"
TIMEOUT=15
QUICK_TIMEOUT=5
...
_print_text_summary() {
    ...
    if [[ $FAILED_TESTS -eq 0 ]]; then
        ...
        if [[ "${RESULTS_STATUS[0]:-}" == "fail" ]] && [[ "${RESULTS_NAMES[0]:-}" == *"LLM"* ]]; then
            echo "  - LLM not responding - check: docker logs yuyinods-llama-server"
        fi
...
EXAMPLES:
    yuyinods-test.sh                    # Run all tests
    yuyinods-test.sh --quick            # Fast health check
    yuyinods-test.sh --json > results.json
    yuyinods-test.sh --service llm      # Test LLM only
