...
# Use before or during install when services are not yet up (unlike ./yuyinods-preflight.sh).
#
# Usage:
...
#
# Also reachable as: ./yuyinods-preflight.sh --install-env [same args]
...
JSON_FILE=""
STRICT=false
YUYINODS_ROOT="${YUYINODS_ROOT:-$ROOT_DIR}"
...
if [[ -d "$YUYINODS_ROOT" ]]; then
    # POSIX-friendly: df -P, parse available KB
    if DFOUT="$(df -Pk "$YUYINODS_ROOT" 2>/dev/null | tail -1)"; then
        AVAIL_KB="$(echo "$DFOUT" | awk '{print $4}')"
        if [[ "$AVAIL_KB" =~ ^[0-9]+$ ]]; then
            AVAIL_GB=$((AVAIL_KB / 1048576))
            if [[ "$AVAIL_GB" -ge "$MIN_DISK_GB_FREE" ]]; then
                append_check "DISK_SPACE" "pass" \
                    "Free space on $YUYINODS_ROOT: ~${AVAIL_GB}GB (min ${MIN_DISK_GB_FREE}GB)" ""
            else
                append_check "DISK_SPACE" "warn" \
                    "Low free space on $YUYINODS_ROOT: ~${AVAIL_GB}GB (recommended ≥${MIN_DISK_GB_FREE}GB free)" \
                    "Free disk space or set YUYINODS_ROOT to a volume with more room; see LINUX-TROUBLESHOOTING-GUIDE.md#disk_space."
            fi
        else
            append_check "DISK_SPACE" "warn" "Could not parse free space for $YUYINODS_ROOT" ""
        fi
    else
        append_check "DISK_SPACE" "warn" "df failed for $YUYINODS_ROOT" ""
    fi
else
    append_check "DISK_SPACE" "warn" "YUYINODS_ROOT does not exist yet: $YUYINODS_ROOT" \
...
if command -v ufw >/dev/null 2>&1 && systemctl is-active --quiet ufw 2>/dev/null; then
    append_check "FIREWALL_CHECK" "warn" \
        ...
        "Installer will auto-add a scoped rule for the actual yuyinods-network subnet after compose starts."
...
if [[ -f "$ROOT_DIR/docker-compose.base.yml" ]] || [[ -f "$ROOT_DIR/docker-compose.yml" ]]; then
    ...
    append_check "COMPOSE_FILES" "warn" \
        ...
        "Run this script from the extracted ODS source tree (yuyinods/)."
fi

emit_report() {
    python3 - "$CHECKS_JSONL" "$ROOT_DIR" "$KERNEL" "$DISTRO_ID" "$DISTRO_VERSION_ID" "$DISTRO_PRETTY" "$DISTRO_LIKE" "$YUYINODS_ROOT" "$MIN_DISK_GB_FREE" <<'PY'
import json
import sys
