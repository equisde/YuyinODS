...
def main() -> int:
    ...
    add_regex_check(
        checks,
        errors,
        "installers/macos/lib/constants.sh YUYINODS_VERSION",
    ...
    add_regex_check(
        checks,
        errors,
        "installers/windows/lib/constants.ps1 YUYINODS_VERSION",
        ROOT / "installers/windows/lib/constants.ps1",
        r'^\$script:YUYINODS_VERSION\s*=\s*"([^"]+)"',
    )
    add_regex_check(
        checks,
        errors,
        "installers/phases/06-directories.sh YUYINODS_VERSION fallback",
        ROOT / "installers/phases/06-directories.sh",
        r"^YUYINODS_VERSION=\$\{VERSION:-([^}]+)\}",
    )
    add_regex_check(
        checks,
        errors,
