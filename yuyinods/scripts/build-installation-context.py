...
def _container_to_service_map(repo_root: Path) -> dict[str, str]:
    ...
    (often ``yuyinods-<id>`` but sometimes different, e.g. open-webui →
    yuyinods-webui). We need both directions: incoming docker ps names are
    ...
    for manifest in sorted(services_dir.glob("*/manifest.yaml")):
        ...
        if sid:
            # Always include the default `yuyinods-<id>` convention too —
            # some manifests omit container_name when it matches.
            cnames.append(f"yuyinods-{sid}")
            for cname in cnames:
                index[cname] = sid
    return index
...
def _running_services(repo_root: Path) -> set[str]:
    ...
    map so e.g. ``yuyinods-webui`` resolves to service id ``open-webui``.
    Children of compound services (``yuyinods-langfuse-clickhouse`` etc.) get
    ...
    try:
        result = subprocess.run(
            ["docker", "ps", "--format", "{{.Names}}", "--filter", "name=yuyinods-"],
    ...
    for line in result.stdout.splitlines():
        ...
        if cname in cmap:
            ...
            continue
        # Compound-service collapse: yuyinods-langfuse-clickhouse → "langfuse"
        # if the latter exists as a known service id.
        if cname.startswith("yuyinods-"):
            stem = cname[len("yuyinods-"):]
            parts = stem.split("-")
...
def _humanize_gpu(env: dict[str, str]) -> str:
    ...
    mode = env.get("YUYINODS_MODE", "").lower()
    if backend == "amd":
        if mode == "lemonade":
...
def _capabilities_block(running: set[str], device: str) -> list[str]:
    ...
    for sid in sorted(running - seen):
        if sid in {"llama-server", "hermes", "hermes-proxy", "dashboard",
                   "dashboard-api", "yuyinods-proxy", "litellm"}:
...
def build_context_block(env_path: Path) -> str:
    ...
    device = env.get("YUYINODS_DEVICE_NAME") or socket.gethostname() or "this machine"
...
def build_soul(
    ...
    # /yuyinods-fleet-test on mac-mini after a prior failed install.
