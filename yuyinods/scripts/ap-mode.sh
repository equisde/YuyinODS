...
# when YUYINODS_AP_MODE=true is set in .env AND the device isn't yet
...
#
# Config: read from /etc/yuyinods/ap-mode.conf if it exists, otherwise
# falls back to sane defaults documented in docs/AP-MODE.md.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONF_DIR="${YUYINODS_AP_CONF_DIR:-/etc/yuyinods}"
RUN_DIR="${YUYINODS_AP_RUN_DIR:-/run/yuyinods-ap-mode}"
...
# Defaults — override in /etc/yuyinods/ap-mode.conf
YUYINODS_AP_SSID="${YUYINODS_AP_SSID:-YuyinODS-Setup}"
YUYINODS_AP_PASSWORD="${YUYINODS_AP_PASSWORD:-}"
YUYINODS_AP_INTERFACE="${YUYINODS_AP_INTERFACE:-wlan0}"
YUYINODS_AP_GATEWAY_IP="${YUYINODS_AP_GATEWAY_IP:-192.168.7.1}"
# YUYINODS_AP_PREFIX is CIDR prefix length used by `ip addr add`.
# YUYINODS_AP_NETMASK stays accepted (as dotted-decimal) for back-compat;
# bring_up_interface converts it to a prefix when YUYINODS_AP_PREFIX is
# unset. PREFIX is left empty here on purpose so an operator who only
# sets YUYINODS_AP_NETMASK in /etc/yuyinods/ap-mode.conf gets the prefix
# derived from THEIR netmask rather than the script default short-
# circuiting them to /24.
YUYINODS_AP_PREFIX="${YUYINODS_AP_PREFIX:-}"
YUYINODS_AP_NETMASK="${YUYINODS_AP_NETMASK:-255.255.255.0}"
YUYINODS_AP_DHCP_RANGE="${YUYINODS_AP_DHCP_RANGE:-192.168.7.10,192.168.7.50,1h}"
YUYINODS_AP_CHANNEL="${YUYINODS_AP_CHANNEL:-6}"
...
require_password() {
  ...
  if [[ -z "${YUYINODS_AP_PASSWORD}" ]]; then
    log "WARNING: YUYINODS_AP_PASSWORD unset — bringing up an open AP."
    log "  Set YUYINODS_AP_PASSWORD in ${CONF_DIR}/ap-mode.conf to require auth."
  elif [[ "${YUYINODS_AP_PASSWORD,,}" == "changeme-set-per-device" ]]; then
    err "YUYINODS_AP_PASSWORD still has the example placeholder value"
    err "set a unique per-device AP password in ${CONF_DIR}/ap-mode.conf"
    return 1
  elif [[ ${#YUYINODS_AP_PASSWORD} -lt 8 ]]; then
    err "YUYINODS_AP_PASSWORD must be at least 8 characters (WPA2 minimum)"
    return 1
  fi
}
...
interface=${YUYINODS_AP_INTERFACE}
driver=nl80211
ssid=${YUYINODS_AP_SSID}
hw_mode=g
channel=${YUYINODS_AP_CHANNEL}
auth_algs=1
wmm_enabled=1
HEREDOC

  if [[ -n "${YUYINODS_AP_PASSWORD}" ]]; then
    cat >> "${HOSTAPD_CONF}" <<HEREDOC
wpa=2
wpa_key_mgmt=WPA-PSK
wpa_pairwise=CCMP
rsn_pairwise=CCMP
wpa_passphrase=${YUYINODS_AP_PASSWORD}
HEREDOC
  fi
...
listen-address=${YUYINODS_AP_GATEWAY_IP}
dhcp-range=${YUYINODS_AP_DHCP_RANGE}
dhcp-option=3,${YUYINODS_AP_GATEWAY_IP}
dhcp-option=6,${YUYINODS_AP_GATEWAY_IP}
# Captive-portal DNS: every name resolves to the device's setup IP.
address=/#/${YUYINODS_AP_GATEWAY_IP}
...
install_iptables_rules() {
  ...
  if ! iptables -t nat -C PREROUTING -i "${YUYINODS_AP_INTERFACE}" -p tcp --dport 80 \
      -j DNAT --to-destination "${YUYINODS_AP_GATEWAY_IP}:80" -m comment --comment "yuyinods-ap-mode" 2>/dev/null; then
    iptables -t nat -A PREROUTING -i "${YUYINODS_AP_INTERFACE}" -p tcp --dport 80 \
      -j DNAT --to-destination "${YUYINODS_AP_GATEWAY_IP}:80" -m comment --comment "yuyinods-ap-mode"
  fi
  if ! iptables -t nat -C PREROUTING -i "${YUYINODS_AP_INTERFACE}" -p tcp --dport 443 \
      -j DNAT --to-destination "${YUYINODS_AP_GATEWAY_IP}:443" -m comment --comment "yuyinods-ap-mode" 2>/dev/null; then
    iptables -t nat -A PREROUTING -i "${YUYINODS_AP_INTERFACE}" -p tcp --dport 443 \
      -j DNAT --to-destination "${YUYINODS_AP_GATEWAY_IP}:443" -m comment --comment "yuyinods-ap-mode"
  fi
}
...
remove_iptables_rules() {
  ...
  while iptables -t nat -C PREROUTING -i "${YUYINODS_AP_INTERFACE}" -p tcp --dport 80 \
      -j DNAT --to-destination "${YUYINODS_AP_GATEWAY_IP}:80" -m comment --comment "yuyinods-ap-mode" 2>/dev/null; do
    iptables -t nat -D PREROUTING -i "${YUYINODS_AP_INTERFACE}" -p tcp --dport 80 \
      -j DNAT --to-destination "${YUYINODS_AP_GATEWAY_IP}:80" -m comment --comment "yuyinods-ap-mode" || true
  done
  while iptables -t nat -C PREROUTING -i "${YUYINODS_AP_INTERFACE}" -p tcp --dport 443 \
      -j DNAT --to-destination "${YUYINODS_AP_GATEWAY_IP}:443" -m comment --comment "yuyinods-ap-mode" 2>/dev/null; do
    iptables -t nat -D PREROUTING -i "${YUYINODS_AP_INTERFACE}" -p tcp --dport 443 \
      -j DNAT --to-destination "${YUYINODS_AP_GATEWAY_IP}:443" -m comment --comment "yuyinods-ap-mode" || true
  done
}
...
release_interface_from_nm() {
  ...
  nmcli device set "${YUYINODS_AP_INTERFACE}" managed no 2>/dev/null || true
}

reclaim_interface_for_nm() {
  nmcli device set "${YUYINODS_AP_INTERFACE}" managed yes 2>/dev/null || true
}
...
bring_up_interface() {
  ...
  # silently fails on most modern iproute2 builds. We use YUYINODS_AP_PREFIX
  # when set; otherwise we convert YUYINODS_AP_NETMASK to a prefix at
  # bring-up time so back-compat with operator configs that set NETMASK
  # is preserved.
  local prefix="${YUYINODS_AP_PREFIX}"
  if [[ -z "${prefix}" ]]; then
    prefix="$(_netmask_to_prefix "${YUYINODS_AP_NETMASK}")" || return 1
  fi
  ip link set dev "${YUYINODS_AP_INTERFACE}" up
  ip addr flush dev "${YUYINODS_AP_INTERFACE}"
  ip addr add "${YUYINODS_AP_GATEWAY_IP}/${prefix}" dev "${YUYINODS_AP_INTERFACE}"
}
  ...
  cat > "${STATE_FILE}" <<HEREDOC
{
  ...
  "ssid": "${YUYINODS_AP_SSID}",
  "interface": "${YUYINODS_AP_INTERFACE}",
  "gateway_ip": "${YUYINODS_AP_GATEWAY_IP}",
  "since": "$(date -Iseconds)"
}
HEREDOC
...
cmd_up() {
  ...
  log "bringing up AP on ${YUYINODS_AP_INTERFACE} as '${YUYINODS_AP_SSID}'"
  release_interface_from_nm
  bring_up_interface
  ...
  write_state "active"
  log "AP up: SSID=${YUYINODS_AP_SSID} gateway=${YUYINODS_AP_GATEWAY_IP}"
  log "  any hostname resolves to ${YUYINODS_AP_GATEWAY_IP} (captive portal)"
  log "  HTTP/HTTPS on ${YUYINODS_AP_INTERFACE} redirected to the gateway"
}
...
cmd_down() {
  ...
  log "tearing down AP on ${YUYINODS_AP_INTERFACE}"
  ...
  remove_iptables_rules
  ip addr flush dev "${YUYINODS_AP_INTERFACE}" 2>/dev/null || true
  reclaim_interface_for_nm

  rm -f "${STATE_FILE}" "${HOSTAPD_CONF}" "${DNSMASQ_CONF}"
  log "AP down; ${YUYINODS_AP_INTERFACE} returned to NetworkManager"
}
...
Usage: ap-mode.sh {up|down|status}
  ...
  up      Bring up the AP. DESTRUCTIVE — takes ${YUYINODS_AP_INTERFACE}
