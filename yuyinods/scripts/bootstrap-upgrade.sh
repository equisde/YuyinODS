...
compose_recreate_llama_server_with_retry() {
    ...
    local max_attempts="${YUYINODS_BOOTSTRAP_COMPOSE_RETRY_ATTEMPTS:-3}"
    ...
    while true; do
        ...
        sleep "${YUYINODS_BOOTSTRAP_COMPOSE_RETRY_DELAY:-15}"
        attempt=$(( attempt + 1 ))
    done
}
...
acquire_upgrade_lock() {
    ...
    local lock_dir="$tmp_root/yuyinods-bootstrap-upgrade-${lock_key}.lock"
...
restore_docker_llama_server_after_swap_failure() {
    ...
    if ! compose_recreate_llama_server_with_retry "${COMPOSE_ARGS[@]}"; then
        log "WARNING: rollback recreate failed; inspect docker logs yuyinods-llama-server"
        return 1
    fi
...
docker_llama_server_container_failed_after_swap() {
    ...
    local state
    state=$($DOCKER_CMD inspect yuyinods-llama-server \
...
move_bootstrap_model_aside_for_windows_swap() {
    ...
    BOOTSTRAP_SWAP_BACKUP_PATH="${BOOTSTRAP_PATH}.yuyinods-swap-backup"
...
restart_windows_lemonade_with_full_model() {
    ...
    YUYINODS_WIN_PID_FILE="$(windows_path "$pid_file")" \
    YUYINODS_WIN_MODELS_DIR="$(windows_path "$MODELS_DIR")" \
    YUYINODS_WIN_BIND_ADDR="$bind_addr" \
    YUYINODS_WIN_LEMONADE_PORT="$lemonade_port" \
    "$ps_cmd" -NoProfile -ExecutionPolicy Bypass -Command '
        ...
        $pidPath = $env:YUYINODS_WIN_PID_FILE
        ...
        # full-model instance.
        $port = [int]$env:YUYINODS_WIN_LEMONADE_PORT
        ...
        $odsModelsDir = $env:YUYINODS_WIN_MODELS_DIR
        ...
        $args = @(
            "serve",
            "--port", $env:YUYINODS_WIN_LEMONADE_PORT,
            "--host", $env:YUYINODS_WIN_BIND_ADDR,
            "--no-tray",
            "--llamacpp", "vulkan",
            "--extra-models-dir", $env:YUYINODS_WIN_MODELS_DIR
        )
    ...
    # budget as the llama.cpp warm-up path; override with YUYINODS_LEMONADE_SWAP_ATTEMPTS.
    _swap_attempts="${YUYINODS_LEMONADE_SWAP_ATTEMPTS:-60}"
...
restart_windows_native_llama_server_with_full_model() {
    ...
    YUYINODS_WIN_PID_FILE="$(windows_path "$pid_file")" \
    YUYINODS_WIN_LLAMA_EXE="$(windows_path "$llama_exe")" \
    YUYINODS_WIN_MODEL_PATH="$(windows_path "$model_path")" \
    YUYINODS_WIN_ROLLBACK_MODEL_PATH="$(windows_path "$rollback_model_path")" \
    YUYINODS_WIN_LOG_PATH="$(windows_path "$log_path")" \
    YUYINODS_WIN_BIND_ADDR="$bind_addr" \
    YUYINODS_WIN_LLAMA_PORT="$llama_port" \
    YUYINODS_WIN_CTX_SIZE="$ctx_size" \
    YUYINODS_WIN_REASONING_FORMAT="$reasoning_fmt" \
    YUYINODS_WIN_FLASH_ATTN="$(read_env_value LLAMA_ARG_FLASH_ATTN)" \
    YUYINODS_WIN_CACHE_TYPE_K="$(read_env_value LLAMA_ARG_CACHE_TYPE_K)" \
    YUYINODS_WIN_CACHE_TYPE_V="$(read_env_value LLAMA_ARG_CACHE_TYPE_V)" \
    YUYINODS_WIN_N_CPU_MOE="$(read_env_value LLAMA_ARG_N_CPU_MOE)" \
    YUYINODS_WIN_PARALLEL="$(read_env_value LLAMA_PARALLEL)" \
    YUYINODS_WIN_CHECKPOINT_EVERY_N_TOKENS="$(read_env_value LLAMA_ARG_CHECKPOINT_EVERY_N_TOKENS)" \
    YUYINODS_WIN_NO_CACHE_PROMPT="$(read_env_value LLAMA_ARG_NO_CACHE_PROMPT)" \
    YUYINODS_WIN_SPEC_TYPE="$(read_env_value LLAMA_ARG_SPEC_TYPE)" \
    YUYINODS_WIN_SPEC_DRAFT_N_MAX="$(read_env_value LLAMA_ARG_SPEC_DRAFT_N_MAX)" \
    "$ps_cmd" -NoProfile -ExecutionPolicy Bypass -Command '
        $ErrorActionPreference = "Stop"
                ...
                foreach ($listener in $listeners) {
                    if ($listener.OwningProcess -gt 0) {
                        ...
                        if ($proc -and (
                            ...
                            ($proc.ExecutablePath -and $proc.ExecutablePath.Equals($env:YUYINODS_WIN_LLAMA_EXE, [StringComparison]::OrdinalIgnoreCase)) -or
        ...
        function Start-ODSLlama {
            ...
            $args = @(
                "--model", $ModelPath,
                "--host", $env:YUYINODS_WIN_BIND_ADDR,
                "--port", $env:YUYINODS_WIN_LLAMA_PORT,
                "--n-gpu-layers", "999",
                "--ctx-size", $env:YUYINODS_WIN_CTX_SIZE,
                "--reasoning-format", $env:YUYINODS_WIN_REASONING_FORMAT,
                "--metrics"
            )
            if ($env:YUYINODS_WIN_FLASH_ATTN) { $args += @("--flash-attn", $env:YUYINODS_WIN_FLASH_ATTN) }
            if ($env:YUYINODS_WIN_CACHE_TYPE_K) { $args += @("--cache-type-k", $env:YUYINODS_WIN_CACHE_TYPE_K) }
            if ($env:YUYINODS_WIN_CACHE_TYPE_V) { $args += @("--cache-type-v", $env:YUYINODS_WIN_CACHE_TYPE_V) }
            if ($env:YUYINODS_WIN_N_CPU_MOE) { $args += @("--n-cpu-moe", $env:YUYINODS_WIN_N_CPU_MOE) }
            if ($env:YUYINODS_WIN_PARALLEL) { $args += @("--parallel", $env:YUYINODS_WIN_PARALLEL) }
            if ($env:YUYINODS_WIN_CHECKPOINT_EVERY_N_TOKENS) { $args += @("--checkpoint-every-n-tokens", $env:YUYINODS_WIN_CHECKPOINT_EVERY_N_TOKENS) }
            if ($env:YUYINODS_WIN_NO_CACHE_PROMPT -and $env:YUYINODS_WIN_NO_CACHE_PROMPT -notin @("0", "false", "off", "no")) { $args += @("--no-cache-prompt") }
            if ($env:YUYINODS_WIN_SPEC_TYPE) { $args += @("--spec-type", $env:YUYINODS_WIN_SPEC_TYPE) }
            if ($env:YUYINODS_WIN_SPEC_DRAFT_N_MAX) { $args += @("--spec-draft-n-max", $env:YUYINODS_WIN_SPEC_DRAFT_N_MAX) }

            New-Item -ItemType Directory -Path (Split-Path -Parent $env:YUYINODS_WIN_PID_FILE) -Force | Out-Null
            New-Item -ItemType Directory -Path (Split-Path -Parent $env:YUYINODS_WIN_LOG_PATH) -Force | Out-Null
            $proc = Start-Process -FilePath $env:YUYINODS_WIN_LLAMA_EXE `
                -ArgumentList $args -WindowStyle Hidden -RedirectStandardOutput $env:YUYINODS_WIN_LOG_PATH -RedirectStandardError ($env:YUYINODS_WIN_LOG_PATH + ".err") -PassThru
            Set-Content -LiteralPath $env:YUYINODS_WIN_PID_FILE -Value $proc.Id
            return $proc
        }
        ...
        $port = [int]$env:YUYINODS_WIN_LLAMA_PORT
        Stop-ODSLlamaListeners -Port $port

        $fullProc = Start-ODSLlama -ModelPath $env:YUYINODS_WIN_MODEL_PATH
        if (Wait-ODSLlamaHealth -ProcessId ([int]$fullProc.Id) -Port $port) { exit 0 }

        Stop-ODSProcessId -ProcessId ([int]$fullProc.Id)
        if ($env:YUYINODS_WIN_ROLLBACK_MODEL_PATH -and (Test-Path $env:YUYINODS_WIN_ROLLBACK_MODEL_PATH)) {
            $rollbackProc = Start-ODSLlama -ModelPath $env:YUYINODS_WIN_ROLLBACK_MODEL_PATH
...
patch_hermes_model_after_swap() {
    ...
    if [[ -n "$DOCKER_CMD" ]] && $DOCKER_CMD ps --filter name=yuyinods-hermes --format '{{.Names}}' 2>/dev/null | grep -q yuyinods-hermes; then
        ...
        $DOCKER_CMD exec yuyinods-hermes sh -c \
            "$live_patch" 2>&1 || {
                log "ERROR: Could not patch Hermes live config after full-model swap."
                return 1
            }
        $DOCKER_CMD restart yuyinods-hermes 2>&1 || {
...
LITELLM_NATIVE_LOCAL_EOF
    ...
    if [[ -n "$DOCKER_CMD" ]] && $DOCKER_CMD ps --filter name=yuyinods-litellm --format '{{.Names}}' 2>/dev/null | grep -q yuyinods-litellm; then
        $DOCKER_CMD restart yuyinods-litellm 2>&1 || log "WARNING: LiteLLM restart failed after native Windows config refresh (non-fatal)"
    fi
}
...
_dl_success=false
_download_attempts="${YUYINODS_BOOTSTRAP_DOWNLOAD_ATTEMPTS:-6}"
...
if [[ "$_windows_lemonade_swap_applies" == "true" ]]; then
...
elif [[ -n "$DOCKER_CMD" ]] && $DOCKER_CMD ps --filter name=yuyinods-llama-server --format '{{.Names}}' 2>/dev/null | grep -q yuyinods-llama-server; then
    ...
    if [[ "$_gpu_backend" != "amd" ]] && [[ -n "$DOCKER_CMD" ]]; then
        _running_cmd=$($DOCKER_CMD inspect yuyinods-llama-server --format '{{join .Config.Cmd " "}}' 2>/dev/null || echo "")
    ...
    if $_healthy; then
        ...
        if $DOCKER_CMD ps --filter name=yuyinods-litellm --format '{{.Names}}' 2>/dev/null | grep -q yuyinods-litellm; then
            ...
            _renderer_py="${YUYINODS_PYTHON_CMD:-}"
...
LITELLM_UPGRADE_EOF
            ...
            $DOCKER_CMD restart yuyinods-litellm 2>&1 || log "WARNING: LiteLLM restart failed (non-fatal)"
        fi
        ...
        if $DOCKER_CMD ps --filter name=yuyinods-openclaw --format '{{.Names}}' 2>/dev/null | grep -q yuyinods-openclaw; then
        ...
        if $DOCKER_CMD ps --filter name=yuyinods-hermes --format '{{.Names}}' 2>/dev/null | grep -q yuyinods-hermes; then
            ...
            $DOCKER_CMD exec yuyinods-hermes sh -c \
                ...
                log "WARNING: Could not patch Hermes /opt/data/config.yaml (non-fatal — operator can hand-edit and 'docker restart yuyinods-hermes')"
            log "Restarting Hermes to pick up model change..."
            $DOCKER_CMD restart yuyinods-hermes 2>&1 || log "WARNING: Hermes restart failed (non-fatal — hand-restart with 'docker restart yuyinods-hermes')"
            ...
            if $DOCKER_CMD exec yuyinods-hermes curl -sf --max-time 120 -X POST \
            ...
            for _i in $(seq 1 30); do
                if $DOCKER_CMD exec yuyinods-hermes curl -sf --max-time 3 http://127.0.0.1:9119/api/status >/dev/null 2>&1; then
                    _hermes_ready=true
                    break
                fi
                sleep 2
            ...
            if $_hermes_ready; then
                if $DOCKER_CMD exec yuyinods-hermes timeout 90 \
        ...
        log "Check: docker logs yuyinods-llama-server"
        _rollback_status="Previous active model config restore was attempted; inspect docker logs yuyinods-llama-server and re-run to retry."
    ...
    if [[ ! -x "$LLAMA_SERVER_BIN" ]]; then
        ...
        log "Run './yuyinods-macos.sh restart' to load the new model manually."
    else
        ...
        if [[ ! -f "$_model_path" ]]; then
            ...
            if $_healthy; then
                ...
                if [[ -n "${_old_model_path:-}" && -f "$_old_model_path" ]]; then
                    ...
                    log "Run './yuyinods-macos.sh restart' to manually recover."
                fi
                write_status "failed" 100 "$TOTAL_BYTES" "$TOTAL_BYTES" 0 \
                    "Full model downloaded and verified, but native macOS llama-server did not load it after swap. Bootstrap model kept; run './yuyinods-macos.sh restart' or re-run to retry."
                exit 1
            fi
        fi
    fi
...
if command -v systemctl &>/dev/null && systemctl --user is-active yuyinods-host-agent.service &>/dev/null; then
    log "Restarting yuyinods-host-agent (systemd)..."
    systemctl --user restart yuyinods-host-agent.service 2>&1 || \
        log "WARNING: Could not restart host agent (non-fatal)"
elif [[ -f "$HOME/Library/LaunchAgents/com.ods.host-agent.plist" ]]; then
    log "Restarting yuyinods-host-agent (launchctl)..."
