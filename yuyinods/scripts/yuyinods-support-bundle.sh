...
JSON_OUTPUT=false
INCLUDE_LOGS=true
DOCKER_BIN="${YUYINODS_SUPPORT_BUNDLE_DOCKER:-docker}"

usage() {
    cat <<'EOF'
Usage: scripts/yuyinods-support-bundle.sh [OPTIONS]
...
BUNDLE_NAME="yuyinods-support-${timestamp}-$$"
...
docker_cli_available() {
    [[ "${YUYINODS_SUPPORT_BUNDLE_DISABLE_DOCKER:-}" == "1" ]] && return 1
...
collect_diagnostics() {
    if [[ -f "$ROOT_DIR/scripts/yuyinods-doctor.sh" ]]; then
        collect_shell "diagnostics/yuyinods-doctor.log" "yuyinods-doctor" "$(shell_quote "$BASH_CMD") $(shell_quote "$ROOT_DIR/scripts/yuyinods-doctor.sh") $(shell_quote "$BUNDLE_DIR/diagnostics/yuyinods-doctor.json")"
        redact_file "$BUNDLE_DIR/diagnostics/yuyinods-doctor.json"
    else
        write_file "diagnostics/yuyinods-doctor.log" <<< "scripts/yuyinods-doctor.sh not found"
        record_command "yuyinods-doctor" "diagnostics/yuyinods-doctor.log" "127"
    fi
...
collect_compose_validation() {
    ...
    ods_mode="$(read_env_value YUYINODS_MODE local)"
    ...
    if [[ "${YUYINODS_SUPPORT_BUNDLE_DISABLE_DOCKER:-}" == "1" ]] || { ! docker_compose_available && ! command -v docker-compose >/dev/null 2>&1; }; then
...
collect_docker() {
    ...
    if [[ "$count" -eq 0 ]]; then
        write_file "logs/no-yuyinods-containers.txt" <<< "No running YuyinODS-like containers found"
    fi
}
...
doctor = load_json(bundle_dir / "diagnostics" / "yuyinods-doctor.json") or {}
...
evidence = {
    ...
    "backend": {
        "ods_mode": env.get("YUYINODS_MODE"),
