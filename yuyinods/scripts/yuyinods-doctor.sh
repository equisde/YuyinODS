...
usage() {
    ...
    echo ""
    echo "Arguments:"
    echo "  REPORT_PATH  Output JSON path (default: /tmp/yuyinods-doctor-report.json)"
    echo ""
    echo "Exit codes: 0 = report generated, 1 = error (e.g. missing dependency)"
    echo ""
    echo "See docs/YuyinODS-DOCTOR.md for details."
}
case "${1:-}" in
    -h|--help) usage; exit 0 ;;
esac

REPORT_FILE="${1:-/tmp/yuyinods-doctor-report.json}"

CAP_FILE="/tmp/yuyinods-doctor-capabilities.json"
PREFLIGHT_FILE="/tmp/yuyinods-doctor-preflight.json"
...
if [[ "$_doctor_gpu_backend" == "nvidia" ]] && command -v nvidia-smi >/dev/null 2>&1; then
    ...
    if [[ -n "$_dgx_gpu_raw" ]]; then
        ...
        if [[ "$DGX_SPARK_GPU_NAME" == *"GB10"* || "$DGX_SPARK_COMPUTE_CAP" == "12.1" ]]; then
            DGX_SPARK_GPU="true"
            if [[ "$DOCKER_DAEMON" == "true" ]] && docker ps -a --format '{{.Names}}' 2>/dev/null | grep -qx 'yuyinods-llama-server'; then
                _llama_arch_line="$(docker logs yuyinods-llama-server 2>&1 | grep 'CUDA : ARCHS =' | tail -1 || true)"
                ...
                DGX_SPARK_CUDA_ARCH_STATUS="unknown"
                DGX_SPARK_CUDA_ARCH_MESSAGE="DGX Spark detected, but yuyinods-llama-server is not available for CUDA arch inspection."
            fi
        fi
    fi
fi
...
if [[ "$DOCKER_DAEMON" == "true" ]] && docker ps --format '{{.Names}}' 2>/dev/null | grep -qx 'yuyinods-hermes'; then
    _hermes_worker_count="$(
        docker exec yuyinods-hermes sh -c \
...
YUYINODS_MANAGED_CONTAINER_COUNT="0"
YUYINODS_RUNNING_CONTAINER_COUNT="0"
if [[ "$DOCKER_DAEMON" == "true" ]]; then
    YUYINODS_MANAGED_CONTAINER_COUNT="$(
        docker ps -a --format '{{.Names}}' 2>/dev/null | grep -c '^yuyinods-' || true
    )"
    YUYINODS_RUNNING_CONTAINER_COUNT="$(
        docker ps --format '{{.Names}}' 2>/dev/null | grep -c '^yuyinods-' || true
    )"
fi
...
def _looks_like_local_llama_route(value):
    ...
    return (
        ...
        or "yuyinods-llama-server" in lowered
...
def _looks_like_installer_generated_lemonade_key(value):
    return str(value or "").strip().startswith("sk-yuyinods-lemonade-")
...
def _collect_inference_contract():
    ...
    ods_mode = (env_get("YUYINODS_MODE", "local") or "local").strip().lower()
    ...
    if ods_mode not in {"local", "cloud", "hybrid", "lemonade"}:
        issues.append(
            _inference_issue(
                "YuyinODS-RUNTIME-MODE-UNKNOWN",
                "blocker",
                ".env",
                f"YUYINODS_MODE={ods_mode!r} is not one of local/cloud/hybrid/lemonade.",
            )
        )
    ...
    if ods_mode == "cloud":
        if compose_flags_exists and not cloud_overlay:
            issues.append(
                _inference_issue(
                    "YuyinODS-RUNTIME-CLOUD-OVERLAY-MISSING",
                    "blocker",
                    ".compose-flags",
                    "YUYINODS_MODE=cloud but docker-compose.cloud.yml is not in the resolved compose stack.",
                )
            )
        if _looks_like_local_llama_route(llm_api_url):
            issues.append(
                _inference_issue(
                    "YuyinODS-RUNTIME-CLOUD-LLM-LOCAL-ROUTE",
                    "blocker",
                    ".env",
                    f"YUYINODS_MODE=cloud but LLM_API_URL points at local llama-server ({llm_api_url}).",
                )
            )
        if _looks_like_local_llama_route(hermes_base_url):
            issues.append(
                _inference_issue(
                    "YuyinODS-RUNTIME-CLOUD-HERMES-LOCAL-ROUTE",
                    "blocker",
                    ".env",
                    f"YUYINODS_MODE=cloud but HERMES_LLM_BASE_URL points at local llama-server ({hermes_base_url}).",
                )
            )
        if llm_api_url and not _looks_like_litellm_route(llm_api_url):
            issues.append(
                _inference_issue(
                    "YuyinODS-RUNTIME-CLOUD-GATEWAY-BYPASS",
                    "warn",
                    ".env",
                    f"YUYINODS_MODE=cloud normally routes ODS services through LiteLLM, but LLM_API_URL={llm_api_url}.",
                )
            )
    ...
    if lemonade_external:
        if compose_flags_exists and not cloud_overlay:
            issues.append(
                _inference_issue(
                    "YuyinODS-RUNTIME-EXTERNAL-LEMONADE-CLOUD-OVERLAY-MISSING",
                    "blocker",
                    ".compose-flags",
        ...
        if compose_flags_exists and not lemonade_external_overlay:
            issues.append(
                _inference_issue(
                    "YuyinODS-RUNTIME-EXTERNAL-LEMONADE-OVERLAY-MISSING",
                    "warn",
                    ".compose-flags",
        ...
        if _looks_like_local_llama_route(llm_api_url):
            issues.append(
                _inference_issue(
                    "YuyinODS-RUNTIME-EXTERNAL-LEMONADE-LOCAL-ROUTE",
                    "blocker",
                    ".env",
        ...
        if (host_routed_lemonade or network_lemonade) and not lemonade_auth_configured:
            ...
            issues.append(
                _inference_issue(
                    "YuyinODS-RUNTIME-EXTERNAL-LEMONADE-UNAUTHENTICATED-HOST-ROUTE",
                    "warn",
                    ".env",
                    detail,
                )
    ...
    if ods_mode == "local" and not lemonade_external:
        if compose_flags_exists and cloud_overlay:
            issues.append(
                _inference_issue(
                    "YuyinODS-RUNTIME-LOCAL-CLOUD-OVERLAY",
                    "warn",
                    ".compose-flags",
                    "YUYINODS_MODE=local but docker-compose.cloud.yml is still in the compose stack.",
                )
            )
        if _looks_like_litellm_route(llm_api_url) and gpu_backend != "amd":
            issues.append(
                _inference_issue(
                    "YuyinODS-RUNTIME-LOCAL-LITELLM-ROUTE",
                    "warn",
                    ".env",
                    f"YUYINODS_MODE=local on non-AMD usually routes directly to llama-server, but LLM_API_URL={llm_api_url}.",
                )
            )

    diagnoses = []
    issue_titles = {
        "YuyinODS-RUNTIME-MODE-UNKNOWN": "Runtime mode is not recognized",
        "YuyinODS-RUNTIME-CLOUD-OVERLAY-MISSING": "Cloud mode is missing the cloud compose overlay",
        "YuyinODS-RUNTIME-CLOUD-LLM-LOCAL-ROUTE": "Cloud mode still routes chat clients to local llama-server",
        "YuyinODS-RUNTIME-CLOUD-HERMES-LOCAL-ROUTE": "Cloud mode still routes Hermes to local llama-server",
        "YuyinODS-RUNTIME-CLOUD-GATEWAY-BYPASS": "Cloud mode bypasses the LiteLLM gateway",
        "YuyinODS-RUNTIME-EXTERNAL-LEMONADE-CLOUD-OVERLAY-MISSING": "External Lemonade is missing the cloud compose overlay",
        "YuyinODS-RUNTIME-EXTERNAL-LEMONADE-OVERLAY-MISSING": "External Lemonade is missing its compose overlay",
        "YuyinODS-RUNTIME-EXTERNAL-LEMONADE-LOCAL-ROUTE": "External Lemonade still routes clients to local llama-server",
        "YuyinODS-RUNTIME-EXTERNAL-LEMONADE-UNAUTHENTICATED-HOST-ROUTE": "External Lemonade host route has no user-provided API key",
        "YuyinODS-RUNTIME-LOCAL-CLOUD-OVERLAY": "Local mode still has the cloud compose overlay",
        "YuyinODS-RUNTIME-LOCAL-LITELLM-ROUTE": "Local mode routes through LiteLLM unexpectedly",
    }
    next_steps = {
        "YuyinODS-RUNTIME-CLOUD-OVERLAY-MISSING": [
            "Regenerate compose flags with YUYINODS_MODE=cloud and include docker-compose.cloud.yml.",
        ...
        "YuyinODS-RUNTIME-MODE-UNKNOWN": [
            "Set YUYINODS_MODE to local, cloud, hybrid, or lemonade.",
        ],
    }
    for issue in issues:
        diagnoses.append(
...
def _collect_install_diagnoses(artifacts):
    ...
    if combined_failure_text:
        ...
        if failed_image:
            ...
            diagnoses.append(
                _diagnosis(
                    "YuyinODS-DOCKER-IMAGE-UNRESOLVED",
                    "blocker",
                    "high",
        ...
        if _source_mentions_zero_containers(
            ...
            diagnoses.append(
                _diagnosis(
                    "YuyinODS-COMPOSE-ZERO-CONTAINERS",
                    "blocker",
                    "high",
        ...
        if "modulenotfounderror: no module named 'yaml'" in lowered or "pyyaml is required" in lowered:
            diagnoses.append(
                _diagnosis(
                    "YuyinODS-PYTHON-PYYAML-MISSING",
                    "blocker",
                    "high",
        ...
        if alpine_probe_source:
            diagnoses.append(
                _diagnosis(
                    "YuyinODS-WINDOWS-FILE-SHARING-PROBE-IMAGE",
                    "warn",
                    "medium",
...
for ext in ext_diagnostics:
    ...
    for issue in issues:
        if issue == "container_not_running":
            ...
            fix_hints.append(f"Extension {ext_id}: health check failed. Check logs with 'docker logs yuyinods-{ext_id}'.")
