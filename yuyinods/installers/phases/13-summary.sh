...
# Mode is now stored in .env as YUYINODS_MODE (set by phase 06)
if ! $DRY_RUN; then
    mkdir -p "$INSTALL_DIR"
else
...
if [[ -f "$SCRIPT_DIR/yuyinods-preflight.sh" ]]; then
    ...
    for _pf_attempt in 1 2 3; do
        if bash "$SCRIPT_DIR/yuyinods-preflight.sh" 2>>"$LOG_FILE"; then
...
#=============================================================================
if ! $DRY_RUN; then
    COMPLETION_FILE="$INSTALL_DIR/completions/yuyinods-cli.bash"
    if [[ -f "$COMPLETION_FILE" ]]; then
        # Add completion sourcing to .bashrc if not already present
        if ! grep -q "yuyinods-cli.bash" "$HOME/.bashrc" 2>/dev/null; then
            cat >> "$HOME/.bashrc" << 'BASHRC_EOF'

# ODS CLI bash completion
if [[ -f "$HOME/yuyinods/completions/yuyinods-cli.bash" ]]; then
    . "$HOME/yuyinods/completions/yuyinods-cli.bash"
fi
BASHRC_EOF
            ai_ok "Bash completion enabled for yuyinods-cli"
        fi
    fi
fi
...
#=============================================================================
if ! $DRY_RUN; then
    if [[ -x "$INSTALL_DIR/yuyinods-cli" ]]; then
        if ! command -v ods &>/dev/null; then
            if sudo -n ln -sf "$INSTALL_DIR/yuyinods-cli" /usr/local/bin/yuyinods 2>/dev/null; then
                ai_ok "ods command installed (try: ods status)"
            else
                # Fallback: user-local bin directory (no sudo needed)
                mkdir -p "$HOME/.local/bin"
                if ln -sf "$INSTALL_DIR/yuyinods-cli" "$HOME/.local/bin/yuyinods" 2>/dev/null; then
                    ai_ok "ods command installed to ~/.local/bin/yuyinods"
                    if [[ ":$PATH:" != *":$HOME/.local/bin:"* ]]; then
                        ai_warn "Add to your shell profile: export PATH=\"\$HOME/.local/bin:\$PATH\""
                    fi
                else
                    ai_warn "Could not create 'ods' command. Add manually:"
                    ai "  sudo ln -sf $INSTALL_DIR/yuyinods-cli /usr/local/bin/yuyinods"
                fi
            fi
        else
...
if ! $DRY_RUN; then
    ...
    if $DOCKER_CMD inspect yuyinods-perplexica &>/dev/null; then
