...
if $DRY_RUN; then
    ...
    if [[ -n "$OPENCODE_BIN" && -x "$OPENCODE_BIN" ]]; then
        ...
        # Read OLLAMA_PORT and YUYINODS_MODE from .env generated in phase 06
        if [[ -f "$INSTALL_DIR/.env" ]]; then
            [[ -z "${OLLAMA_PORT:-}" ]] && OLLAMA_PORT=$(grep -m1 '^OLLAMA_PORT=' "$INSTALL_DIR/.env" | cut -d= -f2-)
            # Always re-read YUYINODS_MODE from .env — Phase 06 may have changed it
            # (e.g. "local" → "lemonade" for AMD) but the shell variable is stale.
            YUYINODS_MODE=$(grep -m1 '^YUYINODS_MODE=' "$INSTALL_DIR/.env" | cut -d= -f2-)
        ...
        if [[ "${YUYINODS_MODE:-local}" == "lemonade" ]]; then
...
if [[ -f "$INSTALL_DIR/bin/yuyinods-host-agent.py" ]]; then
    ...
    if [[ -n "$AGENT_PYTHON" ]]; then
        if systemctl status >/dev/null 2>&1 || [[ -d /run/systemd/system ]]; then
            ...
            if [[ -f "$HOME/.config/systemd/user/yuyinods-host-agent.service" ]]; then
                systemctl --user stop yuyinods-host-agent.service 2>/dev/null || true
                systemctl --user disable yuyinods-host-agent.service 2>/dev/null || true
                rm -f "$HOME/.config/systemd/user/yuyinods-host-agent.service"
                systemctl --user daemon-reload 2>/dev/null || true
            ...
            if [[ -f "$INSTALL_DIR/scripts/systemd/yuyinods-host-agent.service" ]]; then
                svc_tmp="$(mktemp "${TMPDIR:-/tmp}/yuyinods-host-agent.service.XXXXXX")" || svc_tmp=""
                if [[ -z "$svc_tmp" ]]; then
                    ai_warn "Failed to create secure temp file for yuyinods-host-agent.service; skipping systemd unit install"
                else
                    cp "$INSTALL_DIR/scripts/systemd/yuyinods-host-agent.service" "$svc_tmp"
                    ...
                    if grep -q '__INSTALL_DIR__\|__HOME__\|__PYTHON3__\|__INSTALL_USER__' "$svc_tmp"; then
                        ...
                        sudo install -m 644 "$svc_tmp" /etc/systemd/system/yuyinods-host-agent.service
                    fi
                    rm -f "$svc_tmp"
                fi
            fi
            ...
            if sudo systemctl enable --now yuyinods-host-agent.service 2>&1 | tee -a "$LOG_FILE" >/dev/null; then
            ...
            if sudo systemctl is-enabled yuyinods-host-agent.service >/dev/null 2>&1; then
                if sudo systemctl restart yuyinods-host-agent.service 2>&1 | tee -a "$LOG_FILE" >/dev/null; then
                    ai_ok "ODS host agent restarted (loaded new binary)"
                else
                    ai_warn "ODS host agent restart failed (non-fatal) — run: sudo systemctl restart yuyinods-host-agent.service"
                fi
            fi
...
if [[ -f "$INSTALL_DIR/bin/yuyinods-mdns.py" ]] && [[ "$(uname -s)" == "Linux" ]]; then
    ...
    # Install the systemd unit alongside yuyinods-host-agent. Reuses the same
    ...
    if python3 -c "import zeroconf" 2>/dev/null && \
       ...
       [[ -f "$INSTALL_DIR/scripts/systemd/yuyinods-mdns.service" ]]; then
        ...
        svc_tmp="$(mktemp "${TMPDIR:-/tmp}/yuyinods-mdns.service.XXXXXX")" || svc_tmp=""
        if [[ -z "$svc_tmp" ]]; then
            ai_warn "Failed to create secure temp file for yuyinods-mdns.service; skipping systemd unit install"
        else
            cp "$INSTALL_DIR/scripts/systemd/yuyinods-mdns.service" "$svc_tmp"
            sed -i "s|__INSTALL_DIR__|${INSTALL_DIR}|g" "$svc_tmp" 2>/dev/null || \
                sed -i '' "s|__INSTALL_DIR__|${INSTALL_DIR}|g" "$svc_tmp"
            sed -i "s|__HOME__|${HOME}|g" "$svc_tmp" 2>/dev/null || \
                sed -i '' "s|__HOME__|${HOME}|g" "$svc_tmp"
            ...
            sed -i "s|__INSTALL_USER__|${_agent_user}|g" "$svc_tmp" 2>/dev/null || \
                sed -i '' "s|__INSTALL_USER__|${_agent_user}|g" "$svc_tmp"
            if grep -q '__INSTALL_DIR__\|__HOME__\|__PYTHON3__\|__INSTALL_USER__' "$svc_tmp"; then
                ai_warn "yuyinods-mdns systemd unit has unrendered placeholders — check $svc_tmp"
            else
                sudo install -m 644 "$svc_tmp" /etc/systemd/system/yuyinods-mdns.service
                sudo systemctl daemon-reload 2>/dev/null || true
                if sudo systemctl enable --now yuyinods-mdns.service 2>&1 | tee -a "$LOG_FILE" >/dev/null; then
                    _device_name="$(grep -E '^YUYINODS_DEVICE_NAME=' "$INSTALL_DIR/.env" 2>/dev/null | cut -d= -f2 | tr -d '"' || true)"
                    ...
                    # on yuyinods-proxy being on :80 and YUYINODS_PROXY_BIND=0.0.0.0. Those
                    ...
                    ai "  Enable yuyinods-proxy (YUYINODS_PROXY_BIND defaults to 0.0.0.0) to make http://${_device_name}.local serve chat. See docs/YuyinODS-PROXY.md."
                else
