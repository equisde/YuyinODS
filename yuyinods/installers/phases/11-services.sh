...
if $DRY_RUN; then
    ...
    _phase11_external_lemonade() {
        ...
        mode="${YUYINODS_MODE:-$(_phase11_env_get YUYINODS_MODE local)}"
    ...
    _phase11_apply_cpu_fallback() {
        ...
        _phase11_env_set YUYINODS_MODE "local"
    ...
    _phase11_allow_container_host_firewall() {
        local network_name="${1:-yuyinods-network}"
        local port="$2"
        local rule_label="$3"
        local bind_addr="${4:-}"
        ...
        if [[ -n "$bind_addr" && "$bind_addr" != "0.0.0.0" ]]; then
            if [[ "$rule_label" == "yuyinods-host-agent" ]]; then
                ai_warn "YUYINODS_AGENT_BIND=$bind_addr; skipping automatic host-agent firewall rule."
            else
    ...
    if [[ -x "$INSTALL_DIR/scripts/resolve-compose-stack.sh" ]]; then
        ...
        # for the rest of the install AND every subsequent yuyinods-cli invocation.
    ...
    # Persist compose flags so yuyinods-cli can reuse them without re-resolving
    ...
    _phase11_compose() {
        ...
        docker_tmp="${YUYINODS_DOCKER_TMPDIR:-${YUYINODS_TEMP_DIR:-/tmp}/docker-tmp}"
    ...
    _phase11_assert_managed_containers() {
        ...
        command_text="$(_phase11_compose_command_text)"
        ids="$(_phase11_compose "${COMPOSE_FLAGS_ARR[@]}" ps -q 2>>"$LOG_FILE" || true)"
        ...
        if [[ "$write_report" != "false" ]] && command -v write_compose_failure_report >/dev/null 2>&1; then
            report_path="$(COMPOSE_FLAGS_REPORT="${COMPOSE_FLAGS_ARR[*]}" write_compose_failure_report \
                ...
                "$LOG_FILE" \
                "${GPU_BACKEND:-unknown}" \
                "No YuyinODS containers were created. Run the saved ps/logs commands from the launch record, fix the compose/runtime failure, then re-run ./install.sh." |
                tail -n 1)" || true
...
" "$svc" 2>/dev/null || printf '\tfalse\n'
    ...
    if [[ "${YUYINODS_MODE:-local}" == "cloud" ]]; then
        ...
        ai "Existing Lemonade mode - skipping YuyinODS-managed GGUF download"
        litellm_cf="$INSTALL_DIR/extensions/services/litellm/compose.yaml"
        litellm_disabled="${litellm_cf}.disabled"
        if [[ -f "$litellm_disabled" && ! -f "$litellm_cf" ]]; then
            mv "$litellm_disabled" "$litellm_cf"
    ...
    if [[ "${YUYINODS_MODE:-local}" != "cloud" && -n "$GGUF_URL" ]] && ! _phase11_external_lemonade; then
        ...
        if [[ "${YUYINODS_MODE:-local}" != "cloud" && -n "$GGUF_URL" && ! -f "$GGUF_DIR/$GGUF_FILE" ]] && ! _phase11_external_lemonade; then
    ...
    if [[ "$ENABLE_COMFYUI" == "true" && "${YUYINODS_MODE:-local}" != "cloud" && ( "$GPU_BACKEND" == "amd" || "$GPU_BACKEND" == "nvidia" ) ]]; then
    ...
    if [[ "${YUYINODS_MODE:-local}" != "cloud" ]] && ! _phase11_external_lemonade; then
...
MODELS_INI_EOF
    ...
    if [[ "${ENABLE_HERMES:-false}" == "true" ]]; then
        ...
        if [[ -f "$_hermes_tpl" ]]; then
            ...
            if [[ "${GPU_BACKEND:-}" == "amd" && "${YUYINODS_MODE:-local}" != "cloud" ]] && ! _phase11_external_lemonade; then
            ...
            if [[ "${YUYINODS_MODE:-local}" != "cloud" ]] && { [[ "${GPU_BACKEND:-}" == "amd" ]] || _phase11_external_lemonade; }; then
    ...
    if ! _phase11_compose "${COMPOSE_FLAGS_ARR[@]}" config --quiet 1>/dev/null 2>"$LOG_FILE.compose-check"; then
    ...
    if ! _enabled_compose_services="$(_phase11_compose "${COMPOSE_FLAGS_ARR[@]}" config --services 2>>"$LOG_FILE")"; then
    ...
    if [[ ${#_failed_build_services[@]} -gt 0 ]]; then
        ...
        if [[ ${#_retained_failed_build_services[@]} -gt 0 ]]; then
            ai_bad "Build failed for core/overlay service(s): ${_retained_failed_build_services[*]}"
            ai "Builds use TMPDIR=${YUYINODS_DOCKER_TMPDIR:-${YUYINODS_TEMP_DIR:-/tmp}/docker-tmp} and COMPOSE_BAKE=false."
            ai "See $LOG_FILE for the Docker build error, fix it, then re-run the installer."
            exit 1
        fi
    fi
    ...
    _phase11_write_compose_launch_record
    for _attempt in 1 2 3; do
        _phase11_compose "${COMPOSE_FLAGS_ARR[@]}" up -d --remove-orphans --no-build >> "$LOG_FILE" 2>&1 &
        compose_pid=$!
    ...
    sleep 10
    _phase11_compose "${COMPOSE_FLAGS_ARR[@]}" up -d --remove-orphans --no-build >> "$LOG_FILE" 2>&1 || true
    # Step 3: catch any stragglers from the second pass
    $DOCKER_CMD start $($DOCKER_CMD ps -a --filter status=created -q) 2>/dev/null || true

    # If YUYINODS_AGENT_BIND is unset, the Linux host-agent binds to the ODS
    # Docker network gateway once that network exists. Phase 07 may have
    # started it before compose created yuyinods-network, so restart it here to
    # let the safer scoped bind take effect.
    if [[ -z "${YUYINODS_AGENT_BIND:-}" ]] \
      && [[ "$(uname -s 2>/dev/null || echo unknown)" == "Linux" ]] \
      && command -v systemctl >/dev/null 2>&1 \
      && sudo -n systemctl is-enabled yuyinods-host-agent.service >/dev/null 2>&1; then
        if sudo -n systemctl restart yuyinods-host-agent.service 2>&1 | tee -a "$LOG_FILE" >/dev/null; then
            ai_ok "Restarted yuyinods-host-agent after yuyinods-network creation"
        else
            ai_warn "yuyinods-host-agent restart after network creation failed (non-fatal)"
        fi
    fi
    ...
    # traffic. Add a scoped rule only after compose has created yuyinods-network,
    # so we allow the actual Docker subnet instead of a broad RFC1918 range.
    _phase11_allow_host_agent_firewall yuyinods-network
    ...
    if $compose_ok; then
        ...
        if [[ -n "${_python_cmd:-}" ]] && [[ -f "$INSTALL_DIR/scripts/build-installation-context.py" ]]; then
            ...
            if $DOCKER_CMD ps --format '{{.Names}}' 2>/dev/null | grep -qx 'yuyinods-hermes'; then
                $DOCKER_CMD exec yuyinods-hermes cp /opt/hermes/docker/SOUL.md /opt/data/SOUL.md \
                    >>"$LOG_FILE" 2>&1 || \
