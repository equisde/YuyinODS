...
_phase12_external_lemonade() {
    ...
    mode="${YUYINODS_MODE:-$(_phase12_env_get YUYINODS_MODE local)}"
...
if [[ "${YUYINODS_MODE:-local}" == "cloud" ]] || _phase12_external_lemonade; then
    ...
    if [[ "${COMPOSE_STARTED_WITH_DELAYED_HEALTH:-false}" == "true" ]]; then
        ...
        _llm_health_attempts="${YUYINODS_LLM_DELAYED_HEALTH_ATTEMPTS:-300}"
    fi
