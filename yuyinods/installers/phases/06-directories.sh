...
if $DRY_RUN; then
    ...
    YUYINODS_DATA_DIR="${YUYINODS_DATA_DIR:-$YUYINODS_DOCKER_ROOT/data}"
    YUYINODS_CONFIG_DIR="${YUYINODS_CONFIG_DIR:-$INSTALL_DIR/config}"
    ...
    if [[ "$YUYINODS_MODELS_DIR" != "$INSTALL_DIR/data/models" ]]; then
        rm -rf "$INSTALL_DIR/data/models"
        ln -sfn "$YUYINODS_MODELS_DIR" "$INSTALL_DIR/data/models"
    fi
...
Fix with: sudo chown -R \$(id -u):\$(id -g) $INSTALL_DIR/config $INSTALL_DIR/data — then re-run the installer."
    ...
    if [[ "$SCRIPT_DIR" != "$INSTALL_DIR" ]]; then
        ...
        chmod +x "$INSTALL_DIR"/*.sh "$INSTALL_DIR"/scripts/*.sh "$INSTALL_DIR"/yuyinods-cli 2>>"$LOG_FILE" || warn "Some scripts may not be executable — verify after install"
    ...
    # be picked up; preserve data/yuyinodsforge for users who want to archive it.
    _retired_odsforge_dir="$INSTALL_DIR/extensions/services/yuyinodsforge"
...
raise SystemExit(1)' 2>/dev/null && return 0
    ...
    YUYINODS_AGENT_KEY=$(_env_get YUYINODS_AGENT_KEY "$(openssl rand -hex 32 2>/dev/null || head -c 32 /dev/urandom | xxd -p)")
    # HMAC key for signing yuyinods-session cookies (magic-link redemption).
    ...
    YUYINODS_SESSION_SECRET=$(_env_get YUYINODS_SESSION_SECRET "$(openssl rand -hex 32 2>/dev/null || head -c 32 /dev/urandom | xxd -p)")
    ...
    LANGFUSE_PROJECT_PUBLIC_KEY=$(_env_get LANGFUSE_PROJECT_PUBLIC_KEY "pk-lf-yuyinods-$(openssl rand -hex 16 2>/dev/null || head -c 16 /dev/urandom | xxd -p)")
    LANGFUSE_PROJECT_SECRET_KEY=$(_env_get LANGFUSE_PROJECT_SECRET_KEY "sk-lf-yuyinods-$(openssl rand -hex 16 2>/dev/null || head -c 16 /dev/urandom | xxd -p)")
    ...
    YUYINODS_MODE_VALUE="$(if [[ "$LEMONADE_EXTERNAL_VALUE" == "true" ]]; then echo "lemonade"; elif [[ "$GPU_BACKEND" == "amd" && "${YUYINODS_MODE:-local}" == "local" ]]; then echo "lemonade"; else echo "${YUYINODS_MODE:-local}"; fi)"
    _default_llm_api_url="$(if [[ "$LEMONADE_EXTERNAL_VALUE" == "true" ]]; then echo "http://litellm:4000"; elif [[ "$GPU_BACKEND" == "amd" && "${YUYINODS_MODE:-local}" == "local" ]]; then echo "http://litellm:4000"; elif [[ "${YUYINODS_MODE:-local}" == "local" ]]; then echo "http://llama-server:8080"; else echo "http://litellm:4000"; fi)"
    ...
    if [[ "${YUYINODS_MODE:-local}" == "cloud" ]]; then
        ...
        _default_hermes_api_key="sk-yuyinods-hermes-local"
    fi
    ...
    # Device name — used by yuyinods-mdns (publishes <name>.local + per-service
    ...
    YUYINODS_DEVICE_NAME=$(_env_get YUYINODS_DEVICE_NAME "$_device_default")
    ...
    _phase06_lemonade_uses_host_9000() {
        ...
        [[ "${GPU_BACKEND:-}" == "amd" && "${YUYINODS_MODE:-local}" != "cloud" ]] && return 0
        return 1
    }
...
#=== ODS Version (used by yuyinods-cli update for version-compat checks) ===
YUYINODS_VERSION=${VERSION:-2.5.3}
...
#=== YuyinODS Paths ===
YUYINODS_BASE_DIR=${YUYINODS_BASE_DIR}
YUYINODS_APP_DIR=${YUYINODS_APP_DIR}
YUYINODS_MODELS_DIR=${YUYINODS_MODELS_DIR}
YUYINODS_TEMP_DIR=${YUYINODS_TEMP_DIR}
YUYINODS_DOCKER_ROOT=${YUYINODS_DOCKER_ROOT}
YUYINODS_DATA_DIR=${YUYINODS_DATA_DIR}
...
LLM_API_URL=${LLM_API_URL_VALUE}
LLM_BACKEND=$(if [[ "$YUYINODS_MODE_VALUE" == "lemonade" ]]; then echo "lemonade"; else echo "llama-server"; fi)
LLM_API_BASE_PATH=$(if [[ "$YUYINODS_MODE_VALUE" == "lemonade" ]]; then echo "${LEMONADE_API_BASE_PATH_VALUE}"; else echo "/v1"; fi)
AMD_INFERENCE_RUNTIME=$(if [[ "$LEMONADE_EXTERNAL_VALUE" == "true" || ( "$GPU_BACKEND" == "amd" && "${YUYINODS_MODE:-local}" == "local" ) ]]; then echo "lemonade"; else echo ""; fi)
AMD_INFERENCE_BACKEND=$(if [[ "$LEMONADE_EXTERNAL_VALUE" == "true" ]]; then echo "${AMD_INFERENCE_BACKEND:-auto}"; elif [[ "$GPU_BACKEND" == "amd" && "${YUYINODS_MODE:-local}" == "local" ]]; then echo "${BACKEND_LEMONADE_LINUX_BACKEND:-rocm}"; else echo ""; fi)
AMD_INFERENCE_LOCATION=$(if [[ "$LEMONADE_EXTERNAL_VALUE" == "true" ]]; then echo "host"; elif [[ "$GPU_BACKEND" == "amd" && "${YUYINODS_MODE:-local}" == "local" ]]; then echo "container"; else echo ""; fi)
AMD_INFERENCE_PORT=$(if [[ "$LEMONADE_EXTERNAL_VALUE" == "true" ]]; then echo "${LEMONADE_PORT_VALUE}"; elif [[ "$GPU_BACKEND" == "amd" && "${YUYINODS_MODE:-local}" == "local" ]]; then echo "${BACKEND_LEMONADE_API_PORT:-8080}"; else echo ""; fi)
AMD_INFERENCE_SUPPORTED_BACKENDS=$(if [[ "$LEMONADE_EXTERNAL_VALUE" == "true" ]]; then echo "${AMD_INFERENCE_SUPPORTED_BACKENDS:-auto}"; elif [[ "$GPU_BACKEND" == "amd" && "${YUYINODS_MODE:-local}" == "local" ]]; then echo "${BACKEND_LEMONADE_LINUX_BACKEND:-rocm}"; else echo ""; fi)
AMD_INFERENCE_RUNTIME_MODE=$(if [[ "$LEMONADE_EXTERNAL_VALUE" == "true" ]]; then echo "external-lemonade"; elif [[ "$GPU_BACKEND" == "amd" && "${YUYINODS_MODE:-local}" == "local" ]]; then echo "linux-container"; else echo ""; fi)
AMD_INFERENCE_MANAGED=$(if [[ "$LEMONADE_EXTERNAL_VALUE" == "true" ]]; then echo "false"; elif [[ "$GPU_BACKEND" == "amd" && "${YUYINODS_MODE:-local}" == "local" ]]; then echo "true"; else echo ""; fi)
...
HERMES_PROXY_UPSTREAM=${HERMES_PROXY_UPSTREAM:-yuyinods-hermes:9119}
YUYINODS_AUTH_UPSTREAM=${YUYINODS_AUTH_UPSTREAM:-yuyinods-dashboard-api:3002}

#=== Security (auto-generated, keep secret!) ===
WEBUI_SECRET=${WEBUI_SECRET}
DASHBOARD_API_KEY=${DASHBOARD_API_KEY}
YUYINODS_AGENT_KEY=${YUYINODS_AGENT_KEY}
YUYINODS_SESSION_SECRET=${YUYINODS_SESSION_SECRET}
...
# Used by yuyinods-mdns to publish <name>.local on the LAN, by yuyinods-proxy
...
# this line and restarting yuyinods-mdns + yuyinods-proxy.
YUYINODS_DEVICE_NAME=${YUYINODS_DEVICE_NAME}
