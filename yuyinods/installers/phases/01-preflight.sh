...
if [[ ! -d "$INSTALL_DIR" ]] && ! _ods_truthy "${YUYINODS_ALLOW_DREAMSERVER_PARALLEL:-}"; then
    ...
    if (( ${#_legacy_dreamserver_findings[@]} > 0 )); then
        ...
        error "Existing DreamServer install detected before first ODS install. Stop, uninstall, or migrate the old DreamServer stack first. To run both intentionally, set YUYINODS_ALLOW_DREAMSERVER_PARALLEL=1 and choose non-conflicting ports/install paths."
    fi
...
re-run, e.g.:  INSTALL_DIR=\"\$HOME/yuyinods\" $0"
            ;;
    esac
...
# binds to the yuyinods-network gateway by default on Linux, and compose containers
...
if command -v ufw >/dev/null 2>&1 && systemctl is-active --quiet ufw 2>/dev/null; then
    ...
    ai_warn "The installer will auto-add a scoped rule for the actual yuyinods-network subnet after compose starts."
