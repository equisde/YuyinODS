...
if [[ "$GPU_BACKEND" == "amd" ]] && $DRY_RUN; then
    ...
    if [[ -d "$INSTALL_DIR/scripts/systemd" ]]; then
        for _unit in "$INSTALL_DIR/scripts/systemd"/*.service "$INSTALL_DIR/scripts/systemd"/*.timer; do
            ...
            case "$(basename "$_unit")" in
                yuyinods-host-agent.service|yuyinods-ap-mode.service) continue ;;
            esac
    ...
    mkdir -p "$INSTALL_DIR/data/memory-archives/yuyinods-agent"/{memory,agents,tools}
    ...
    if [[ "$total_ram_mb" -gt 0 ]]; then
        ...
        cat > /tmp/yuyinods-gtt-tuning.conf << GTT_EOF
...
GTT_EOF
        if sudo -n cp /tmp/yuyinods-gtt-tuning.conf /etc/modprobe.d/amdgpu_llm_optimized.conf 2>/dev/null; then
            ...
            ai "  sudo cp /tmp/yuyinods-gtt-tuning.conf /etc/modprobe.d/amdgpu_llm_optimized.conf"
        fi
        rm -f /tmp/yuyinods-gtt-tuning.conf
    else
