...
if $INTERACTIVE && ! $DRY_RUN; then
    ...
    if [[ "${INSTALL_CHOICE:-1}" == "3" ]]; then
        ...
        # Warn if ComfyUI enabled on low-tier hardware. If Full Stack enabled
        # it before reaching custom prompts, keep the default as enabled.
...
# Tier safety net: optionally disable ComfyUI on Tier 0/1 in non-interactive
# mode. Full Stack keeps it enabled by default; set
# YUYINODS_AUTO_DISABLE_COMFYUI=true to restore automatic pruning.
if ! $INTERACTIVE && [[ "$ENABLE_COMFYUI" == "true" && "${YUYINODS_AUTO_DISABLE_COMFYUI:-false}" == "true" ]]; then
    case "${TIER:-}" in
        0|1)
...
if [[ "${ENABLE_HERMES:-false}" == "true" && "${YUYINODS_MODE:-local}" != "cloud" ]]; then
...
if ! $DRY_RUN; then
    ...
    _sync_extension_compose "${ENABLE_ODS_PROXY:-false}" yuyinods-proxy "ODS proxy" "LAN web proxy not enabled"
...
TOPOLOGY_FILE=$(mktemp /tmp/yuyinods_gpu_topology.XXXXXX.json)
