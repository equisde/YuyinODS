...
generate_ods_env() {
    ...
    if [[ -f "$env_path" ]] && [[ "$force_overwrite" != "true" ]]; then
        ...
        # Upsert YUYINODS_AGENT_KEY when missing (pre-PR-#979 upgrade path)
        if [[ -z "$(read_env_value "$env_path" "YUYINODS_AGENT_KEY")" ]]; then
            upsert_env_value "$env_path" "YUYINODS_AGENT_KEY" "$(new_secure_hex 32)"
        fi
        ...
        if [[ -z "$(read_env_value "$env_path" "YUYINODS_AGENT_HOST")" ]]; then
            upsert_env_value "$env_path" "YUYINODS_AGENT_HOST" "host.docker.internal"
        fi
        ...
        # Upsert YUYINODS_SESSION_SECRET when missing — HMAC key for the
        # yuyinods-session cookie minted by magic-link redemption. Without
        ...
        if [[ -z "$(read_env_value "$env_path" "YUYINODS_SESSION_SECRET")" ]]; then
            upsert_env_value "$env_path" "YUYINODS_SESSION_SECRET" "$(new_secure_hex 32)"
        fi
        # YUYINODS_DEVICE_NAME backfill: older macOS installs omitted this key,
        # so magic links defaulted to auth.ods.local/chat.ods.local and
        # collided with every other default install on the LAN.
        if [[ -z "$(read_env_value "$env_path" "YUYINODS_DEVICE_NAME")" ]]; then
            upsert_env_value "$env_path" "YUYINODS_DEVICE_NAME" "$(detect_device_name)"
        fi
    ...
    local litellm_key
    litellm_key="sk-yuyinods-$(new_secure_hex 16)"
    ...
    langfuse_project_public_key="pk-lf-yuyinods-$(new_secure_hex 16)"
    local langfuse_project_secret_key
    langfuse_project_secret_key="sk-lf-yuyinods-$(new_secure_hex 16)"
...
# Generated by macOS installer v${YUYINODS_VERSION} on ${timestamp}
# Tier: ${tier} (${TIER_NAME})
...
# Device name used by yuyinods-mdns/yuyinods-proxy hostnames and magic-link URLs.
...
YUYINODS_DEVICE_NAME=${device_name}
# Docker Desktop containers reach loopback-only host services through this name.
YUYINODS_AGENT_HOST=${YUYINODS_AGENT_HOST:-host.docker.internal}
...
HERMES_LLM_API_KEY=sk-yuyinods-hermes-local
HERMES_LANGUAGE=en
...
YUYINODS_AGENT_KEY=${ods_agent_key}
YUYINODS_SESSION_SECRET=${ods_session_secret}
...
# here and run 'yuyinods-macos.sh restart' to use a different model.
