#!/bin/bash
# ============================================================================
# YuyinODS macOS Installer -- UI Helpers
# ============================================================================
...
log() { echo -e "${GRN}[INFO]${NC} $1" | tee -a "$YUYINODS_LOG_FILE"; }

# ── AI narrator voice ──

ai()       { echo -e "  ${GRN}>${NC} $1" | tee -a "$YUYINODS_LOG_FILE"; }
ai_ok()    { echo -e "  ${BGRN}[OK]${NC} $1" | tee -a "$YUYINODS_LOG_FILE"; }
ai_warn()  { echo -e "  ${AMB}[!!]${NC} $1" | tee -a "$YUYINODS_LOG_FILE"; }
ai_err()   { echo -e "  ${RED}[XX]${NC} $1" | tee -a "$YUYINODS_LOG_FILE"; }
info_box() { echo -e "  ${DGRN}$1${NC} ${WHT}$2${NC}" | tee -a "$YUYINODS_LOG_FILE"; }
...
show_phase() {
    ...
    echo ""
    echo -e "  ${DGRN}YUYINODS SEQUENCE [${elapsed}]${NC}  ${WHT}PHASE ${phase}/${total}${NC} ${BGRN}-- ${name}${NC}"
    if [[ -n "$estimate" ]]; then
...
show_ods_banner() {
    echo ""
    echo -e "${BGRN}  YY   YY UU   UU YY   YY III NN   NN  OOOOO  DDDD   SSSSS${NC}"
    echo -e "${BGRN}   YY YY  UU   UU  YY YY  III NNN  NN OO   OO DD DD SS${NC}"
    echo -e "${BGRN}    YYY   UU   UU   YYY   III NN N NN OO   OO DD DD  SSS${NC}"
    echo -e "${BGRN}    YYY   UU   UU   YYY   III NN  NNN OO   OO DD DD    SS${NC}"
    echo -e "${BGRN}    YYY    UUUUU    YYY   III NN   NN  OOOOO  DDDD  SSSS${NC}"
    echo ""
    echo -e "  ${WHT}YuyinODS macOS Installer v${YUYINODS_VERSION}${NC}"
...
download_with_progress() {
    ...
    local max_retries="${4:-${YUYINODS_MODEL_DOWNLOAD_RETRIES:-8}}"
    ...
    local connect_timeout="${YUYINODS_DOWNLOAD_CONNECT_TIMEOUT:-30}"
    local low_speed_time="${YUYINODS_DOWNLOAD_LOW_SPEED_TIME:-300}"
    local low_speed_limit="${YUYINODS_DOWNLOAD_LOW_SPEED_LIMIT:-1024}"
    local attempt=1
...
show_success_card() {
    ...
    local _bind
    _bind=$(grep "^BIND_ADDRESS=" "$YUYINODS_INSTALL_DIR/.env" 2>/dev/null | cut -d= -f2- | tr -d '"' || echo "127.0.0.1")
    ...
    echo ""
    echo -e "       ${DGRN}Manage:${NC}     ${GRN}./yuyinods-macos.sh status${NC}"
    echo -e "       ${DGRN}Logs:${NC}       ${GRN}./yuyinods-macos.sh logs llama-server${NC}"
    echo -e "       ${DGRN}Stop:${NC}       ${GRN}./yuyinods-macos.sh stop${NC}"
    echo ""
