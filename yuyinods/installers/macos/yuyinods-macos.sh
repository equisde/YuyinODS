#!/bin/bash
# ============================================================================
# ODS macOS CLI -- yuyinods-macos.sh
# ============================================================================
# Day-to-day management of a ODS installation on macOS.
# Mirrors the Windows yuyinods.ps1 command structure.
#
# Usage:
#   ./yuyinods-macos.sh status              # Health checks + Apple Silicon info
#   ./yuyinods-macos.sh start [service]     # Start all or one service
#   ./yuyinods-macos.sh stop [service]      # Stop all or one service
#   ./yuyinods-macos.sh restart [service]   # Restart all or one service
#   ./yuyinods-macos.sh logs <service> [N]  # Tail logs (default 100 lines)
#   ./yuyinods-macos.sh config show         # View .env (secrets masked)
#   ./yuyinods-macos.sh config edit         # Open .env in $EDITOR
#   ./yuyinods-macos.sh chat "message"      # Quick chat via API
#   ./yuyinods-macos.sh update              # Pull latest images and restart
#   ./yuyinods-macos.sh version             # Show version
#   ./yuyinods-macos.sh help                # Show help
#
# ============================================================================
...
# Lets `bash /path/to/install/yuyinods-macos.sh status` work without YUYINODS_HOME
...
export YUYINODS_SCRIPT_HINT="$SCRIPT_DIR"
...
test_install() {
    if [[ ! -d "$INSTALL_DIR" ]]; then
        ...
        ai "Invoke from inside the install dir (bash <install>/yuyinods-macos.sh status), export YUYINODS_HOME=<install>, or run the installer."
        exit 1
    fi
...
get_compose_flags() {
    ...
    # cache is missing or stale. Mirrors yuyinods-cli's get_compose_flags fallback.
...
cmd_logs() {
    ...
    if [[ -z "$service" ]]; then
        ai "Usage: ./yuyinods-macos.sh logs <service> [lines]"
...
cmd_chat() {
    local message="${1:-}"
    if [[ -z "$message" ]]; then
        ai "Usage: ./yuyinods-macos.sh chat \"your message\""
        return
    fi
    ...
    response=$(curl -sf -X POST "http://127.0.0.1:8080/v1/chat/completions" \
        ...
        ai "Is llama-server running? Try: ./yuyinods-macos.sh status"
        return
    }
...
cmd_version() {
    echo -e "${BGRN}ODS v${YUYINODS_VERSION} (macOS Apple Silicon)${NC}"
}

show_help() {
    echo ""
    echo -e "  ${BGRN}ODS CLI (macOS)${NC}"
    echo -e "  ${DGRN}Version ${YUYINODS_VERSION}${NC}"
    echo ""
    echo -e "  ${WHT}USAGE${NC}"
    echo -e "  ${DGRN}  ./yuyinods-macos.sh <command> [options]${NC}"
    echo ""
    ...
    echo -e "  ${DGRN}  ./yuyinods-macos.sh status${NC}"
    echo -e "  ${DGRN}  ./yuyinods-macos.sh logs llama-server 50${NC}"
    echo -e "  ${DGRN}  ./yuyinods-macos.sh restart open-webui${NC}"
    echo -e "  ${DGRN}  ./yuyinods-macos.sh chat \"What is quantum computing?\"${NC}"
    echo ""
}
