...
function New-ODSEnv {
    ...
    $litellmKey      = Get-EnvOrNew "LITELLM_KEY"        "sk-yuyinods-$(New-SecureHex -Bytes 16)"
    $litellmLemonadeApiKey = Get-EnvOrNew "LITELLM_LEMONADE_API_KEY" "sk-yuyinods-lemonade-$(New-SecureHex -Bytes 16)"
    ...
    $odsAgentKey   = Get-EnvOrNew "YUYINODS_AGENT_KEY"    (New-SecureHex -Bytes 32)
    # HMAC key for signing yuyinods-session cookies. Without it the dashboard-api
    # session_signer raises on issue() and verify-session returns no-secret —
    # the magic-link gate effectively breaks.
    $odsSessionSecret = Get-EnvOrNew "YUYINODS_SESSION_SECRET" (New-SecureHex -Bytes 32)
    ...
    $langfuseProjectPublicKey  = Get-EnvOrNew "LANGFUSE_PROJECT_PUBLIC_KEY" "pk-lf-yuyinods-$(New-SecureHex -Bytes 16)"
    $langfuseProjectSecretKey  = Get-EnvOrNew "LANGFUSE_PROJECT_SECRET_KEY" "sk-lf-yuyinods-$(New-SecureHex -Bytes 16)"
    ...
    # Lemonade, YUYINODS_MODE must also be lemonade so the LiteLLM container mounts
    ...
    $hermesLlmApiKey = $(if ($hermesUsesLiteLlm) { $litellmKey } else { "sk-yuyinods-hermes-local" })
...
# Generated by Windows installer v$($script:YUYINODS_VERSION) on $timestamp
...
YUYINODS_AGENT_HOST=$(Get-EnvOrNew "YUYINODS_AGENT_HOST" "host.docker.internal")
...
YUYINODS_AGENT_BIND=$(Get-EnvOrNew "YUYINODS_AGENT_BIND" "0.0.0.0")

#=== YuyinODS Paths ===
YUYINODS_BASE_DIR=$(Get-EnvOrNew "YUYINODS_BASE_DIR" "$($script:YUYINODS_BASE_DIR)")
YUYINODS_APP_DIR=$(Get-EnvOrNew "YUYINODS_APP_DIR" "$($script:YUYINODS_APP_DIR)")
YUYINODS_MODELS_DIR=$(Get-EnvOrNew "YUYINODS_MODELS_DIR" "$($script:YUYINODS_MODELS_DIR)")
YUYINODS_TEMP_DIR=$(Get-EnvOrNew "YUYINODS_TEMP_DIR" "$($script:YUYINODS_TEMP_DIR)")
YUYINODS_DOCKER_ROOT=$(Get-EnvOrNew "YUYINODS_DOCKER_ROOT" "$($script:YUYINODS_DOCKER_ROOT)")
YUYINODS_DATA_DIR=$(Get-EnvOrNew "YUYINODS_DATA_DIR" "$(Join-Path $script:YUYINODS_DOCKER_ROOT "data")")
YUYINODS_CONFIG_DIR=$(Get-EnvOrNew "YUYINODS_CONFIG_DIR" "$InstallDir\config")
YUYINODS_HOME=$(Get-EnvOrNew "YUYINODS_HOME" "$InstallDir")
YUYINODS_INSTALL_DIR=$(Get-EnvOrNew "YUYINODS_INSTALL_DIR" "$InstallDir")
YUYINODS_MODELS_DIR=$(Get-EnvOrNew "YUYINODS_MODELS_DIR" "$($script:YUYINODS_MODELS_DIR)")
YUYINODS_TEMP_DIR=$(Get-EnvOrNew "YUYINODS_TEMP_DIR" "$($script:YUYINODS_TEMP_DIR)")
YUYINODS_DOCKER_ROOT=$(Get-EnvOrNew "YUYINODS_DOCKER_ROOT" "$($script:YUYINODS_DOCKER_ROOT)")
COMPOSE_PROJECT_NAME=$(Get-EnvOrNew "COMPOSE_PROJECT_NAME" "yuyinods")

#=== LLM Backend Mode ===
YUYINODS_MODE=$effectiveODSMode
...
HERMES_PROXY_UPSTREAM=yuyinods-hermes:9119
YUYINODS_AUTH_UPSTREAM=yuyinods-dashboard-api:3002

#=== Security (auto-generated, keep secret!) ===
WEBUI_SECRET=$webuiSecret
DASHBOARD_API_KEY=$dashboardApiKey
YUYINODS_AGENT_KEY=$odsAgentKey
YUYINODS_SESSION_SECRET=$odsSessionSecret
...
model_list:
  - model_name: default
    litellm_params:
      ...
      api_key: sk-yuyinods-hermes-local
      extra_body:
