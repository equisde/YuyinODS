...
if ($dryRun) {
    ...
    Write-AI "[DRY RUN] Would copy yuyinods.ps1 CLI + lib/ to install root"
    if ($enableOpenClaw) {
...
$_dockerRoot = $(if ($script:YUYINODS_DOCKER_ROOT) { $script:YUYINODS_DOCKER_ROOT } else { Join-Path "/home/parallax/ODS" "root-container" })
$_dataDir   = $(if ($env:YUYINODS_DATA_DIR) { $env:YUYINODS_DATA_DIR } else { Join-Path $_dockerRoot "data" })
$_modelsDir = $(if ($script:YUYINODS_MODELS_DIR) { $script:YUYINODS_MODELS_DIR } else { Join-Path "/home/parallax/ODS" "models" })
$_tempDir   = $(if ($script:YUYINODS_TEMP_DIR) { $script:YUYINODS_TEMP_DIR } else { Join-Path "/home/parallax/ODS" "temp" })
...
$_compatDataDir = Join-Path $installDir "data"
if ($_compatDataDir -ne $_dataDir) {
    if (Test-Path -LiteralPath $_compatDataDir -PathType Container) {
        $existing = Get-ChildItem -LiteralPath $_compatDataDir -Force -ErrorAction SilentlyContinue
        if (-not $existing) {
            Remove-Item -LiteralPath $_compatDataDir -Force
        }
    }
    if (-not (Test-Path -LiteralPath $_compatDataDir)) {
        New-Item -ItemType Junction -Path $_compatDataDir -Target $_dataDir | Out-Null
    }
}
$_compatModelsDir = Join-Path $_dataDir "models"
if ($_compatModelsDir -ne $_modelsDir) {
    if (Test-Path -LiteralPath $_compatModelsDir -PathType Container) {
        $existing = Get-ChildItem -LiteralPath $_compatModelsDir -Force -ErrorAction SilentlyContinue
        if (-not $existing) {
            Remove-Item -LiteralPath $_compatModelsDir -Force
        }
    }
    if (-not (Test-Path -LiteralPath $_compatModelsDir)) {
        New-Item -ItemType Junction -Path $_compatModelsDir -Target $_modelsDir | Out-Null
    }
}
...
$_expectedRegularFiles = @(
    ...
    "extensions\services\yuyinods-proxy\Caddyfile",
...
# preserving data/yuyinodsforge for user-controlled archival.
...
# can manage ODS with: .\yuyinods.ps1 status
...
$_odsSrc  = Join-Path $_scriptDir "yuyinods.ps1"
$_odsDst  = Join-Path $installDir "yuyinods.ps1"
if (Test-Path $_odsSrc) {
    Copy-Item -Path $_odsSrc -Destination $_odsDst -Force
    # Also copy lib/ so yuyinods.ps1 can find its helper functions
    ...
    Write-Utf8NoBom -Path (Join-Path $installDir "yuyinods-cli.cmd") -Content $_cmdShim
    ...
    Write-AISuccess "Installed ODS CLI shims (ods, yuyinods-cli)"
} else {
    Write-AIWarn "yuyinods.ps1 not found at $_odsSrc -- CLI management unavailable"
}
...
function Invoke-HermesSoulRefresh {
    ...
    if (Test-Path $_builder) {
        ...
        foreach ($_candidate in $_pythonCandidates) {
            ...
            try {
                & $_cmd.Source @($_candidate.Args) $_builder "--template" $_template "--env" $_envPath "--output" $_output @_profileArgs *>> $script:YUYINODS_LOG_FILE
                ...
                Add-Content -Path $script:YUYINODS_LOG_FILE -Value "Hermes SOUL.md render failed with $($_candidate.Command): $_msg"
            }
        }
    }
    ...
    if ($SyncContainer) {
        ...
        if ($_names -contains "yuyinods-hermes") {
