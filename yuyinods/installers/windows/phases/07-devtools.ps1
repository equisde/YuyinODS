...
if ($dryRun) {
    ...
    Write-AI "[DRY RUN] Would start ODS Host Agent on port $($script:YUYINODS_AGENT_PORT)"
    Write-AI "[DRY RUN] Would register $($script:YUYINODS_AGENT_TASK_NAME) scheduled task for login persistence"
    return
}
...
function Test-ODSHostAgentPythonCandidate {
    ...
    # runnable Python interpreter for yuyinods-host-agent.
...
$_agentScript = Join-Path (Join-Path $installDir "bin") "yuyinods-host-agent.py"
if (Test-Path $_agentScript) {
    ...
    if ($_python3) {
        ...
        if (Test-Path $script:YUYINODS_AGENT_PID_FILE) {
            $_oldPid = $null
            try {
                $_oldPid = [int](Get-Content $script:YUYINODS_AGENT_PID_FILE -Raw).Trim()
                Stop-Process -Id $_oldPid -Force -ErrorAction SilentlyContinue
            } catch { }
            Remove-Item $script:YUYINODS_AGENT_PID_FILE -Force -ErrorAction SilentlyContinue
        }

        # Ensure data directory exists for PID and log files
        $pidDir = Split-Path $script:YUYINODS_AGENT_PID_FILE
        ...
        $_pidFileLiteral = & $_psQuote $script:YUYINODS_AGENT_PID_FILE
        $_installDirLiteral = & $_psQuote $installDir
        $_logFileLiteral = & $_psQuote $script:YUYINODS_AGENT_LOG_FILE
        $_agentCommand = @"
`$env:PATH = $_dockerPathLiteral + `$env:PATH
`$agentArgs = $_pythonPrefixArgsLiteral + @($_agentScriptLiteral, '--port', '$($script:YUYINODS_AGENT_PORT)', '--pid-file', $_pidFileLiteral, '--install-dir', $_installDirLiteral)
...
"@
        ...
        try {
            ...
            Write-AISuccess "Host agent scheduled and started (Task: $($script:YUYINODS_AGENT_TASK_NAME))"
            ...
            try {
                ...
                if ($resp.StatusCode -eq 200) {
                    Write-AISuccess "ODS host agent started (port $($script:YUYINODS_AGENT_PORT))"
                } else {
                    Write-AIWarn "ODS host agent started but health check returned $($resp.StatusCode)"
                }
            } catch {
                Write-AIWarn "ODS host agent started but not yet responding -- check: .\yuyinods.ps1 agent status"
            }
        } catch {
            $taskError = $_
            Write-AIWarn "Could not register login task -- start manually: .\yuyinods.ps1 agent start"
        ...
        Write-AI "  Install Python 3.12 and re-run the installer, or start manually: .\yuyinods.ps1 agent start"
    }
} else {
