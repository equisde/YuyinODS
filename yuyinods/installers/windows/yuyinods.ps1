# ============================================================================
# ODS Windows CLI -- yuyinods.ps1
# ============================================================================
# Day-to-day management of a ODS installation on Windows.
# Mirrors the Linux yuyinods-cli command structure.
#
# Usage:
#   .\yuyinods.ps1 status              # Health checks + GPU status
#   .\yuyinods.ps1 start [service]     # Start all or one service
#   .\yuyinods.ps1 stop [service]      # Stop all or one service
#   .\yuyinods.ps1 restart [service]   # Restart all or one service
#   .\yuyinods.ps1 logs <service> [N]  # Tail logs (default 100 lines)
#   .\yuyinods.ps1 config show         # View .env (secrets masked)
#   .\yuyinods.ps1 config edit         # Open .env in notepad
#   .\yuyinods.ps1 chat "message"      # Quick chat via API
#   .\yuyinods.ps1 update              # Pull latest images and restart
#   .\yuyinods.ps1 doctor              # Diagnose runtime readiness
#   .\yuyinods.ps1 repair voice        # Repair voice/STT/TTS readiness
#   .\yuyinods.ps1 report              # Generate Windows diagnostics bundle
#   .\yuyinods.ps1 version             # Show version
#   .\yuyinods.ps1 help                # Show help
#
# ============================================================================
...
$InstallDir = $script:YUYINODS_INSTALL_DIR
...
function Test-Install {
    if (-not (Test-Path $InstallDir)) {
        Write-AIError "ODS not found at $InstallDir. Set YUYINODS_HOME or run installer first."
        exit 1
    }
...
function Invoke-HermesSoulRefresh {
    <#
    .SYNOPSIS
        Render data/persona/SOUL.md and optionally copy it into yuyinods-hermes.
    #>
    ...
    if (Test-Path $builder) {
        ...
        foreach ($candidate in $pythonCandidates) {
            ...
            try {
                & $cmd.Source @($candidate.Args) $builder "--template" $template "--env" $envPath "--output" $output @profileArgs *>> $script:YUYINODS_LOG_FILE
                if ($LASTEXITCODE -eq 0 -and (Test-Path -LiteralPath $output -PathType Leaf)) {
                    $rendered = $true
                    break
                }
            } catch {
                Add-Content -Path $script:YUYINODS_LOG_FILE -Value "Hermes SOUL.md refresh failed with $($candidate.Command): $($_.Exception.Message)"
            }
        }
    }
    ...
    if ($SyncContainer) {
        ...
        if ($names -contains "yuyinods-hermes") {
            & docker exec yuyinods-hermes cp /opt/hermes/docker/SOUL.md /opt/data/SOUL.md *>> $script:YUYINODS_LOG_FILE
...
function Invoke-ODSSttModelDownloadTrigger {
    ...
    # bounded so slow Hugging Face transfers do not wedge yuyinods.ps1 or install.
...
function Invoke-Status {
    ...
    try {
        ...
        # Host agent status
        try {
            $resp = Invoke-WebRequest -Uri $script:YUYINODS_AGENT_HEALTH_URL `
                -TimeoutSec 3 -UseBasicParsing -ErrorAction SilentlyContinue
            if ($resp.StatusCode -eq 200) {
                Write-AISuccess "Host Agent: running (port $($script:YUYINODS_AGENT_PORT))"
            } else {
                Write-AIWarn "Host Agent: responded with $($resp.StatusCode)"
            }
        } catch {
            Write-AIWarn "Host Agent: not responding (port $($script:YUYINODS_AGENT_PORT))"
        }
        ...
        if ($Service) {
            ...
            if ($composeExit -ne 0) {
                ...
                Write-ODSComposeDiagnostics -InstallDir $InstallDir -ComposeFlags $flags `
                    -Phase "yuyinods.ps1 start ($Service)"
                exit 1
            }
            ...
            if ($composeExit -ne 0) {
                Write-AIError "docker compose up failed (exit code: $composeExit)"
                Write-ODSComposeDiagnostics -InstallDir $InstallDir -ComposeFlags $flags -Phase "yuyinods.ps1 start (all)"
                exit 1
            }
    ...
    try {
        ...
        if ($Service) {
            ...
            if ($composeExit -ne 0) {
                ...
                Write-ODSComposeDiagnostics -InstallDir $InstallDir -ComposeFlags $flags `
                    -Phase "yuyinods.ps1 stop ($Service)"
                exit 1
            }
            ...
            if ($composeExit -ne 0) {
                ...
                Write-ODSComposeDiagnostics -InstallDir $InstallDir -ComposeFlags $flags -Phase "yuyinods.ps1 stop (all)"
                exit 1
            }
    ...
    try {
        ...
        if ($Service) {
            ...
            if ($composeExit -ne 0) {
                ...
                Write-ODSComposeDiagnostics -InstallDir $InstallDir -ComposeFlags $flags `
                    -Phase "yuyinods.ps1 restart ($Service)"
                exit 1
            }
            ...
            if ($composeExit -ne 0) {
                Write-AIError "docker compose restart failed (exit code: $composeExit)"
                Write-ODSComposeDiagnostics -InstallDir $InstallDir -ComposeFlags $flags -Phase "yuyinods.ps1 restart (all)"
                exit 1
            }
...
function Invoke-Logs {
    ...
    if (-not $Service) {
        Write-AI "Usage: .\yuyinods.ps1 logs <service> [lines]"
...
function Invoke-Chat {
    ...
    if (-not $Message) {
        Write-AI "Usage: .\yuyinods.ps1 chat `"your message`""
        return
    }
    ...
    try {
        ...
        Write-AI "Is llama-server running? Try: .\yuyinods.ps1 status"
    }
}
...
function Invoke-Update {
    ...
    try {
        ...
        if ($pullExit -ne 0) {
            ...
            Write-ODSComposeDiagnostics -InstallDir $InstallDir -ComposeFlags $flags -Phase "yuyinods.ps1 update (pull)"
            exit 1
        }
        ...
        if ($upExit -ne 0) {
            ...
            Write-ODSComposeDiagnostics -InstallDir $InstallDir -ComposeFlags $flags -Phase "yuyinods.ps1 update (up --force-recreate)"
            exit 1
        }
...
function Invoke-Doctor {
    ...
    try {
        ...
        if ($voice.SttModelCached) {
            ...
            Write-Host "  Repair: .\yuyinods.ps1 repair voice" -ForegroundColor DarkGray
...
function Invoke-RepairVoice {
    ...
    try {
        ...
        if ($composeExit -ne 0) {
            Write-AIError "docker compose up failed (exit code: $composeExit)"
            Write-ODSComposeDiagnostics -InstallDir $InstallDir -ComposeFlags $flags -Phase "yuyinods.ps1 repair voice"
            exit 1
        }
        ...
        $voice = Get-ODSVoiceDiagnosis
        if (-not $voice.WhisperHealthy) {
            Write-AIError "Whisper STT is still not responding. Check: .\yuyinods.ps1 logs whisper 100"
            exit 1
        }
        ...
        if ($voice.TtsHealthy) {
            ...
            Write-AIError "Kokoro TTS is still not responding. Check: .\yuyinods.ps1 logs tts 100"
            exit 1
        }
...
function Invoke-Repair {
    ...
    switch ($Target.ToLower()) {
        ...
        default {
            Write-AI "Usage: .\yuyinods.ps1 repair voice"
...
function Invoke-Agent {
    ...
    $agentScript = Join-Path (Join-Path $InstallDir "bin") "yuyinods-host-agent.py"
    $pidFile     = $script:YUYINODS_AGENT_PID_FILE
    $logFile     = $script:YUYINODS_AGENT_LOG_FILE
    $port        = $script:YUYINODS_AGENT_PORT
    $healthUrl   = $script:YUYINODS_AGENT_HEALTH_URL
...
"@
            ...
            try { Stop-ScheduledTask -TaskName $script:YUYINODS_AGENT_TASK_NAME -ErrorAction SilentlyContinue } catch { }
            ...
            Register-ScheduledTask -TaskName $script:YUYINODS_AGENT_TASK_NAME `
                ...
                -Force | Out-Null
            Start-ScheduledTask -TaskName $script:YUYINODS_AGENT_TASK_NAME
            ...
            try {
                ...
                Write-AIWarn "Host agent started but not yet responding -- check: .\yuyinods.ps1 agent status"
            }
        }
        "stop" {
            try { Stop-ScheduledTask -TaskName $script:YUYINODS_AGENT_TASK_NAME -ErrorAction SilentlyContinue } catch { }
        ...
        default {
            Write-Host ""
            Write-Host "  Usage: .\yuyinods.ps1 agent [status|start|stop|restart|logs]" -ForegroundColor DarkGray
            Write-Host ""
        }
    }
}
...
function Show-Help {
    ...
    Write-Host "  Version $($script:YUYINODS_VERSION)" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  USAGE" -ForegroundColor White
    Write-Host "    .\yuyinods.ps1 <command> [options]" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host "    .\yuyinods.ps1 status" -ForegroundColor DarkGray
    Write-Host "    .\yuyinods.ps1 logs llama-server 50" -ForegroundColor DarkGray
    Write-Host "    .\yuyinods.ps1 restart open-webui" -ForegroundColor DarkGray
    Write-Host "    .\yuyinods.ps1 repair voice" -ForegroundColor DarkGray
    Write-Host "    .\yuyinods.ps1 chat `"What is quantum computing?`"" -ForegroundColor DarkGray
    Write-Host ""
}
...
switch ($Command.ToLower()) {
    ...
    "version" { Write-Host "ODS v$($script:YUYINODS_VERSION) (Windows)" -ForegroundColor Green }
    "help"    { Show-Help }
    default   {
