...
if (-not [string]::IsNullOrWhiteSpace($InstallDir)) {
    $env:YUYINODS_HOME = [System.IO.Path]::GetFullPath($InstallDir)
    $env:YUYINODS_HOME = [System.IO.Path]::GetFullPath($InstallDir)
}
    ...
    try {
        ...
        if ($gpuInfo.Backend -eq "amd" -and -not $cloudMode) {
            ...
            if ($useLemonade) {
                ...
                if (Test-Path $envPath) {
                    $envContent = Get-Content $envPath -Raw
                    $envContent = $envContent -replace "(?m)^YUYINODS_MODE=.*$", "YUYINODS_MODE=local"
...
"@
        ...
        # Save compose flags before build/up so yuyinods.ps1 and diagnostics have
        ...
        function Invoke-ODSWindowsPlainDockerBuildService {
            ...
            if ([string]::IsNullOrWhiteSpace($imageTag)) {
                $imageTag = "yuyinods-${Service}:latest"
            }
...
$llmContainer = if ($useLemonade -or $cloudMode -or $gpuInfo.Backend -eq "amd") { "" } else { "yuyinods-llama-server" }
$readinessChecks = @(
    @{ Name = "Dashboard"; Url = "http://localhost:$dashboardPort"; Container = "yuyinods-dashboard"; OpenUrl = "http://localhost:$dashboardPort" }
    @{ Name = "Chat UI (Open WebUI)"; Url = "http://localhost:$webuiPort"; Container = "yuyinods-webui"; OpenUrl = "http://localhost:$webuiPort" }
    @{ Name = $llmEndpoint.Name; Url = $llmEndpoint.HealthUrl; Container = $llmContainer; OpenUrl = $llmEndpoint.BaseUrl }
    @{ Name = "Dashboard API"; Url = "http://localhost:$dashboardApiPort/health"; Container = "yuyinods-dashboard-api"; OpenUrl = "http://localhost:$dashboardApiPort" }
)
if (Test-ODSWindowsServiceEnabled -ServiceId "litellm" -Plan $servicePlan) {
    ...
    $readinessChecks += @{ Name = "LiteLLM"; Url = "http://localhost:$litellmPort/health/readiness"; Container = "yuyinods-litellm"; OpenUrl = "http://localhost:$litellmPort" }
}
if (Test-ODSWindowsServiceEnabled -ServiceId "searxng" -Plan $servicePlan) {
    ...
    $readinessChecks += @{ Name = "SearXNG"; Url = "http://localhost:$searxngPort/healthz"; Container = "yuyinods-searxng"; OpenUrl = "http://localhost:$searxngPort" }
}
if (Test-ODSWindowsServiceEnabled -ServiceId "token-spy" -Plan $servicePlan) {
    $tokenSpyPort = Get-ReadinessPort -Name "TOKEN_SPY_PORT" -Default "3005"
    $readinessChecks += @{ Name = "Token Spy"; Url = "http://localhost:$tokenSpyPort/health"; Container = "yuyinods-token-spy"; OpenUrl = "http://localhost:$tokenSpyPort" }
}
if ($enableVoice) {
    $whisperPort = Get-ReadinessPort -Name "WHISPER_PORT" -Default "9000"
    $ttsPort = Get-ReadinessPort -Name "TTS_PORT" -Default "8880"
    $readinessChecks += @{ Name = "Whisper (STT)"; Url = "http://localhost:$whisperPort/health"; Container = "yuyinods-whisper"; OpenUrl = "http://localhost:$whisperPort" }
    if ($sttModelCacheUrl) {
        $readinessChecks += @{ Name = "Whisper STT model cache"; Url = $sttModelCacheUrl; Container = "yuyinods-whisper"; OpenUrl = $sttModelNameForReadiness; Hint = "Run: $sttRecoveryCmd" }
    }
    $readinessChecks += @{ Name = "Kokoro (TTS)"; Url = "http://localhost:$ttsPort/health"; Container = "yuyinods-tts"; OpenUrl = "http://localhost:$ttsPort" }
}
if ($enableWorkflows) {
    $n8nPort = Get-ReadinessPort -Name "N8N_PORT" -Default "5678"
    $readinessChecks += @{ Name = "n8n"; Url = "http://localhost:$n8nPort/healthz"; Container = "yuyinods-n8n"; OpenUrl = "http://localhost:$n8nPort" }
}
if ($enableRag) {
    $qdrantPort = Get-ReadinessPort -Name "QDRANT_PORT" -Default "6333"
    $readinessChecks += @{ Name = "Qdrant"; Url = "http://localhost:$qdrantPort"; Container = "yuyinods-qdrant"; OpenUrl = "http://localhost:$qdrantPort" }
}
if ($enableOpenClaw) {
    $openClawPort = Get-ReadinessPort -Name "OPENCLAW_PORT" -Default "7860"
    $readinessChecks += @{ Name = "OpenClaw"; Url = "http://localhost:$openClawPort"; Container = "yuyinods-openclaw"; OpenUrl = "http://localhost:$openClawPort" }
}
if (Test-ODSWindowsServiceEnabled -ServiceId "hermes-proxy" -Plan $servicePlan) {
    ...
    $readinessChecks += @{ Name = "Hermes Proxy"; Url = "http://localhost:$hermesProxyPort/health"; Container = "yuyinods-hermes-proxy"; OpenUrl = "http://localhost:$hermesProxyPort" }
}
if ($enableComfyui) {
    $comfyPort = Get-ReadinessPort -Name "COMFYUI_PORT" -Default "8188"
    $readinessChecks += @{ Name = "ComfyUI"; Url = "http://localhost:$comfyPort"; Container = "yuyinods-comfyui"; OpenUrl = "http://localhost:$comfyPort" }
}
if (Test-ODSWindowsServiceEnabled -ServiceId "perplexica" -Plan $servicePlan) {
    $perplexicaPort = Get-ReadinessPort -Name "PERPLEXICA_PORT" -Default "3004"
    $readinessChecks += @{ Name = "Perplexica"; Url = "http://localhost:$perplexicaPort"; Container = "yuyinods-perplexica"; OpenUrl = "http://localhost:$perplexicaPort" }
}
if (Test-ODSWindowsServiceEnabled -ServiceId "privacy-shield" -Plan $servicePlan) {
    $privacyPort = Get-ReadinessPort -Name "SHIELD_PORT" -Default "8085"
    $readinessChecks += @{ Name = "Privacy Shield"; Url = "http://localhost:$privacyPort/health"; Container = "yuyinods-privacy-shield"; OpenUrl = "http://localhost:$privacyPort" }
}
$installReadiness = Write-ODSInstallReadinessSummary -Checks $readinessChecks `
    -StatusCommand ".\yuyinods.ps1 status" `
...
if ($allHealthy) {
    ...
    Write-Host "  .\yuyinods.ps1 status" -ForegroundColor Cyan
...
if ($SummaryJsonPath) {
    $summary = @{
        version    = $script:YUYINODS_VERSION
        tier       = $selectedTier
