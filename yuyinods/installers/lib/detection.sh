...
amd_gpu_missing_runtime_devices() {
    local root="${YUYINODS_AMD_DEVICE_ROOT:-/dev}"
    local kfd="$root/kfd"
    local dri="$root/dri"
    local missing=()

    if [[ -n "${YUYINODS_AMD_DEVICE_ROOT:-}" ]]; then
        [[ -e "$kfd" ]] || missing+=("$kfd")
    else
        [[ -c "$kfd" ]] || missing+=("$kfd")
    fi
...
apply_cpu_gpu_fallback() {
    ...
    GPU_DEVICE_ID=""
    HAS_NPU=false
    [[ "${YUYINODS_MODE:-local}" == "lemonade" ]] && YUYINODS_MODE="local"
    BACKEND_ID="cpu"
    CAP_LLM_BACKEND="cpu"
    CAP_GPU_VENDOR="cpu"
...
detect_gpu() {
    ...
    # discrete-NVIDIA branch below. Test hooks: YUYINODS_NV_TEGRA_RELEASE,
    # YUYINODS_DEVICE_TREE_COMPATIBLE, YUYINODS_DEVICE_TREE_MODEL, YUYINODS_GPU0_SYSFS,
    # YUYINODS_UNAME_M.
    local _tegra_release="${YUYINODS_NV_TEGRA_RELEASE:-/etc/nv_tegra_release}"
    local _dt_compat="${YUYINODS_DEVICE_TREE_COMPATIBLE:-/proc/device-tree/compatible}"
    local _dt_model="${YUYINODS_DEVICE_TREE_MODEL:-/proc/device-tree/model}"
    local _gpu0_sysfs="${YUYINODS_GPU0_SYSFS:-/sys/devices/gpu.0}"
    local _uname_m="${YUYINODS_UNAME_M:-$(uname -m)}"
    local _is_jetson=false
    if [[ "${YUYINODS_ENABLE_EXPERIMENTAL_JETSON:-0}" == "1" && "$_uname_m" == "aarch64" ]]; then
    ...
    # YUYINODS_DRM_SYS can be overridden in tests to point at a mock sysfs tree.
    local _drm_sys="${YUYINODS_DRM_SYS:-/sys/class/drm}"
...
fix_nvidia_secure_boot() {
    ...
    local svc_name="yuyinods-install-resume"
