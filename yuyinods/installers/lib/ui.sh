#!/bin/bash
# ============================================================================
# YuyinODS Installer — UI (CRT Theme)
# ============================================================================
...
show_phase() {
  ...
  echo ""
  bootline
  echo -e "${BGRN}YUYINODS SEQUENCE [${ts}]${NC}  ${GRN}PHASE ${phase}/${total} — ${name}${NC}"
...
show_stranger_boot() {
  ...
  echo ""
  static_line
  echo -e "${BGRN}  Y U Y I N O D S${NC}   ${GRN}Local AI // Sovereign Intelligence // $(date +%Y)${NC}"
...
show_install_menu() {
    ...
    case "$INSTALL_CHOICE" in
        1)
            ...
            # Full Stack means all features. Low-tier systems may run ComfyUI
            # slowly or fail at runtime, but do not silently disable it unless
            # the operator explicitly opts into automatic pruning.
            case "${TIER:-}" in
                0|1)
                    if [[ "${YUYINODS_AUTO_DISABLE_COMFYUI:-false}" == "true" ]]; then
                        ENABLE_COMFYUI=false
                        log "ComfyUI auto-disabled for Tier $TIER (YUYINODS_AUTO_DISABLE_COMFYUI=true)"
                        ai_warn "Image generation (ComfyUI) disabled by YUYINODS_AUTO_DISABLE_COMFYUI."
                    else
                        log "ComfyUI enabled on Tier $TIER by Full Stack selection"
                        ai_warn "Image generation enabled on low-tier hardware; ComfyUI may be slow or need more memory."
                    fi
                    ;;
            esac
            ;;
...
show_success_card() {
    ...
    static_line
    echo ""
    type_line_dramatic "YUYINODS INSTALLATION COMPLETE." "$BGRN"
    echo ""
    echo -e "${GRN}+--------------------------------------------------------------+${NC}"
