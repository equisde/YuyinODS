#!/bin/bash
# YuyinODS macOS installer (doctor/preflight MVP).

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPORT_FILE="${PREFLIGHT_REPORT_FILE:-/tmp/yuyinods-preflight-macos.json}"
DOCTOR_FILE="${DOCTOR_REPORT_FILE:-/tmp/yuyinods-doctor-macos.json}"
NO_DELEGATE=false
...
echo "YuyinODS macOS installer (MVP)"
echo ""
...
if [[ -x "$SCRIPT_DIR/scripts/yuyinods-doctor.sh" ]]; then
    "$SCRIPT_DIR/scripts/yuyinods-doctor.sh" "$DOCTOR_FILE" >/dev/null 2>&1 || true
