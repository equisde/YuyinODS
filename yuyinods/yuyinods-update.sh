#!/bin/bash
# yuyinods-update.sh - ODS Update Manager
#
# Commands:
...
resolve_compose_flags() {
    ...
    ods_mode="$(env_file_value YUYINODS_MODE)"
    ...
    if [[ -x "${INSTALL_DIR}/scripts/resolve-compose-stack.sh" ]]; then
        resolved=$(bash "${INSTALL_DIR}/scripts/resolve-compose-stack.sh" \
            ...
            --yuyinods-mode "${ods_mode:-local}" | tail -1) || resolved=""
...
is_git_checkout() {
    ...
    git -C "${top}" ls-files --error-unmatch "${prefix}yuyinods-update.sh" >/dev/null 2>&1
}

runtime_update_guidance() {
    log_info "For routine runtime/image updates, run: cd \"${INSTALL_DIR}\" && ./yuyinods-cli update"
...
ensure_source_checkout_for_update() {
    if ! command -v git >/dev/null 2>&1; then
        log_error "git is required for yuyinods-update.sh source-code updates."
        runtime_update_guidance
        return 1
    fi

    if ! is_git_checkout; then
        log_error "yuyinods-update.sh update only works from a git-backed ODS source checkout."
...
wait_for_healthy() {
    ...
    local delay=10
    local health_log
    health_log=$(mktemp /tmp/yuyinods-health-XXXXXX.log)
...
_update_rollback() {
    ...
    log_warn "Rollback complete. Run 'yuyinods-update.sh health' to verify."
}
...
cmd_check() {
    ...
    case $cmp_result in
        ...
        2)
            ...
            echo ""
            echo "Run 'ods update' or './yuyinods-cli update' for normal runtime updates."
            if is_git_checkout; then
                echo "Source checkout detected: run 'yuyinods-update.sh update' only when you intend to pull source code."
            fi
            ;;
    esac
...
Usage: yuyinods-update.sh <command> [options]
...
Examples:
  yuyinods-update.sh check
  yuyinods-update.sh status
  yuyinods-update.sh backup pre-experiment
  ods update                    # normal runtime/image update
  yuyinods-update.sh update          # source checkout only
  yuyinods-update.sh rollback
  yuyinods-update.sh rollback 20260317-120000
  yuyinods-update.sh changelog v1.1.0
  yuyinods-update.sh health
