...
interrupt_handler() {
    ...
    if (( now - LAST_SIGINT <= 3 )); then
        ...
        echo -e "\033[0;32m    Log file: ${LOG_FILE:-/tmp/yuyinods-install.log}\033[0m"
        exit 130
    fi
    LAST_SIGINT=$now
    echo ""
...
if [[ "$(uname -s 2>/dev/null || true)" == "Linux" ]]; then
    export YUYINODS_PYTHON_PREFER_SYSTEM="${YUYINODS_PYTHON_PREFER_SYSTEM:-1}"
fi
...
INTERACTIVE=true
YUYINODS_MODE="${YUYINODS_MODE:-local}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --tier) TIER="$2"; shift 2 ;;
        --cloud) YUYINODS_MODE="cloud"; shift ;;
        --use-existing-lemonade) LEMONADE_EXTERNAL=true; YUYINODS_MODE="lemonade"; shift ;;
        --lemonade-url) LEMONADE_EXTERNAL=true; YUYINODS_MODE="lemonade"; LEMONADE_BASE_URL="$2"; shift 2 ;;
        ...
        # Without yuyinods-proxy on host :80, mDNS publishes the hostname but
