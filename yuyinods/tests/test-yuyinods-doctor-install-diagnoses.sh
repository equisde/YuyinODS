...
cwd=/tmp/not-the-yuyinods-install-dir
...
if (cd "$ROOT_DIR" && bash scripts/yuyinods-doctor.sh "$REPORT" >/dev/null 2>&1); then
    pass "yuyinods-doctor runs with fixture install artifacts"
else
    fail "yuyinods-doctor failed with fixture install artifacts"
fi
...
if jq -e '.diagnoses[] | select(.id == "YuyinODS-DOCKER-IMAGE-UNRESOLVED") | .evidence[0].source == "install-report-2999-01-01-000000.txt"' "$REPORT" >/dev/null; then
    pass "image diagnosis records install report evidence"
else
    fail "image diagnosis missing install report evidence"
fi

if jq -e '.diagnoses[] | select(.id == "YuyinODS-WINDOWS-FILE-SHARING-PROBE-IMAGE") | .evidence[0].source == "logs/install.log"' "$REPORT" >/dev/null; then
...
case "${1:-}" in
    ...
    ps)
        if [[ "$*" == *"--format"* ]]; then
            printf '%s\n' yuyinods-dashboard yuyinods-webui yuyinods-llama-server
        fi
        exit 0
        ;;
    inspect)
...
if (cd "$ROOT_DIR" && PATH="$FAKE_BIN:$PATH" bash scripts/yuyinods-doctor.sh "$REPORT" >/dev/null 2>&1); then
    pass "yuyinods-doctor runs with recovered stale zero-container artifact"
else
    fail "yuyinods-doctor failed with recovered stale zero-container artifact"
fi

if jq -e '.diagnoses[] | select(.id == "YuyinODS-COMPOSE-ZERO-CONTAINERS")' "$REPORT" >/dev/null; then
...
LITELLM_LEMONADE_API_KEY=sk-yuyinods-lemonade-fixture
...
if (cd "$ROOT_DIR" && bash scripts/yuyinods-doctor.sh "$REPORT" >/dev/null 2>&1); then
    pass "yuyinods-doctor runs with external Lemonade fixture"
else
    fail "yuyinods-doctor failed with external Lemonade fixture"
fi

if jq -e '.diagnoses[] | select(.id == "YuyinODS-RUNTIME-EXTERNAL-LEMONADE-UNAUTHENTICATED-HOST-ROUTE")' "$REPORT" >/dev/null; then
...
if (cd "$ROOT_DIR" && bash scripts/yuyinods-doctor.sh "$REPORT" >/dev/null 2>&1); then
    pass "yuyinods-doctor runs with authenticated external Lemonade fixture"
else
    fail "yuyinods-doctor failed with authenticated external Lemonade fixture"
fi
