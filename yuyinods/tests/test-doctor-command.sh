...
if grep -q "^cmd_doctor()" "$ROOT_DIR/yuyinods-cli"; then
...
if grep -Eq "doctor|diag|d\)" "$ROOT_DIR/yuyinods-cli"; then
...
if grep -q "json_mode" "$ROOT_DIR/yuyinods-cli"; then
...
if grep -q "python3.*report_file" "$ROOT_DIR/yuyinods-cli"; then
...
if grep -q "return \$?" "$ROOT_DIR/yuyinods-cli"; then
...
if grep -q "Runtime Environment" "$ROOT_DIR/yuyinods-cli"; then
...
if grep -q "AMD runtime" "$ROOT_DIR/yuyinods-cli"; then
...
if grep -q "Preflight Checks" "$ROOT_DIR/yuyinods-cli"; then
...
if grep -q "Suggested Fixes" "$ROOT_DIR/yuyinods-cli"; then
...
if grep -q "Diagnoses" "$ROOT_DIR/yuyinods-cli"; then
...
if grep -q "doctor.*diagnostics" "$ROOT_DIR/yuyinods-cli"; then
    pass "help text updated"
else
    fail "help not updated"
fi

# Test 11: yuyinods-doctor.sh exists
...
if bash -n "$ROOT_DIR/yuyinods-cli" 2>/dev/null; then
...
if grep -q "report_file=" "$ROOT_DIR/yuyinods-cli"; then
