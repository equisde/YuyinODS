...
#
# Usage:
#   bash yuyinods/tests/reproducers/langfuse-uid-check.sh
#
# Exit codes:
...
PLATFORM="$(uname -s)"
TMPDIR_REPRO="/tmp/yuyinods-3E-repro-$$"
...
docker run --rm \
    ...
    --name "yuyinods-3E-repro-phase1-$$" \
    "$POSTGRES_IMAGE" \
    postgres -c 'shared_buffers=16MB' >/tmp/yuyinods-3E-repro-phase1.log 2>&1 &
P1_PID=$!
...
if kill -0 "$P1_PID" 2>/dev/null; then
    ...
    docker stop "yuyinods-3E-repro-phase1-$$" >/dev/null 2>&1 || true
    ...
    if grep -qi "permission denied\|could not\|cannot create" /tmp/yuyinods-3E-repro-phase1.log; then
...
docker run --rm \
    -e POSTGRES_PASSWORD=reproducer \
    -v "$TMPDIR_REPRO":/var/lib/postgresql/data \
    --name "yuyinods-3E-repro-phase2-$$" \
    "$POSTGRES_IMAGE" \
    postgres -c 'shared_buffers=16MB' >/tmp/yuyinods-3E-repro-phase2.log 2>&1 &
P2_PID=$!
sleep 10
if kill -0 "$P2_PID" 2>/dev/null; then
    log "phase 2: container still running after 10s (initdb succeeded)"
    docker stop "yuyinods-3E-repro-phase2-$$" >/dev/null 2>&1 || true
...
case "$PLATFORM" in
    Linux)
        ...
        echo "  See /tmp/yuyinods-3E-repro-phase1.log and /tmp/yuyinods-3E-repro-phase2.log"
        exit 1
        ;;
    Darwin)
