...
YUYINODS_DIR="$SRC" bash "$YUYINODS_BACKUP" --type full >/dev/null 2>&1 || fail "Backup failed"
...
YUYINODS_DIR="$DST" bash "$YUYINODS_RESTORE" -f "$BACKUP_ID" >/dev/null 2>&1 || fail "Restore failed"
