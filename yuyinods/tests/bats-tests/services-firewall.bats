...
setup() {
    ...
    export YUYINODS_AGENT_PORT=7710
...
@test "services firewall: UFW rule is scoped to detected yuyinods-network subnet" {
    ...
    assert_success
    assert_output --partial "ufw allow from 10.89.0.0/24 to any port 7710 proto tcp comment yuyinods-host-agent"
    refute_output --partial "172.16.0.0/12"
}

@test "services firewall: firewalld rich rule is scoped to detected yuyinods-network subnet" {
    helper="$BATS_TEST_TMPDIR/helper.sh"
    extract_firewall_helper "$helper"
...
@test "services firewall: explicit non-wildcard YUYINODS_AGENT_BIND skips auto rule" {
    helper="$BATS_TEST_TMPDIR/helper.sh"
    extract_firewall_helper "$helper"
    ...
    run bash -c '
        ...
        ai_warn() { echo "WARN: $1"; }
        export SYSTEMCTL_ACTIVE_UNITS="ufw"
        export YUYINODS_AGENT_BIND="127.0.0.1"
        _phase11_allow_host_agent_firewall yuyinods-network
        cat "$COMMAND_LOG"
    '
...
@test "services firewall: external Lemonade UFW rule is scoped to detected yuyinods-network subnet" {
    helper="$BATS_TEST_TMPDIR/helper.sh"
    extract_firewall_helper "$helper"
    ...
    assert_success
    assert_output --partial "ufw allow from 10.91.0.0/24 to any port 13305 proto tcp comment yuyinods-external-lemonade"
    refute_output --partial "172.16.0.0/12"
}
