...
setup() {
    ...
    export SCRIPT_DIR="$BATS_TEST_TMPDIR/yuyinods"
...
MOCK
    ...
    export YUYINODS_DRM_SYS="$BATS_TEST_TMPDIR/sys/class/drm"
...
@test "amd_gpu_runtime_devices_available: accepts mocked AMD device nodes" {
    export YUYINODS_AMD_DEVICE_ROOT="$BATS_TEST_TMPDIR/dev"
    mkdir -p "$YUYINODS_AMD_DEVICE_ROOT/dri"
    touch "$YUYINODS_AMD_DEVICE_ROOT/kfd"
    touch "$YUYINODS_AMD_DEVICE_ROOT/dri/renderD128"
...
@test "amd_gpu_runtime_devices_available: reports missing AMD device nodes" {
    ...
    assert_success
    assert_output --partial "$YUYINODS_AMD_DEVICE_ROOT/kfd"
    assert_output --partial "$YUYINODS_AMD_DEVICE_ROOT/dri/renderD*"
}
