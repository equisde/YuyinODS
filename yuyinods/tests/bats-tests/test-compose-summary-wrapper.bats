#!/usr/bin/env bats
# ============================================================================
# BATS tests for _compose_run_with_summary in yuyinods-cli.
# ============================================================================
...
#     log_error) text from yuyinods-cli.
...
setup() {
    ...
    # Minimal logger helpers that match yuyinods-cli's line 45–50 definitions.
    # We redefine rather than extract because (a) they're trivial and (b)
    # yuyinods-cli's real definitions pull in color vars we've already stubbed.
    log()       { echo "[ods] $1"; }
    success()   { echo "✓ $1"; }
    warn()      { echo "⚠ $1"; }
    log_error() { echo "✗ $1" >&2; }
    ...
    # Extract _compose_run_with_summary from yuyinods-cli.
    local _cli="$BATS_TEST_DIRNAME/../../yuyinods-cli"
...
case "${DOCKER_STUB_MODE:-success}" in
    success)
        echo "Network yuyinods-net created"
        echo "Container yuyinods-foo started"
        exit 0
        ;;
    fail-keyword)
        echo "Container yuyinods-foo starting"
        echo "Error response from daemon: service yuyinods-foo failed to start"
        echo "unhealthy container yuyinods-bar"
        echo "dependency yuyinods-baz not ready"
        exit 1
        ;;
    fail-nomatch)
        echo "Container yuyinods-foo starting"
    ...
    fail-startup-once)
        ...
        echo "$count" > "$state_file"
        if [[ "$count" -eq 1 ]]; then
            echo "dependency failed to start: container yuyinods-llama-server exited (137)"
            exit 1
        fi
        echo "Container yuyinods-llama-server started"
        exit 0
        ;;
    fail-startup-twice)
        state_file="$TMPDIR_TEST/startup-retry-count"
    ...
    fail-startup-twice-then-name-conflict)
        ...
        if [[ "$count" -eq 3 ]]; then
            echo "Error response from daemon: Conflict. The container name \"/yuyinods-llama-server\" is already in use by container \"d287436ffd1902846f8d81593e4a29c92761457cb57167290e9d7f039d318733\". You have to remove (or rename) that container to be able to reuse that name."
            exit 1
        fi
        echo "Container yuyinods-llama-server started"
        exit 0
...
STUB
    ...
    export YUYINODS_COMPOSE_STARTUP_RETRY_DELAY=0
    : > "$DOCKER_CALL_LOG"
}
...
# grep the pipeline's final exit code. yuyinods-cli line 6 IS `set -euo pipefail`
...
@test "wrapper: failure with no keyword match fires the warn fallback (under pipefail)" {
    ...
    run bash -c '
        ...
        log_error() { echo "✗ $1" >&2; }
        _check_docker_access() { :; }
        eval "$(awk "/^_compose_run_with_summary\(\) \{/,/^\}$/" "'"$BATS_TEST_DIRNAME/../../yuyinods-cli"'")"
...
@test "wrapper: compose up retries once on transient port allocation race" {
    ...
    export YUYINODS_COMPOSE_PORT_RETRY_DELAY=0
    run _compose_run_with_summary "Restarting all services" up -d
    assert_success
...
@test "wrapper: compose up retries repeated transient dependency startup failures" {
    ...
    export YUYINODS_COMPOSE_STARTUP_RETRY_ATTEMPTS=3
    run _compose_run_with_summary "Restarting all services" up -d
    assert_success
...
@test "wrapper: compose up cleans stale ODS name conflict after startup retries" {
    ...
    export YUYINODS_COMPOSE_STARTUP_RETRY_ATTEMPTS=3
    export YUYINODS_COMPOSE_NAME_CONFLICT_RETRY_DELAY=0
    run _compose_run_with_summary "Restarting all services" up -d
    assert_success
    assert_output --partial "retrying (1/2)"
    assert_output --partial "retrying (2/2)"
