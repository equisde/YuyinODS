#!/usr/bin/env bats
# ============================================================================
# BATS tests for yuyinods-cli's shell-flag hygiene.
# ============================================================================
...
setup() {
    export CLI="$BATS_TEST_DIRNAME/../../yuyinods-cli"
}
...
@test "flags: no weaker shell-mode line exists anywhere in yuyinods-cli" {
...
@test "flags: no bare '| head -1' pipelines in yuyinods-cli" {
...
# ── end-to-end: yuyinods-cli doesn't crash on nounset with minimal env ─────────

@test "flags: --version runs under minimal env without unbound-var crash" {
    # Strip the environment down to the bare minimum. If yuyinods-cli references
...
@test "flags: yuyinods-cli passes bash -n (syntax check)" {
    run bash -n "$CLI"
    assert_success
}
