...
_run_resolver() {
    run env YUYINODS_MODE="${YUYINODS_MODE:-local}" \
...
@test "user-ext (DRAFT, needs #1051): compose.local.yaml is included in local mode on nvidia" {
    ...
    YUYINODS_MODE=local _run_resolver --gpu-backend nvidia --tier 1
    assert_success
...
@test "user-ext (DRAFT, needs #1051): compose.local.yaml is excluded on apple backend (deadlock guard)" {
    ...
    YUYINODS_MODE=local _run_resolver --gpu-backend apple --tier AP_BASE
    assert_success
