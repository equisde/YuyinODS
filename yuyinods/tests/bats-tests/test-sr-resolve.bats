...
# ============================================================================
# Guards against re-breakage of the `yuyinods-<id>` prefix-strip added in
...
#   3. `yuyinods-<id>` prefix    →  strips prefix, resolves to ID
#   4. `yuyinods-<alias>` prefix →  strips prefix, resolves through alias
#   5. Unknown `yuyinods-*`      →  passes through as-is (not our container)
#   6. Unknown non-ods      →  passes through as-is (best-effort)
#   7. Empty input            →  passes through empty
#   8. Container that happens to start with `yuyinods-` but isn't our
...
@test "sr_resolve: case 3 — yuyinods-<id> prefix strips and resolves" {
    # This is the PR-10 / #406 regression point: users copy container names
    # from `docker ps` (e.g. `yuyinods-alpha`) and expect `ods restart` to
    # accept them.
    run _sr "yuyinods-alpha"
    assert_success
    assert_output "alpha"
}

@test "sr_resolve: case 4 — yuyinods-<alias> prefix strips and resolves through alias" {
    run _sr "yuyinods-a"
    assert_success
    assert_output "alpha"
...
@test "sr_resolve: case 5 — unknown yuyinods-* passes through as-is" {
    # Container that starts with `yuyinods-` but whose stripped form isn't a
    # known alias: return the input verbatim (best-effort; compose will
    # fail later with a clear error).
    run _sr "yuyinods-unknown-service"
    assert_success
    assert_output "yuyinods-unknown-service"
}
...
@test "sr_resolve: case 8 — yuyinods-* where strip doesn't match any alias passes through" {
    # Container name-like string that isn't from our extensions and whose
    # stripped form doesn't collide with any known alias.
    run _sr "yuyinods-some-other-project-container"
    assert_success
    assert_output "yuyinods-some-other-project-container"
}

# ── additional correctness guards ───────────────────────────────────────────

@test "sr_resolve: does NOT strip yuyinods- when the full input is already a known alias" {
    ...
    # resolves even though `yuyinods-bravo` is its container name.
    run _sr "bravo"
    assert_success
    assert_output "bravo"
}
