...
# ==============================================================═
# C6. yuyinods-cli status (replaced status.sh)
echo -e "${CYAN}-- C6. yuyinods-cli status command ------------------------------"

YUYINODS_CLI="${SCRIPT_DIR}/../yuyinods-cli"
if [ -f "$YUYINODS_CLI" ]; then
    if grep -q "cmd_status" "$YUYINODS_CLI" 2>/dev/null; then
        log_pass "yuyinods-cli has cmd_status function"
    else
        log_fail "yuyinods-cli missing cmd_status function"
    fi
else
    log_warn "yuyinods-cli not found"
fi

# ==============================================================═
# C7. yuyinods-cli TTS service alias
echo -e "${CYAN}-- C7. yuyinods-cli TTS Service Alias --------------------------"

YUYINODS_CLI="${SCRIPT_DIR}/../yuyinods-cli"
if [ -f "$YUYINODS_CLI" ]; then
    if grep -qE "piper.*yuyinods-piper|tts.*yuyinods-piper" "$YUYINODS_CLI" 2>/dev/null; then
        log_fail "yuyinods-cli maps TTS to wrong container (yuyinods-piper instead of yuyinods-tts)"
    else
        log_pass "yuyinods-cli TTS mapping appears correct"
    fi
else
    log_warn "yuyinods-cli not found"
fi
...
# ==============================================================═
# C9. yuyinods-update.sh GitHub repo
echo -e "${CYAN}-- C9. yuyinods-update.sh GitHub Repo --------------------------"

UPDATE_SCRIPT="${SCRIPT_DIR}/../yuyinods-update.sh"
if [ -f "$UPDATE_SCRIPT" ]; then
    if grep -q "GITHUB_REPO.*equisde/YuyinODS" "$UPDATE_SCRIPT" 2>/dev/null; then
        log_pass "yuyinods-update.sh GitHub repo configuration is correct (YuyinODS)"
    else
        log_fail "yuyinods-update.sh missing correct GitHub repo (should be equisde/YuyinODS)"
    fi
else
    log_warn "yuyinods-update.sh not found"
fi
