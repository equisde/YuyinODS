...
# fix/yuyinods-cli-apple-silicon-coverage to:
...
# yuyinods-cli requires Bash 4+. `/usr/bin/env bash` picks the first bash on
...
if ! env bash -c '(( BASH_VERSINFO[0] >= 4 ))' 2>/dev/null; then
    ...
    echo "[SKIP] yuyinods-cli requires Bash 4+ on PATH (env bash -> $_probe)."
    echo "       Install via 'brew install bash' or run with PATH including bash 4+."
    exit 0
fi

[[ -x "$YUYINODS_CLI" ]] || { fail "yuyinods-cli not executable at $YUYINODS_CLI"; exit 1; }
command -v jq >/dev/null 2>&1 || { fail "jq is required"; exit 1; }

# ----------------------------------------------------------------------------
# Fixture: fake YUYINODS_HOME satisfying check_install + a stub PATH
# ----------------------------------------------------------------------------
...
# check_install only requires YUYINODS_HOME dir to exist with docker-compose.base.yml
...
# Run yuyinods-cli under the fixture. Returns captured stdout+stderr via $OUT,
# rc via $RC (using globals to keep set -e friendly).
run_ods_cli() {
    set +e
    OUT=$(YUYINODS_HOME="$FAKE_INSTALL" PATH="$STUB_PATH" "$YUYINODS_CLI" "$@" 2>&1)
    RC=$?
    set -e
}
