...
def test_compose_uses_ods_entrypoint() -> None:
    ...
    assert "/app/yuyinods-entrypoint.sh" in compose
    assert "./extensions/services/perplexica/docker-entrypoint.sh:/app/yuyinods-entrypoint.sh:ro" in compose
    assert 'exec /bin/sh /app/yuyinods-entrypoint.sh \\"$@\\"' in compose


def test_bind_mounted_entrypoints_do_not_require_executable_bit() -> None:
    service_entrypoints = (
        (COMPOSE, "/app/yuyinods-entrypoint.sh", 'exec /bin/sh /app/yuyinods-entrypoint.sh \\"$@\\"'),
