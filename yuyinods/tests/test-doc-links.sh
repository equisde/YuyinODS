...
#
# Run from repo root:  bash yuyinods/tests/test-doc-links.sh
...
# If invoked from yuyinods/ (ROOT_DIR), also check the top-level README.
# If invoked from repo root, ROOT_DIR points to yuyinods/ already.
...
main() {
  ...
  pass "No broken local links found in repo README, ods root docs, and yuyinods/docs"
}
