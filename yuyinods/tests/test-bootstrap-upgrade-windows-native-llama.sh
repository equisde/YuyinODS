...
#!/usr/bin/env bash
set -euo pipefail
: "${YUYINODS_WIN_PID_FILE:?}"
: "${YUYINODS_WIN_LLAMA_EXE:?}"
: "${YUYINODS_WIN_MODEL_PATH:?}"
: "${YUYINODS_WIN_ROLLBACK_MODEL_PATH:?}"
: "${YUYINODS_WIN_LLAMA_PORT:?}"
: "${YUYINODS_WIN_CTX_SIZE:?}"
: "${YUYINODS_WIN_REASONING_FORMAT:?}"
{
  printf 'exe=%s\n' "$YUYINODS_WIN_LLAMA_EXE"
  printf 'model=%s\n' "$YUYINODS_WIN_MODEL_PATH"
  printf 'rollback=%s\n' "$YUYINODS_WIN_ROLLBACK_MODEL_PATH"
  ...
  printf 'ctx=%s\n' "$YUYINODS_WIN_CTX_SIZE"
  printf 'reasoning=%s\n' "$YUYINODS_WIN_REASONING_FORMAT"
} >> "${YUYINODS_FAKE_PS_TRACE:?}"
mkdir -p "$(dirname "$YUYINODS_WIN_PID_FILE")"
printf '4242\n' > "$YUYINODS_WIN_PID_FILE"
exit 0
EOF_PS
...
if [[ "${1:-}" == "ps" ]]; then
  case " $* " in
    *"name=yuyinods-litellm"*)
      printf 'yuyinods-litellm\n'
      ;;
  esac
  exit 0
fi
if [[ "${1:-}" == "restart" && "${2:-}" == "yuyinods-litellm" ]]; then
  printf 'restart yuyinods-litellm\n' >> "${YUYINODS_FAKE_DOCKER_TRACE:?}"
  exit 0
fi
exit 0
EOF_DOCKER
...
PATH="$fakebin:$PATH" YUYINODS_FAKE_PS_TRACE="$trace" YUYINODS_FAKE_DOCKER_TRACE="$docker_trace" bash "$TARGET" \
    "$install_dir" \
    "Full.gguf" \
...
grep -q 'restart yuyinods-litellm' "$docker_trace" \
