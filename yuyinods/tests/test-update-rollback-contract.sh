...
YUYINODS_BACKUP="$ROOT_DIR/yuyinods-backup.sh"
YUYINODS_RESTORE="$ROOT_DIR/yuyinods-restore.sh"
...
[[ -x "$YUYINODS_BACKUP" ]] || fail "yuyinods-backup.sh is not executable"
[[ -x "$YUYINODS_RESTORE" ]] || fail "yuyinods-restore.sh is not executable"
...
YUYINODS_DIR="$SRC" RETENTION_COUNT=10 bash "$YUYINODS_BACKUP" --type full >/dev/null 2>&1
...
YUYINODS_DIR="$SRC" bash "$YUYINODS_RESTORE" -f "$BACKUP_ID" >/dev/null 2>&1
