...
for path in \
    ...
    "installers/windows/yuyinods.ps1" \
...
require_literal "bin/yuyinods-host-agent.py" '"LLAMA_ARG_SPEC_TYPE": "--spec-type"' "host-agent spec type mapping"
require_literal "bin/yuyinods-host-agent.py" '"LLAMA_ARG_SPEC_DRAFT_N_MAX": "--spec-draft-n-max"' "host-agent spec n max mapping"
...
require_literal "installers/windows/yuyinods.ps1" '--spec-type", $envVars["LLAMA_ARG_SPEC_TYPE"]' "Windows CLI native spec type"
require_literal "installers/macos/yuyinods-macos.sh" '--spec-type "$ENV_LLAMA_ARG_SPEC_TYPE"' "macOS CLI native spec type"
