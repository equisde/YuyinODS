...
suffix="$(date +%s)-$$"
work_dir="$(mktemp -d "${TMPDIR:-/tmp}/yuyinods-external-lemonade-e2e.XXXXXX")"
runtime_dir="$work_dir/yuyinods"
mock_container="yuyinods-lemonade-mock-$suffix"
litellm_container="yuyinods-litellm-e2e-$suffix"
compose_project="yuyinods-external-lemonade-e2e-$suffix"
litellm_override_file="e2e-litellm-container.override.yml"
ods_network="yuyinods-network"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path in {"/api/v1/health", "/health"}:
            ...
            self._send(200, {"object": "list", "data": [{"id": MODEL, "object": "model", "owned_by": "yuyinods-e2e"}]})
        else:
...
YUYINODS_AGENT_KEY=e2e-agent-key
YUYINODS_SESSION_SECRET=e2e-session-secret
ENV
chmod 600 .env
...
flags="$(LEMONADE_EXTERNAL=true YUYINODS_MODE=lemonade AMD_INFERENCE_RUNTIME=lemonade AMD_INFERENCE_MANAGED=false \
    YUYINODS_PYTHON_CMD="$PY" ./scripts/resolve-compose-stack.sh \
