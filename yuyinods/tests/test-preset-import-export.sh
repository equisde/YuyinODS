...
test_export_in_help() {
    ...
    if grep -q "preset export" "$YUYINODS_CLI" 2>/dev/null; then
...
test_import_in_help() {
    ...
    if grep -q "preset import" "$YUYINODS_CLI" 2>/dev/null; then
...
test_export_case() {
    ...
    if grep -A2 "export|e)" "$YUYINODS_CLI" 2>/dev/null | grep -q "preset export"; then
...
test_import_case() {
    ...
    if grep -A2 "import|i)" "$YUYINODS_CLI" 2>/dev/null | grep -q "preset import"; then
...
test_export_uses_tar() {
    ...
    if grep -A20 "export|e)" "$YUYINODS_CLI" 2>/dev/null | grep -q "tar czf"; then
...
test_import_security() {
    ...
    if grep -A30 "import|i)" "$YUYINODS_CLI" 2>/dev/null | grep -q "path traversal"; then
...
test_export_validation() {
    ...
    if grep -A10 "export|e)" "$YUYINODS_CLI" 2>/dev/null | grep -q "Preset not found"; then
...
test_import_validation() {
    ...
    if grep -A50 "import|i)" "$YUYINODS_CLI" 2>/dev/null | grep -q "meta.txt"; then
...
test_export_relative_paths() {
    ...
    if grep -A15 "export|e)" "$YUYINODS_CLI" 2>/dev/null | grep -q "cd.*PRESETS_DIR"; then
...
test_import_overwrite() {
    ...
    if grep -A30 "import|i)" "$YUYINODS_CLI" 2>/dev/null | grep -q "already exists"; then
...
test_usage_updated() {
    ...
    if grep "preset.*export.*import" "$YUYINODS_CLI" 2>/dev/null; then
