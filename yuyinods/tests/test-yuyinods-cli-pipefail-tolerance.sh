#!/usr/bin/env bash
# Regression coverage for yuyinods-cli paths made stricter by shell strict mode.

set -euo pipefail

# yuyinods-cli requires Bash 4+; macOS ships Bash 3.2, so running this suite
...
# meaningful yuyinods-cli behavior to test on this host.
...
run_ods() {
    ...
    # pattern as test-validate-env.sh and test-yuyinods-config-secret-mask.sh.
    local output rc
    set +e
    output=$(YUYINODS_HOME="$INSTALL_DIR" NO_COLOR=1 "$BASH" "$YUYINODS_CLI" "$@" 2>&1)
    rc=$?
    set -e
    printf '%s\n' "$rc"
    printf '%s\n' "$output"
...
if grep -Eq '^set -euo pipefail|^set -eo pipefail' "$YUYINODS_CLI"; then
    pass "yuyinods-cli enables pipefail"
else
    fail "yuyinods-cli does not enable pipefail"
fi

if grep -q '^set -euo pipefail' "$YUYINODS_CLI"; then
    pass "yuyinods-cli enables nounset"
else
    fail "yuyinods-cli does not enable nounset"
fi
