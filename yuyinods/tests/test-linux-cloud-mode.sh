...
flags="$(YUYINODS_PYTHON_CMD="$PY" ./scripts/resolve-compose-stack.sh \
    --script-dir "$ROOT_DIR" \
    --tier CLOUD \
    --gpu-backend cpu \
    --gpu-count 0 \
...
lemonade_flags="$(LEMONADE_EXTERNAL=true AMD_INFERENCE_RUNTIME=lemonade AMD_INFERENCE_MANAGED=false YUYINODS_PYTHON_CMD="$PY" ./scripts/resolve-compose-stack.sh \
    --script-dir "$ROOT_DIR" \
    --tier CLOUD \
    --gpu-backend cpu \
    --gpu-count 0 \
...
if grep -Fq -- '--yuyinods-mode "${YUYINODS_MODE:-local}"' installers/lib/compose-select.sh \
    && grep -Fq -- '--yuyinods-mode "${YUYINODS_MODE:-local}"' installers/phases/03-features.sh \
    && grep -Fq -- '--yuyinods-mode "${YUYINODS_MODE:-local}"' installers/phases/11-services.sh \
    && grep -Fq -- '--yuyinods-mode "${YUYINODS_MODE:-local}"' yuyinods-cli; then
    pass "installer and CLI pass ods mode to compose resolver"
else
    fail "all installer/CLI resolver calls must pass --yuyinods-mode"
fi

if grep -q 'YUYINODS_MODE:-local.*cloud' installers/phases/12-health.sh \
