...
- gpu_backends: [all] must be treated as a wildcard by yuyinods-doctor.
...
def test_ods_doctor_all_backend_wildcard() -> None:
    text = (ROOT / "scripts/yuyinods-doctor.sh").read_text(encoding="utf-8")
    if '" $gpu_backends " =~ " all "' not in text:
        fail("yuyinods-doctor must treat gpu_backends: [all] as compatible with every non-Apple backend")
