#!/usr/bin/env bash
# Regression: after launchctl bootstrap succeeds for yuyinods-host-agent, the
...
# Pull the block that runs after `launchctl bootstrap ... YUYINODS_AGENT_PLIST`
...
success_block="$(awk '
    /_agent_bootstrap_err=.*launchctl bootstrap.*YUYINODS_AGENT_PLIST/ { in_block=1 }
    in_block { print }
...
plist_block="$(awk '
    /cat > "\$YUYINODS_AGENT_PLIST"/ { in_block=1 }
    in_block { print }
...
grep -qE 'launchctl kickstart -p "gui/\$\(id -u\)/\$\{YUYINODS_AGENT_PLIST_LABEL\}"' <<<"$success_block" \
...
grep -qE 'curl[^|]*"http://127\.0\.0\.1:\$\{YUYINODS_AGENT_PORT\}/health"' <<<"$success_block" \
    || fail "host-agent bootstrap success block must poll /health on 127.0.0.1:\${YUYINODS_AGENT_PORT} before declaring [OK]"
...
echo "[OK] macOS installer verifies yuyinods-host-agent is responding before declaring success"
