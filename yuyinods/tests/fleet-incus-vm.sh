...
HOST_LOCK=true
LOCK_FILE="${YUYINODS_FLEET_HOST_LOCK:-${DREAM_FLEET_HOST_LOCK:-/tmp/dream-fleet-heavy.lock}}"
LOCK_TIMEOUT="${YUYINODS_FLEET_HOST_LOCK_TIMEOUT_SECONDS:-${DREAM_FLEET_HOST_LOCK_TIMEOUT_SECONDS:-}}"
WORK_DIR=""
...
Options:
  ...
  --vm-prefix NAME          Prefix for disposable VM names (default: yuyinods-vm)
  ...
  --lock-file PATH          Host lock path for coordinating with full fleet runs
                            (default: YUYINODS_FLEET_HOST_LOCK, DREAM_FLEET_HOST_LOCK,
...
WORK_DIR="$(mktemp -d)"
PAYLOAD="$WORK_DIR/yuyinods-src.tgz"
...
reboot_if_kernel_modules_changed() {
    ...
    local marker="/var/tmp/yuyinods-fleet-incus-kernel-reboot-requested"
    if [[ -f "$marker" ]]; then
...
extract_source() {
    rm -rf "$SRC_DIR"
    mkdir -p "$SRC_DIR"
    tar -xzf /tmp/yuyinods-src.tgz -C "$SRC_DIR"
    useradd -m -s /bin/bash odstest >/dev/null 2>&1 || true
    printf 'odstest ALL=(ALL) NOPASSWD:ALL\n' >/etc/sudoers.d/yuyinodstest
    chmod 0440 /etc/sudoers.d/yuyinodstest
...
run_installer_dry_run() {
    ...
    sudo -u odstest -H bash -lc '
        set -Eeuo pipefail
        cd /opt/yuyinods-src
        export INSTALL_DIR="$HOME/yuyinods-test"
...
run_lane() {
    ...
    wait_for_exec "$vm"
    incus file push "$PAYLOAD" "$vm/tmp/yuyinods-src.tgz"
