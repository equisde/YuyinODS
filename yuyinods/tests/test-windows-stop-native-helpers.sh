...
# stopped, so yuyinods.ps1 stop exited before stopping ODSHostAgent. The
# host agent kept yuyinods-host-agent.log open and blocked reinstall cleanup.
# ============================================================================
...
if grep -q 'function Stop-ODSOpenCodeRuntime' "$YUYINODS_PS1" \
    && grep -q '\$script:OPENCODE_PORT' "$YUYINODS_PS1" \
    && grep -q '\$script:OPENCODE_EXE' "$YUYINODS_PS1" \
    && grep -q 'ParentProcessId' "$YUYINODS_PS1"; then
