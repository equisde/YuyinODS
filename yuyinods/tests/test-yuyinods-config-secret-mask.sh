...
# yuyinods-cli requires Bash 4+; macOS ships Bash 3.2, so "$BASH" "$YUYINODS_CLI"
...
# otherwise skip — without Bash 4+ yuyinods-cli cannot run on this host at all.
if [ "${BASH_VERSINFO[0]}" -lt 4 ]; then
    ...
    echo "[SKIP] yuyinods-cli requires Bash 4+; this host only has Bash ${BASH_VERSION} (brew install bash)"
...
if [[ ! -x "$YUYINODS_CLI" ]]; then
    fail "yuyinods-cli not found at $YUYINODS_CLI"
...
YUYINODS_OPERATOR=operator-secret
YUYINODS_VERSION=2.0.0-test
HOST_RAM_GB=32
EOF
...
cat > "$INSTALL_SCAFFOLD/.env.schema.json" <<'EOF'
{
  "properties": {
    ...
    "YUYINODS_OPERATOR": {"type": "string", "secret": true},
    "YUYINODS_VERSION": {"type": "string"},
...
run_ods_config_show() {
    # Invokes yuyinods-cli with a controlled PATH to simulate environments
    # with/without jq + python3. NO_COLOR=1 keeps output ASCII.
    local _path="$1"
    local _label="$2"
    local _output
    _output=$(NO_COLOR=1 PATH="$_path" YUYINODS_HOME="$INSTALL_SCAFFOLD" \
        "$BASH" "$YUYINODS_CLI" config show 2>&1)
    echo "$_output"
}
...
build_pathdir_excluding() {
    ...
    mkdir -p "$_pdir"
    local _bin
    # Tools yuyinods-cli (the section we exercise) actually uses.
...
if grep -q "YUYINODS_OPERATOR=\\*\\*\\*" <<<"$out1" && ! grep -qF "$SECRET_OPERATOR" <<<"$out1"; then
...
if grep -q "YUYINODS_OPERATOR=\\*\\*\\*" <<<"$out2" && ! grep -qF "$SECRET_OPERATOR" <<<"$out2"; then
...
if grep -q "YUYINODS_VERSION=2.0.0-test" <<<"$out1"; then
    pass "non-secret YUYINODS_VERSION shown in clear (no over-mask)"
else
    fail "non-secret YUYINODS_VERSION incorrectly masked or missing"
fi
