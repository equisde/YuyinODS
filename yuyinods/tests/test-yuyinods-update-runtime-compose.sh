...
UPDATE_SCRIPT="$ROOT_DIR/yuyinods-update.sh"
...
[[ -f "$UPDATE_SCRIPT" ]] || fail "yuyinods-update.sh not found"

TMP_DIR="$(mktemp -d)"
trap 'rm -rf "$TMP_DIR"' EXIT

INSTALL_DIR="$TMP_DIR/yuyinods"
BIN_DIR="$TMP_DIR/bin"
mkdir -p "$INSTALL_DIR/data" "$INSTALL_DIR/config/litellm" "$BIN_DIR"

cp "$UPDATE_SCRIPT" "$INSTALL_DIR/yuyinods-update.sh"
chmod +x "$INSTALL_DIR/yuyinods-update.sh"
...
PATH="$BIN_DIR:$PATH" bash "$INSTALL_DIR/yuyinods-update.sh" health > "$TMP_DIR/health.out" 2>&1 \
...
pass "yuyinods-update health uses saved compose flags in runtime installs"

cat > "$INSTALL_DIR/.version" <<'EOF'
{"version":"1.0.0"}
EOF
PATH="$BIN_DIR:$PATH" bash "$INSTALL_DIR/yuyinods-update.sh" check > "$TMP_DIR/check.out" 2>&1 \
...
if grep -q "Run 'yuyinods-update.sh update' to update" "$TMP_DIR/check.out"; then
    cat "$TMP_DIR/check.out"
    fail "check still recommends direct source updater for runtime installs"
fi
pass "yuyinods-update check recommends runtime update command"

SOURCE_BIN_DIR="$TMP_DIR/source-bin"
SOURCE_PARENT="$TMP_DIR/source-parent"
SOURCE_INSTALL="$SOURCE_PARENT/yuyinods"
mkdir -p "$SOURCE_BIN_DIR" "$SOURCE_INSTALL"
cp "$UPDATE_SCRIPT" "$SOURCE_INSTALL/yuyinods-update.sh"
chmod +x "$SOURCE_INSTALL/yuyinods-update.sh"
...
git -C "$SOURCE_PARENT" add yuyinods/yuyinods-update.sh
PATH="$SOURCE_BIN_DIR:$PATH" bash "$SOURCE_INSTALL/yuyinods-update.sh" check > "$TMP_DIR/source-check.out" 2>&1 \
...
pass "yuyinods-update recognizes nested source checkout layout"

: > "$DOCKER_LOG"
set +e
PATH="$BIN_DIR:$PATH" bash "$INSTALL_DIR/yuyinods-update.sh" update > "$TMP_DIR/update.out" 2>&1
update_exit=$?
set -e
...
grep -q "./yuyinods-cli update" "$TMP_DIR/update.out" \
...
pass "yuyinods-update fails cleanly before mutating non-git runtime installs"
