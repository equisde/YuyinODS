...
# ============================================================================
# Tests that yuyinods-doctor.sh collects and reports extension diagnostics
#
...
if grep -q 'EXT_DIAGNOSTICS="\[\]"' "$ROOT_DIR/scripts/yuyinods-doctor.sh"; then
...
if grep -q "for sid in.*SERVICE_IDS" "$ROOT_DIR/scripts/yuyinods-doctor.sh"; then
...
if grep -q "docker inspect.*State.Status" "$ROOT_DIR/scripts/yuyinods-doctor.sh"; then
...
if grep -q "gpu_backend_incompatible" "$ROOT_DIR/scripts/yuyinods-doctor.sh"; then
...
if grep -q "missing_dependency" "$ROOT_DIR/scripts/yuyinods-doctor.sh"; then
...
if grep -q '"extensions":.*ext_diagnostics' "$ROOT_DIR/scripts/yuyinods-doctor.sh"; then
...
if grep -q "extensions_total" "$ROOT_DIR/scripts/yuyinods-doctor.sh" && \
   grep -q "extensions_healthy" "$ROOT_DIR/scripts/yuyinods-doctor.sh" && \
   grep -q "extensions_issues" "$ROOT_DIR/scripts/yuyinods-doctor.sh"; then
...
if grep -q "Extension.*container not running" "$ROOT_DIR/scripts/yuyinods-doctor.sh"; then
...
if grep -q "ext_total" "$ROOT_DIR/scripts/yuyinods-doctor.sh" && \
   grep -q "ext_healthy" "$ROOT_DIR/scripts/yuyinods-doctor.sh"; then
