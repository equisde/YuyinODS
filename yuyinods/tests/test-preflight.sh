#!/bin/bash
# ============================================================================
# ODS yuyinods-preflight.sh Test Suite
# ============================================================================
# Validates that yuyinods-preflight.sh is syntactically correct, follows the
...
PREFLIGHT="$ROOT_DIR/yuyinods-preflight.sh"
...
echo ""
echo "╔════════════════════════════════════════════════╗"
echo "║   yuyinods-preflight.sh Test Suite               ║"
echo "╚════════════════════════════════════════════════╝"
echo ""
...
# 1. Script exists
if [[ ! -f "$PREFLIGHT" ]]; then
    fail "yuyinods-preflight.sh not found at $PREFLIGHT"
    echo ""
    echo "Result: $PASSED passed, $FAILED failed"
    exit 1
fi
pass "yuyinods-preflight.sh exists"
...
if grep -q 'LLAMA_SERVER_PORT:-8080' "$PREFLIGHT"; then
    ...
    fail "LLM port fallback is not 8080 — check the OLLAMA_PORT expansion in yuyinods-preflight.sh"
fi
...
if grep -q 'docker port yuyinods-llama-server' "$PREFLIGHT"; then
...
if grep -q 'is_external_lemonade()' "$PREFLIGHT" \
   ...
   && grep -q 'yuyinods-litellm' "$PREFLIGHT"; then
    pass "External Lemonade preflight checks LiteLLM gateway"
else
    fail "External Lemonade preflight must not require managed yuyinods-llama-server"
fi
