...
SUPPORT_SCRIPT="$ROOT_DIR/scripts/yuyinods-support-bundle.sh"
...
if YUYINODS_SUPPORT_BUNDLE_BASH="$BASH_WRAPPER" YUYINODS_SUPPORT_BUNDLE_DISABLE_DOCKER=1 bash "$SUPPORT_SCRIPT" --output "$OUTPUT_DIR" --no-logs --json > "$RESULT_JSON"; then
    pass "support bundle command succeeds with Docker disabled"
else
    fail "support bundle command failed with Docker disabled"
fi

if grep -q 'scripts/yuyinods-doctor.sh' "$BASH_WRAPPER_LOG" && grep -q 'scripts/resolve-compose-stack.sh' "$BASH_WRAPPER_LOG"; then
...
assert manifest["tool"] == "yuyinods-support-bundle"
