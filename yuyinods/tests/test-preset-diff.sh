...
YUYINODS_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
YUYINODS_CLI="$YUYINODS_DIR/yuyinods-cli"
...
setup() {
    export YUYINODS_HOME="$YUYINODS_DIR"
    export INSTALL_DIR="$YUYINODS_DIR"
    export SCRIPT_DIR="$YUYINODS_DIR"
    # PRESETS_DIR is set by yuyinods-cli to ${INSTALL_DIR}/presets
    mkdir -p "$YUYINODS_DIR/presets"
    touch "$YUYINODS_DIR/docker-compose.base.yml" 2>/dev/null || true
}

# Cleanup
cleanup() {
    rm -rf "$YUYINODS_DIR/presets/test-preset-"* 2>/dev/null || true
...
test_diff_requires_args() {
    local output
    output=$("$YUYINODS_CLI" preset diff 2>&1 || true)
...
test_diff_nonexistent_preset() {
    local output
    output=$("$YUYINODS_CLI" preset diff nonexistent1 nonexistent2 2>&1 || true)
...
test_diff_identical() {
    local presets_dir="$YUYINODS_DIR/presets"
    ...
    local output
    output=$("$YUYINODS_CLI" preset diff test-preset-a test-preset-b 2>&1 || true)
...
test_diff_env_changes() {
    ...
    local output
    output=$("$YUYINODS_CLI" preset diff test-preset-c test-preset-d 2>&1 || true)
...
test_diff_service_changes() {
    ...
    local output
    output=$("$YUYINODS_CLI" preset diff test-preset-e test-preset-f 2>&1 || true)
...
test_diff_masks_secrets() {
    ...
    local output
    output=$("$YUYINODS_CLI" preset diff test-preset-g test-preset-h 2>&1 || true)
