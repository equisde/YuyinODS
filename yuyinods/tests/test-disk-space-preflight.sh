...
YUYINODS_RESTORE="$SCRIPT_DIR/../yuyinods-restore.sh"
...
[[ -x "$YUYINODS_BACKUP" ]] || fail "yuyinods-backup.sh not found or not executable"
[[ -x "$YUYINODS_RESTORE" ]] || fail "yuyinods-restore.sh not found or not executable"
...
set +e
out=$(PATH="$STUB_BIN:$PATH" YUYINODS_DIR="$FAKE_ODS" bash "$YUYINODS_BACKUP" --type user-data 2>&1)
rc=$?
set -e
...
set +e
out=$(PATH="$STUB_BIN:$PATH" YUYINODS_DIR="$FAKE_ODS" bash "$YUYINODS_RESTORE" -f "$BID" 2>&1 <<<'y')
rc=$?
set -e
