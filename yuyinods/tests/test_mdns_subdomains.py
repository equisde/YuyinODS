"""Contract tests for bin/yuyinods-mdns.py's per-subdomain A-record set.
...
MDNS_SCRIPT = REPO_ROOT / "bin" / "yuyinods-mdns.py"

# The set of subdomains the announcer must publish. Each name corresponds to
# a Caddy block in extensions/services/yuyinods-proxy/Caddyfile and (for the
...
def _extract_subdomain_routes_keys() -> set[str]:
    """Parse yuyinods-mdns.py and return the set of subdomain names it announces.
...
def test_announcer_publishes_all_required_subdomains() -> None:
    ...
    assert not missing, (
        f"bin/yuyinods-mdns.py is missing required subdomains: {sorted(missing)}. "
...
def test_announcer_does_not_advertise_unknown_subdomains() -> None:
    ...
    assert not extra, (
        f"bin/yuyinods-mdns.py publishes subdomains not listed in REQUIRED_SUBDOMAINS: "
