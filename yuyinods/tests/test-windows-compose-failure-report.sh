...
if grep -q "Write-ODSComposeDiagnostics .*SaveReport" "$ROOT_DIR/installers/windows/yuyinods.ps1"; then
    fail "yuyinods.ps1 command failures should not create install reports by default"
else
    pass "yuyinods.ps1 diagnostics remain console-only by default"
fi
