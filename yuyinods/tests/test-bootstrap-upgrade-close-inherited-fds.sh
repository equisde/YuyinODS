...
assert_runtime_lock_release() {
    ...
    lock_file="$(mktemp "${TMPDIR:-/tmp}/yuyinods-fd-${fd}.XXXXXX.lock")"
    pid_file="$(mktemp "${TMPDIR:-/tmp}/yuyinods-fd-${fd}.XXXXXX.pid")"
    log_file="$(mktemp "${TMPDIR:-/tmp}/yuyinods-fd-${fd}.XXXXXX.log")"
