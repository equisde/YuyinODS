...
if command -v pwsh >/dev/null 2>&1; then
    OUTPUT="$(SUMMARY_LIB="$SUMMARY_LIB" pwsh -NoProfile -Command '
        ...
        function Get-ODSReadinessContainerState {
            ...
            if ($Container -eq "yuyinods-webui") { return "running" }
            if ($Container -eq "yuyinods-qdrant") { return "missing" }
            return "healthy"
        }
        $checks = @(
            @{ Name = "Dashboard"; Url = "http://localhost:3001"; Container = "yuyinods-dashboard"; OpenUrl = "http://localhost:3001" },
            @{ Name = "Chat UI"; Url = "http://localhost:3000"; Container = "yuyinods-webui"; OpenUrl = "http://localhost:3000" },
            @{ Name = "Qdrant"; Url = "http://localhost:6333"; Container = "yuyinods-qdrant"; OpenUrl = "http://localhost:6333" }
        )
        Write-ODSInstallReadinessSummary -Checks $checks -StatusCommand ".\yuyinods.ps1 status" -LogPath "C:\yuyinods\logs\install.log" -DashboardUrl "http://localhost:3001"
    ')"
