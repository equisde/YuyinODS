...
# ============================================================================
# Tests that yuyinods-cli enable command validates GPU backend compatibility
...
if grep -q "SERVICE_GPU_BACKENDS.*service_id" "$ROOT_DIR/yuyinods-cli"; then
...
if grep -q "may not work with GPU backend" "$ROOT_DIR/yuyinods-cli"; then
...
if grep -q "Supported backends" "$ROOT_DIR/yuyinods-cli"; then
...
if grep -A5 "may not work with GPU backend" "$ROOT_DIR/yuyinods-cli" | grep -q "Continue anyway"; then
