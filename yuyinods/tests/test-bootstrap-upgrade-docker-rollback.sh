...
case "${1:-}" in
    ...
    compose)
        ...
        if [[ " $* " == *" up -d --force-recreate --no-deps llama-server "* ]]; then
            printf 'compose-up:%s\n' "$(env_value GGUF_FILE)" >> "${YUYINODS_FAKE_DOCKER_LOG:?}"
            exit 0
        fi
        exit 0
        ;;
    ps)
        if [[ " $* " == *"name=yuyinods-llama-server"* ]]; then
            printf 'yuyinods-llama-server\n'
        fi
        exit 0
        ;;
    inspect)
        if [[ "${2:-}" == "yuyinods-llama-server" ]]; then
...
set +e
PATH="$fakebin:$PATH" YUYINODS_FAKE_DOCKER_LOG="$docker_calls" YUYINODS_FAKE_CURL_LOG="$curl_calls" bash "$TARGET" \
    "$install_dir" \
    "Full.gguf" \
