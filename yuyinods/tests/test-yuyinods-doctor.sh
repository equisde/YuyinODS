#!/bin/bash
# ============================================================================
# ODS yuyinods-doctor.sh Test Suite
# ============================================================================
# Ensures scripts/yuyinods-doctor.sh runs without shell errors and produces
# expected JSON output with correct structure. Validates the diagnostic tool
# used in installer simulation and CI artifacts.
#
# Usage: ./tests/test-yuyinods-doctor.sh
# ============================================================================
...
echo ""
echo "╔═══════════════════════════════════════════════╗"
echo "║   yuyinods-doctor.sh Test Suite                  ║"
echo "╚═══════════════════════════════════════════════╝"
echo ""

# 1. Script exists
if [[ ! -f "$ROOT_DIR/scripts/yuyinods-doctor.sh" ]]; then
    fail "scripts/yuyinods-doctor.sh not found"
    echo ""; echo "Result: $PASSED passed, $FAILED failed"; exit 1
fi
pass "yuyinods-doctor.sh exists"

# 2. --help flag works
set +e
help_out=$(cd "$ROOT_DIR" && bash scripts/yuyinods-doctor.sh --help 2>&1)
help_exit=$?
set -e

if [[ "$help_exit" -eq 0 ]] && echo "$help_out" | grep -q "Usage:"; then
    pass "yuyinods-doctor.sh --help displays usage"
else
    fail "yuyinods-doctor.sh --help failed or missing usage text"
fi

# 3. Runs without shell error (default output path)
TEMP_REPORT=$(mktemp /tmp/yuyinods-doctor-test.XXXXXX.json)
trap 'rm -f "$TEMP_REPORT"' EXIT

set +e
out=$(cd "$ROOT_DIR" && bash scripts/yuyinods-doctor.sh "$TEMP_REPORT" 2>&1)
exit_code=$?
set -e

if echo "$out" | grep -q "unbound variable\|syntax error\|command not found"; then
    fail "yuyinods-doctor.sh produced shell error in output"
else
    pass "yuyinods-doctor.sh runs without shell errors"
fi

# Exit code must be 0 or 1 (documented: 0=success, 1=error)
if [[ "$exit_code" -eq 0 ]] || [[ "$exit_code" -eq 1 ]]; then
    pass "yuyinods-doctor.sh exit code is valid (0|1): $exit_code"
else
    fail "yuyinods-doctor.sh exit code should be 0 or 1; got $exit_code"
fi

# 4. Produces JSON output file
if [[ -f "$TEMP_REPORT" ]]; then
    pass "yuyinods-doctor.sh creates output file"
else
    fail "yuyinods-doctor.sh did not create output file at $TEMP_REPORT"
    echo ""; echo "Result: $PASSED passed, $FAILED failed"; exit 1
fi
...
if command -v jq >/dev/null 2>&1; then
    ...
    if [[ $jq_exit -eq 0 ]]; then
        pass "yuyinods-doctor.sh output is valid JSON"
    else
        fail "yuyinods-doctor.sh output is not valid JSON"
    fi
else
...
if command -v jq >/dev/null 2>&1; then
    ...
    for field in "${required_fields[@]}"; do
        ...
        if [[ $jq_exit -ne 0 ]]; then
            fail "yuyinods-doctor.sh output missing required field: $field"
            all_present=false
        fi
    done

    if $all_present; then
        pass "yuyinods-doctor.sh output contains all required fields"
    fi
else
...
if command -v jq >/dev/null 2>&1; then
    ...
    if [[ $jq_exit -eq 0 ]]; then
        pass "yuyinods-doctor.sh autofix_hints is an array"
    else
        fail "yuyinods-doctor.sh autofix_hints is not an array"
    fi
fi
...
if command -v jq >/dev/null 2>&1; then
    ...
    if [[ $jq_exit -eq 0 ]]; then
        pass "yuyinods-doctor.sh diagnoses is an array"
    else
        fail "yuyinods-doctor.sh diagnoses is not an array"
    fi

    jq_exit=0
    jq -e '.install_artifacts.env_file.exists | type == "boolean"' "$TEMP_REPORT" >/dev/null || jq_exit=$?
    if [[ $jq_exit -eq 0 ]]; then
        pass "yuyinods-doctor.sh install_artifacts records env file state"
    else
        fail "yuyinods-doctor.sh install_artifacts missing env file state"
    fi
fi
...
if command -v jq >/dev/null 2>&1; then
    ...
    for field in "${runtime_fields[@]}"; do
        ...
        if [[ "$field_type" != "boolean" ]]; then
            fail "yuyinods-doctor.sh runtime.$field is not boolean (got: $field_type)"
            runtime_ok=false
        fi
    done

    if $runtime_ok; then
        pass "yuyinods-doctor.sh runtime section has correct boolean fields"
    fi

    jq_exit=0
    jq -e '.runtime.inference_contract | type == "object"' "$TEMP_REPORT" >/dev/null || jq_exit=$?
    if [[ $jq_exit -eq 0 ]]; then
        pass "yuyinods-doctor.sh runtime includes inference contract diagnostics"
    else
        fail "yuyinods-doctor.sh runtime missing inference contract diagnostics"
    fi
    ...
    for field in available runtime location runtimeMode managedByODS selectedBackend supportedBackends health warnings; do
        ...
        if [[ $jq_exit -ne 0 ]]; then
            fail "yuyinods-doctor.sh runtime.amd_runtime missing field: $field"
            amd_fields_ok=false
        fi
    done
    if $amd_fields_ok; then
        pass "yuyinods-doctor.sh AMD runtime diagnostics fields present"
    fi
fi
...
if command -v jq >/dev/null 2>&1; then
    ...
    for field in "${summary_fields[@]}"; do
        ...
        if [[ "$field_type" != "number" ]]; then
            fail "yuyinods-doctor.sh summary.$field is not a number (got: $field_type)"
            summary_ok=false
        fi
    done

    if $summary_ok; then
        pass "yuyinods-doctor.sh summary section has correct numeric fields"
    fi
fi
