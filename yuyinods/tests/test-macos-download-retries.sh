...
export YUYINODS_LOG_FILE="$TMP_DIR/install.log"
...
#!/usr/bin/env bash
count_file="${YUYINODS_TEST_CURL_COUNT_FILE:?}"
count=0
[[ -f "$count_file" ]] && count="$(cat "$count_file")"
count=$((count + 1))
printf '%s\n' "$count" > "$count_file"
exit "${YUYINODS_TEST_CURL_EXIT:-56}"
EOF
...
run_failure_case() {
    ...
    export YUYINODS_TEST_CURL_COUNT_FILE="$count_file"
    if [[ "$retries" == "__unset__" ]]; then
        unset YUYINODS_MODEL_DOWNLOAD_RETRIES
    else
        export YUYINODS_MODEL_DOWNLOAD_RETRIES="$retries"
    fi
    ...
    unset YUYINODS_TEST_CURL_COUNT_FILE YUYINODS_MODEL_DOWNLOAD_RETRIES
