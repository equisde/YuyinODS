#!/usr/bin/env bash
# ============================================================================
# Contract test: yuyinods-doctor.sh Darwin fixture
# ============================================================================
# Regression shields for the Darwin branch of scripts/yuyinods-doctor.sh added in
# fix/yuyinods-cli-apple-silicon-coverage:
...
if [[ "$(uname -s)" != "Darwin" ]]; then
    echo "[SKIP] yuyinods-doctor Darwin fixture — macOS only"
    exit 0
fi
...
if [[ -f "$ROOT_DIR/.env" ]]; then
    echo "[SKIP] yuyinods-doctor Darwin fixture — pre-existing $ROOT_DIR/.env (would clobber)"
    exit 0
fi
...
TMPDIR_TEST=$(mktemp -d /tmp/test-yuyinods-doctor-darwin.XXXXXX)
...
# ----------------------------------------------------------------------------
set +e
GPU_BACKEND=apple bash scripts/yuyinods-doctor.sh "$REPORT" \
    >"$TMPDIR_TEST/live.log" 2>&1
rc=$?
set -e

if [[ $rc -eq 0 ]]; then
    pass "yuyinods-doctor.sh exits 0 on Darwin under GPU_BACKEND=apple"
else
    fail "yuyinods-doctor.sh exited non-zero on Darwin (rc=$rc); see $TMPDIR_TEST/live.log"
fi
...
if [[ -f "$REPORT" ]]; then
    ...
    fail "yuyinods-doctor.sh did not produce report at $REPORT"
fi
...
PATH="$STUB_BIN:$PATH" GPU_BACKEND=apple \
    bash scripts/yuyinods-doctor.sh "$REPORT" \
    >"$TMPDIR_TEST/fallback.log" 2>&1
rc=$?
set -e

if [[ $rc -ne 0 ]]; then
    fail "yuyinods-doctor.sh exited non-zero under empty-sysctl stub (rc=$rc); see $TMPDIR_TEST/fallback.log"
elif [[ -f "$REPORT" ]]; then
    ...
    fail "yuyinods-doctor.sh did not produce report under fallback test"
fi
