...
These checks intentionally avoid executing yuyinods-cli. They are a decomposition
...
def main() -> int:
    if not YUYINODS_CLI.exists():
        fail(f"Missing ods CLI: {YUYINODS_CLI}")

    text = YUYINODS_CLI.read_text(encoding="utf-8")

    require(r"^#!/usr/bin/env bash$", text, "yuyinods-cli should remain a Bash entrypoint")
    require(
        r"Usage: ods <command> \[options\]",
        text,
        "yuyinods-cli help must expose the top-level usage line",
    )
    require(
        r'^case "\$\{1:-help\}" in$',
        text,
        "yuyinods-cli must keep a top-level command dispatch case",
    )
    require(r"^cmd_help\(\) \{$", text, "yuyinods-cli must define cmd_help")
    require(
        r'"\$_ods_mode" != "cloud"[\s\S]*"\$_external_lemonade_active" != "true"[\s\S]*docker-compose\.cloud\.yml',
        text,
        "yuyinods-cli must reject stale cloud compose caches in local/managed modes",
    )
    ...
    print("[PASS] yuyinods-cli static command contract")
    return 0
