...
grep -qF 'YUYINODS_ALLOW_DREAMSERVER_PARALLEL' get-yuyinods.sh \
  || { echo "[FAIL] get-yuyinods.sh must require an explicit override before parallel DreamServer installs"; exit 1; }
grep -qF 'YUYINODS_INSTALL_DIR' get-yuyinods.sh \
  || { echo "[FAIL] get-yuyinods.sh must allow an explicit ODS install dir for isolated parallel testing"; exit 1; }
grep -qF 'dream-server' get-yuyinods.sh \
  || { echo "[FAIL] get-yuyinods.sh must detect legacy ~/dream-server installs"; exit 1; }
grep -qF 'name=^/dream-' get-yuyinods.sh \
  || { echo "[FAIL] get-yuyinods.sh must detect legacy DreamServer containers"; exit 1; }
grep -qF 'YUYINODS_ALLOW_DREAMSERVER_PARALLEL' installers/phases/01-preflight.sh \
...
grep -qF 'yuyinods/yuyinods-cli text eol=lf' ../.gitattributes \
  || { echo "[FAIL] .gitattributes must force LF checkout for extensionless yuyinods/yuyinods-cli"; exit 1; }
...
awk '/cmd_restart\(\)/,/^}/' yuyinods-cli | grep -q '_ods_cli_maybe_resume_bootstrap_upgrade' \
  || { echo "[FAIL] ods restart must retry failed bootstrap upgrades"; exit 1; }
awk '/cmd_start\(\)/,/^}/' yuyinods-cli | grep -q '_ods_cli_maybe_resume_bootstrap_upgrade' \
  || { echo "[FAIL] ods start must retry failed bootstrap upgrades"; exit 1; }
awk '/cmd_restart\(\)/,/^}/' yuyinods-cli | grep -q '_ods_cli_wait_for_bootstrap_compose_safe' \
  || { echo "[FAIL] ods restart must wait for active bootstrap hot-swaps before compose"; exit 1; }
awk '/cmd_start\(\)/,/^}/' yuyinods-cli | grep -q '_ods_cli_wait_for_bootstrap_compose_safe' \
  || { echo "[FAIL] ods start must wait for active bootstrap hot-swaps before compose"; exit 1; }
grep -q 'starting|verifying|swapping' yuyinods-cli \
  || { echo "[FAIL] yuyinods-cli bootstrap compose guard must include swapping"; exit 1; }
...
grep -q '_env_set "HSA_OVERRIDE_GFX_VERSION" "11.5.1"' yuyinods-cli \
  || { echo "[FAIL] yuyinods-cli must set HSA override to 11.5.1 for gfx1151"; exit 1; }
grep -q '_env_unset "HSA_OVERRIDE_GFX_VERSION"' yuyinods-cli \
  || { echo "[FAIL] yuyinods-cli must remove HSA override for non-Strix AMD GPUs"; exit 1; }
grep -q '_env_unset "LEMONADE_LLAMACPP_ROCM_BIN"' yuyinods-cli \
  || { echo "[FAIL] yuyinods-cli must remove gfx1151-only custom binary for non-Strix AMD GPUs"; exit 1; }
if grep -q '_env_set "HSA_OVERRIDE_GFX_VERSION" "\$gfx_ver"' yuyinods-cli; then
  echo "[FAIL] yuyinods-cli must not write raw gfx ids such as gfx942 to HSA_OVERRIDE_GFX_VERSION"
  exit 1
fi
...
for s in scripts/build-capability-profile.sh scripts/classify-hardware.sh scripts/load-backend-contract.sh scripts/resolve-compose-stack.sh scripts/preflight-engine.sh scripts/yuyinods-doctor.sh scripts/simulate-installers.sh; do
...
features_phase="yuyinods/installers/phases/03-features.sh"
...
# bug propagates to every subsequent yuyinods-cli invocation.
#
...
_resolver_callers=(
  "yuyinods-cli"
  "yuyinods-update.sh"
  "bin/yuyinods-host-agent.py"
  "scripts/yuyinods-preflight.sh"
  "scripts/validate.sh"
  "installers/lib/compose-select.sh"
  "installers/macos/yuyinods-macos.sh"
...
for spec in \
  ...
  'ENABLE_ODS_PROXY:yuyinods-proxy' \
...
for svc in litellm searxng token-spy hermes hermes-proxy openclaw ape perplexica privacy-shield yuyinods-proxy tailscale brave-search; do
...
for installer in install-core.sh installers/macos/install-macos.sh; do
  grep -Fq 'name=^/yuyinods-openclaw$' "$installer" \
...
grep -q 'export YUYINODS_PYTHON_CMD' installers/macos/install-macos.sh \
