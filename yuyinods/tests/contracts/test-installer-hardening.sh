...
if PATH="$tmpdir:$PATH" USERPROFILE="" LOCALAPPDATA="" YUYINODS_PYTHON_CMD="$fake_py" scripts/resolve-compose-stack.sh --script-dir "$ROOT_DIR" >"$tmpdir/missing-yaml.out" 2>"$missing_yaml_err"; then
...
assert_contains "$bootstrap" 'YUYINODS_REF' "bootstrap should allow PR/fleet lanes to clone a matching ref"
assert_contains "$bootstrap" 'clone_args\+=\(--branch "\$YUYINODS_REF"\)' "bootstrap ref override should apply to git clone"
...
assert_contains "$macos_installer" 'export YUYINODS_PYTHON_CMD' "macOS installer does not export selected Python"
...
assert_contains "$macos_ui" 'YUYINODS_DOWNLOAD_CONNECT_TIMEOUT:-30' "macOS bootstrap model download should allow configurable connect timeout"
assert_contains "$macos_ui" 'YUYINODS_DOWNLOAD_LOW_SPEED_TIME:-300' "macOS bootstrap model download should tolerate slow but active transfers"
assert_contains "$macos_ui" 'YUYINODS_DOWNLOAD_LOW_SPEED_LIMIT:-1024' "macOS bootstrap model download should use a lenient low-speed threshold"
...
assert_contains "installers/windows/yuyinods.ps1" 'Invoke-ODSSttModelDownloadTrigger' "yuyinods.ps1 repair voice should trigger STT preload through a bounded helper"
assert_not_contains "installers/windows/yuyinods.ps1" 'Invoke-WebRequest -Method POST -Uri \$voice\.SttModelUrl -TimeoutSec 3600' "yuyinods.ps1 repair voice should not block on the long STT preload POST"

echo "[contract] Windows Lemonade dashboard activation uses native runtime health"
host_agent="bin/yuyinods-host-agent.py"
...
assert_contains "$win_phase06" 'extensions\\services\\yuyinods-proxy\\Caddyfile' "Windows phase 06 should clean malformed ODS proxy Caddyfile directories before compose"
...
#!/usr/bin/env bash
printf '%s\n' "$*" > "$YUYINODS_TEST_SUDO_ARGS"
exit 0
SUDOEOT
chmod +x "$fakebin/sudo"
sudo_args="$tmpdir/sudo.args"
PATH="$fakebin:$PATH" YUYINODS_TEST_SUDO_ARGS="$sudo_args" bash -c '
  set -euo pipefail
  INTERACTIVE=false
  DRY_RUN=false
...
assert_contains "installers/phases/11-services.sh" 'dependency failed to start: container yuyinods-\(llama-server\|llama-ready\|llama-server-ready\) is unhealthy' "Linux delayed-health grace is not scoped to LLM health-gate failures"
...
assert_contains "installers/phases/12-health.sh" 'YUYINODS_LLM_DELAYED_HEALTH_ATTEMPTS:-300' "Linux delayed compose health does not extend the LLM health wait for large GGUF reloads"
...
assert_contains "installers/windows/phases/07-devtools.ps1" 'Start-ScheduledTask -TaskName \$script:YUYINODS_AGENT_TASK_NAME' "Windows installer should start host-agent through Scheduled Tasks"
assert_contains "installers/windows/lib/env-generator.ps1" 'YUYINODS_AGENT_BIND=.*0\.0\.0\.0' "Windows installer should bind host-agent for dashboard-api container access"
assert_contains "installers/windows/yuyinods.ps1" 'Resolve-ODSHostAgentPython' "yuyinods.ps1 agent start does not resolve a real Python"
assert_contains "installers/windows/yuyinods.ps1" 'WindowsApps' "yuyinods.ps1 agent start does not reject Microsoft Store Python aliases"
assert_contains "installers/windows/yuyinods.ps1" 'PrefixArgs' "yuyinods.ps1 agent start does not support py launcher -3 arguments"
assert_contains "installers/windows/yuyinods.ps1" 'Register-ScheduledTask -TaskName \$script:YUYINODS_AGENT_TASK_NAME' "yuyinods.ps1 agent start should use Scheduled Tasks so SSH-launched agents persist"
assert_contains "installers/windows/yuyinods.ps1" 'RedirectStandardError .* -Wait' "yuyinods.ps1 agent task should wait on Python instead of spawning a transient child"
