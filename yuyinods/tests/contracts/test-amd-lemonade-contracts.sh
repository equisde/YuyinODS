...
if grep -q 'YUYINODS_TALK_HERMES_TIMEOUT=${YUYINODS_TALK_HERMES_TIMEOUT:-900}' docker-compose.amd.yml \
   && grep -q 'YUYINODS_TALK_HERMES_TIMEOUT=${YUYINODS_TALK_HERMES_TIMEOUT:-900}' docker-compose.lemonade-external.yml \
   && grep -q 'YUYINODS_TALK_HERMES_TIMEOUT=${YUYINODS_TALK_HERMES_TIMEOUT:-900}' installers/windows/docker-compose.windows-amd.yml; then
    pass "Lemonade ODS Talk Hermes timeout is upgraded to 900s"
else
    fail "Lemonade ODS Talk must set YUYINODS_TALK_HERMES_TIMEOUT=900 in AMD, external, and Windows AMD overlays"
fi
...
if grep -q -- '--extra-models-dir' docker-compose.amd.yml \
    ...
    pass "docker-compose.amd.yml: Lemonade imports YuyinODS-managed GGUFs from /models"
else
    fail "docker-compose.amd.yml must expose YuyinODS-managed GGUFs through Lemonade --extra-models-dir /models"
fi
...
# ---------------------------------------------------------------------------
# 10. Schema allows YUYINODS_MODE=lemonade
# ---------------------------------------------------------------------------
echo "[contract] .env schema allows lemonade mode"
if grep -q '"lemonade"' .env.schema.json; then
    pass ".env.schema.json: lemonade in YUYINODS_MODE enum"
else
    fail ".env.schema.json: must include lemonade in YUYINODS_MODE enum"
fi
...
if command -v pwsh >/dev/null 2>&1; then
    ...
    if ROOT_DIR="$ROOT_DIR" AMD_LEMONADE_IMAGE="$AMD_LEMONADE_IMAGE" TEMP="$_ps_tmp" ProgramFiles="$_ps_tmp" USERPROFILE="$_ps_tmp" pwsh -NoProfile -Command '
        ...
        $probeRoot = Join-Path $env:TEMP "yuyinods-lemonade-resolver-contract"
...
if command -v pwsh >/dev/null 2>&1; then
    ...
    if ROOT_DIR="$ROOT_DIR" TEMP="$_ps_tmp" USERPROFILE="$_ps_tmp" pwsh -NoProfile -Command '
        ...
        $installDir = Join-Path $env:TEMP "yuyinods-env-generator-amd-lemonade-contract"
        ...
        if ($envText -notmatch "(?m)^YUYINODS_MODE=lemonade$") {
            throw "Expected Windows AMD Lemonade installs to write YUYINODS_MODE=lemonade"
        }
...
if command -v pwsh >/dev/null 2>&1; then
    ...
    if ROOT_DIR="$ROOT_DIR" TEMP="$_ps_tmp" USERPROFILE="$_ps_tmp" pwsh -NoProfile -Command '
        ...
        function Write-AIWarn { param([string]$Message) Write-Host "WARN: $Message" }
        . (Join-Path $env:ROOT_DIR "installers/windows/lib/env-generator.ps1")
        $installDir = Join-Path $env:TEMP "yuyinods-env-generator-windows-local-contract"
        Remove-Item -LiteralPath $installDir -Recurse -Force -ErrorAction SilentlyContinue
        New-Item -ItemType Directory -Path $installDir -Force | Out-Null
...
if grep -q 'backend-contract.ps1' installers/windows/yuyinods.ps1 \
    && grep -q 'Resolve-ODSLemonadeExe' installers/windows/yuyinods.ps1; then
    pass "yuyinods.ps1 resolves Lemonade across both Program Files roots"
else
    fail "yuyinods.ps1 must use Lemonade resolver for installed CLI commands"
fi
...
if [[ "${1:-}" == "version" && "${2:-}" == "--format" ]]; then
    if [[ "${YUYINODS_FAKE_DOCKER_SUDO:-}" == "1" ]]; then
        echo "29.3.0"
        exit 0
    fi
    exit 1
...
if _probe_output="$(
    PATH="$_amd_docker_probe_tmp/bin:$PATH" bash -c '
        set -euo pipefail
        ods_sudo() { YUYINODS_FAKE_DOCKER_SUDO=1 "$@"; }
        eval "$1"
