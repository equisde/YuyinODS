...
cd "$ROOT_DIR"
PYTHON_CMD="${YUYINODS_PYTHON_CMD:-python3}"
...
grep -q 'YUYINODS_TALK_HERMES_TIMEOUT=${YUYINODS_TALK_HERMES_TIMEOUT:-900}' docker-compose.lemonade-external.yml \
  || { echo "[FAIL] external Lemonade overlay must set YUYINODS_TALK_HERMES_TIMEOUT=900"; exit 1; }
...
grep -q 'is_external_lemonade()' yuyinods-preflight.sh \
  || { echo "[FAIL] yuyinods-preflight must detect external Lemonade mode"; exit 1; }
grep -q 'LiteLLM external Lemonade gateway' yuyinods-preflight.sh \
  || { echo "[FAIL] yuyinods-preflight must label the external Lemonade LiteLLM route"; exit 1; }
grep -q 'yuyinods-litellm' yuyinods-preflight.sh \
  || { echo "[FAIL] yuyinods-preflight must check yuyinods-litellm for external Lemonade"; exit 1; }

echo "[contract] doctor warns on unauthenticated host-routed external Lemonade"
grep -q 'YuyinODS-RUNTIME-EXTERNAL-LEMONADE-UNAUTHENTICATED-HOST-ROUTE' scripts/yuyinods-doctor.sh \
  || { echo "[FAIL] yuyinods-doctor must warn when external Lemonade is host-routed without a user API key"; exit 1; }
grep -q 'sk-yuyinods-lemonade-' scripts/yuyinods-doctor.sh \
  || { echo "[FAIL] yuyinods-doctor must distinguish installer-generated LiteLLM provider keys from user Lemonade API keys"; exit 1; }

echo "[contract] resolver selects cloud + external overlay instead of managed AMD overlay"
resolved="$(LEMONADE_EXTERNAL=true YUYINODS_MODE=lemonade \
  ./scripts/resolve-compose-stack.sh --script-dir "$ROOT_DIR" --yuyinods-mode lemonade --gpu-backend amd --tier SH_LARGE --env)"
...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
  ...
  N8N_PASS=test \
  SEARXNG_SECRET=test \
  YUYINODS_SESSION_SECRET=test \
  LEMONADE_EXTERNAL=true \
  YUYINODS_MODE=lemonade \
  GPU_BACKEND=amd \
...
grep -q 'yuyinods-external-lemonade' installers/phases/11-services.sh \
...
grep -q 'docker-compose.lemonade-external.yml' yuyinods-cli \
  || { echo "[FAIL] yuyinods-cli must recognize external Lemonade compose cache state"; exit 1; }
grep -q 'AMD_INFERENCE_MANAGED' yuyinods-cli \
  || { echo "[FAIL] yuyinods-cli must detect unmanaged external Lemonade installs"; exit 1; }
grep -q 'compose.local.yaml' yuyinods-cli \
  || { echo "[FAIL] yuyinods-cli must invalidate stale local dependency overlays"; exit 1; }
