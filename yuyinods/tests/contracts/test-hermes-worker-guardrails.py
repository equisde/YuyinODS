...
DOCTOR = ROOT_DIR / "scripts" / "yuyinods-doctor.sh"
...
def main() -> int:
    for path in (YUYINODS_CLI, DOCTOR, PRUNE_SCRIPT, HERMES_DOCS):
        if not path.exists():
    ...
    for needle in (
        ...
        "FORCE=0",
        "--force",
        "YUYINODS_HERMES_SLASH_WORKER_PS_FIXTURE",
    ...
    require(YUYINODS_CLI, "hermes-workers|slash-workers)", "ods repair target missing")
    require(YUYINODS_CLI, "ods repair hermes-workers", "ods repair help missing")
