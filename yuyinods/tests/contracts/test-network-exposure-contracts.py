...
def test_hermes_is_internal_only_and_proxy_gated() -> None:
    ...
    assert_true("reverse_proxy {$HERMES_PROXY_UPSTREAM:yuyinods-hermes:9119}" in proxy_caddyfile, "hermes-proxy must forward to internal Hermes")
...
def test_hermes_local_provider_has_generous_timeouts() -> None:
    ...
    assert_true(
        "YUYINODS_TALK_HERMES_TIMEOUT=${YUYINODS_TALK_HERMES_TIMEOUT:-900}" in base_compose,
...
def test_ods_proxy_routes_talk_portal() -> None:
    caddyfile = read(SERVICES / "yuyinods-proxy" / "Caddyfile")

    assert_true("talk.{$YUYINODS_DEVICE_NAME:ods}.local" in caddyfile, "yuyinods-proxy must route talk.<device>.local")
