...
grep -q 'YUYINODS_TALK_VISION_URL=.*host.docker.internal' installers/windows/docker-compose.windows-amd.yml \
  || { echo "[FAIL] Windows AMD overlay must route ODS Talk vision calls to the host runtime"; exit 1; }
grep -q 'YUYINODS_TALK_HERMES_TIMEOUT=${YUYINODS_TALK_HERMES_TIMEOUT:-900}' installers/windows/docker-compose.windows-amd.yml \
...
grep -q 'YUYINODS_TALK_VISION_URL: http://host.docker.internal:8080/api/v1' <<<"$rendered" \
  || { echo "[FAIL] ODS Talk vision URL must use the Windows AMD host runtime API path"; exit 1; }
grep -q 'YUYINODS_TALK_HERMES_TIMEOUT: "900"' <<<"$rendered" \
