...
# ---------------------------------------------------------------------------
if grep -Eq 'YUYINODS_LEMONADE_SWAP_ATTEMPTS:-(6[0-9]|[7-9][0-9]|[1-9][0-9]{2,})' "$SCRIPT"; then
    pass "swap wait is configurable (YUYINODS_LEMONADE_SWAP_ATTEMPTS) with a >= 60 default"
else
    fail "swap wait must read YUYINODS_LEMONADE_SWAP_ATTEMPTS with a default of at least 60"
fi
...
if grep -q 'Get-NetTCPConnection -LocalPort' "$SCRIPT" \
   ...
   && grep -q 'YUYINODS_WIN_MODELS_DIR' "$SCRIPT"; then
...
# ---------------------------------------------------------------------------
# 8. The restart helper must stay scoped to YuyinODS-managed native Windows
