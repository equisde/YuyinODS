...
# ============================================================================
# Static checks for the Windows yuyinods.ps1 "report" command wiring.
# ============================================================================
...
grep -q "install-report.ps1" "$YUYINODS_PS1" && pass "yuyinods.ps1 sources report library" || fail "yuyinods.ps1 missing report library source"
grep -q "llm-endpoint.ps1" "$YUYINODS_PS1" && pass "yuyinods.ps1 sources llm endpoint helper" || fail "yuyinods.ps1 missing llm endpoint helper source"
grep -q "function Invoke-Report" "$YUYINODS_PS1" && pass "Invoke-Report function exists" || fail "Invoke-Report function missing"
grep -q '"report"  { Invoke-Report }' "$YUYINODS_PS1" && pass "report command dispatch exists" || fail "report command dispatch missing"
grep -q "Generate Windows diagnostics bundle" "$YUYINODS_PS1" && pass "help text includes report command" || fail "help text missing report command"
