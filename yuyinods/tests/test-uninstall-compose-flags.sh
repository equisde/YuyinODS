...
TARGET="$ROOT_DIR/yuyinods-uninstall.sh"
TMP_DIR=""
...
make_install() {
    ...
    cp "$TARGET" "$install_dir/yuyinods-uninstall.sh"
    cp "$ROOT_DIR/lib/safe-env.sh" "$install_dir/lib/safe-env.sh"
    touch "$install_dir/yuyinods-cli"
...
run_uninstall() {
    ...
    DOCKER_LOG="${DOCKER_LOG:?}" \
        bash "$install_dir/yuyinods-uninstall.sh" --force "$@" >/dev/null
}
...
main() {
    ...
    TMP_DIR="$(mktemp -d -t yuyinods-uninstall-test-XXXXXX)"
    ...
    ln -s "$install_keep/yuyinods-cli" "$home_keep/.local/bin/yuyinods"
    ...
    [[ ! -L "$home_keep/.local/bin/yuyinods" ]] \
