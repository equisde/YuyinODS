...
# Validates that yuyinods-cli properly delegates to yuyinods-backup.sh and yuyinods-restore.sh

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
YUYINODS_CLI="$SCRIPT_DIR/../yuyinods-cli"
...
# Test 1: Verify yuyinods-cli syntax
test_syntax() {
    info "Test 1: Validating yuyinods-cli syntax"
    if bash -n "$YUYINODS_CLI" 2>/dev/null; then
        pass "yuyinods-cli syntax is valid"
    else
        fail "yuyinods-cli has syntax errors"
    fi
}
...
test_backup_in_help() {
    ...
    if grep -q "backup.*Create a backup" "$YUYINODS_CLI" 2>/dev/null; then
...
test_restore_in_help() {
    ...
    if grep -q "restore.*Restore from a backup" "$YUYINODS_CLI" 2>/dev/null; then
...
test_backup_examples() {
    ...
    if grep -q "ods backup" "$YUYINODS_CLI" 2>/dev/null; then
...
test_cmd_backup_exists() {
    ...
    if grep -q "^cmd_backup()" "$YUYINODS_CLI" 2>/dev/null; then
...
test_cmd_restore_exists() {
    ...
    if grep -q "^cmd_restore()" "$YUYINODS_CLI" 2>/dev/null; then
...
test_backup_case() {
    ...
    if grep -q "backup)" "$YUYINODS_CLI" 2>/dev/null; then
...
test_restore_case() {
    ...
    if grep -q "restore)" "$YUYINODS_CLI" 2>/dev/null; then
...
# Test 9: Verify backup delegates to yuyinods-backup.sh
test_backup_delegation() {
    info "Test 9: Checking if cmd_backup delegates to yuyinods-backup.sh"
    if grep -A5 "^cmd_backup()" "$YUYINODS_CLI" 2>/dev/null | grep -q "yuyinods-backup.sh"; then
        pass "cmd_backup delegates to yuyinods-backup.sh"
        return 0
    else
        fail "cmd_backup does not delegate to yuyinods-backup.sh"
        return 0
    fi
}

# Test 10: Verify restore delegates to yuyinods-restore.sh
test_restore_delegation() {
    info "Test 10: Checking if cmd_restore delegates to yuyinods-restore.sh"
    if grep -A5 "^cmd_restore()" "$YUYINODS_CLI" 2>/dev/null | grep -q "yuyinods-restore.sh"; then
        pass "cmd_restore delegates to yuyinods-restore.sh"
        return 0
    else
        fail "cmd_restore does not delegate to yuyinods-restore.sh"
        return 0
    fi
}
...
test_argument_passing() {
    ...
    # in its default path.
    if grep -A25 "^cmd_backup()" "$YUYINODS_CLI" 2>/dev/null | grep -q '\$@'; then
