...
case " $* " in
  *" -sI "*)
    if [[ "${YUYINODS_FAKE_NO_CONTENT_LENGTH:-0}" == "1" ]]; then
      printf 'HTTP/2 200\r\n\r\n'
      exit 0
    fi
    printf 'HTTP/2 200\r\ncontent-length: 100\r\n\r\n'
...
grep -q 'yuyinods-bootstrap-upgrade-' "$TARGET" \
...
set +e
PATH="$fakebin:$PATH" YUYINODS_BOOTSTRAP_DOWNLOAD_ATTEMPTS=2 bash "$TARGET" \
    "$install_dir" \
    "Full.gguf" \
...
set +e
PATH="$fakebin:$PATH" YUYINODS_FAKE_NO_CONTENT_LENGTH=1 YUYINODS_BOOTSTRAP_DOWNLOAD_ATTEMPTS=1 bash "$TARGET" \
    "$unknown_install_dir" \
    "Full.gguf" \
    "https://example.invalid/Full.gguf" \
    "" \
