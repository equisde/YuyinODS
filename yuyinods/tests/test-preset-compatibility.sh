...
# Validates that yuyinods-cli checks for required preset files and extension existence

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
YUYINODS_CLI="$SCRIPT_DIR/../yuyinods-cli"
YUYINODS_DIR="$SCRIPT_DIR/.."
...
test_function_exists() {
    ...
    if grep -q "^validate_preset_compatibility()" "$YUYINODS_CLI" 2>/dev/null; then
...
test_checks_meta_txt() {
    ...
    if grep -A20 "^validate_preset_compatibility()" "$YUYINODS_CLI" 2>/dev/null | grep -q "meta.txt"; then
...
test_checks_extensions_list() {
    ...
    if grep -A20 "^validate_preset_compatibility()" "$YUYINODS_CLI" 2>/dev/null | grep -q "extensions.list"; then
...
test_checks_env_file() {
    ...
    if grep -A20 "^validate_preset_compatibility()" "$YUYINODS_CLI" 2>/dev/null | grep -q "env"; then
...
test_cmd_preset_calls_validation() {
    ...
    if grep -A100 "^cmd_preset()" "$YUYINODS_CLI" 2>/dev/null | grep -q "validate_preset_compatibility"; then
...
test_validation_before_restore() {
    ...
    preset_load_section=$(grep -A50 "load|l)" "$YUYINODS_CLI" 2>/dev/null | head -20)
...
test_function_returns_zero() {
    ...
    if grep -A30 "^validate_preset_compatibility()" "$YUYINODS_CLI" 2>/dev/null | grep -q "return 0"; then
...
test_warns_missing_extensions() {
    ...
    if grep -A30 "^validate_preset_compatibility()" "$YUYINODS_CLI" 2>/dev/null | grep -q "warn"; then
...
# Test 9: Verify yuyinods-cli syntax is valid
test_syntax() {
    info "Test 9: Validating yuyinods-cli syntax"
