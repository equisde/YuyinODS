...
ROOT_PREFLIGHT="$ROOT_DIR/yuyinods-preflight.sh"
...
# yuyinods-preflight.sh delegates --install-env to linux-install-preflight
if grep -q 'linux-install-preflight.sh' "$ROOT_PREFLIGHT" && grep -q '\-\-install-env' "$ROOT_PREFLIGHT"; then
    pass "yuyinods-preflight.sh delegates --install-env to linux-install-preflight.sh"
else
    fail "yuyinods-preflight.sh missing --install-env delegation"
fi

if bash -n "$ROOT_PREFLIGHT" 2>/dev/null; then
    pass "yuyinods-preflight.sh still passes bash -n"
else
    fail "yuyinods-preflight.sh bash -n failed after edit"
fi
