...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/yuyinods-fleet-distro.XXXXXX")"
cleanup() {
...
echo 'odstest ALL=(ALL) NOPASSWD:ALL' > /etc/sudoers.d/yuyinodstest
...
for distro in "${TARGETS[@]}"; do
    ...
    container_name="yuyinods-fleet-${distro}-$$"
    if docker run --rm \
        --name "$container_name" \
        -v "$ROOT_DIR:/work:ro" \
        -v "$WORK_DIR:/fleet:ro" \
