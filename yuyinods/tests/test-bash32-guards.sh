...
assert_guard_before_declare "$ROOT_DIR/scripts/yuyinods-test-functional.sh" direct "yuyinods-test-functional.sh"

if grep -q '"$BASH" "$INSTALL_DIR/scripts/validate-env.sh"' "$ROOT_DIR/yuyinods-cli"; then
    pass "yuyinods-cli config validate runs validate-env.sh through active Bash"
else
    fail "yuyinods-cli config validate should run validate-env.sh through active Bash"
fi

if grep -q '"$BASH" "$INSTALL_DIR/scripts/validate-manifests.sh"' "$ROOT_DIR/yuyinods-cli"; then
    pass "yuyinods-cli config validate runs validate-manifests.sh through active Bash"
else
    fail "yuyinods-cli config validate should run validate-manifests.sh through active Bash"
fi
