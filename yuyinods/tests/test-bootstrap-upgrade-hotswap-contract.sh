...
grep -qF 'YUYINODS_BOOTSTRAP_COMPOSE_RETRY_ATTEMPTS' <<<"$compose_retry_block" \
...
if grep -qE '\brestart[[:space:]]+(llama-server|yuyinods-llama-server)\b' <<<"$active_code"; then
...
if grep -qE '\b(stop|rm)[[:space:]]+yuyinods-llama-server\b' <<<"$missing_flags_block"; then
...
grep -qF 'inspect yuyinods-llama-server --format' <<<"$active_code" \
