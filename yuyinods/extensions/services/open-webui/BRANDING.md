...
| `WEBUI_URL` | empty | Optional public URL Open WebUI uses for share links, OAuth callbacks, and PWA install metadata. Leave empty for traditional localhost usage. Headless/proxy installs should set it to `http://chat.${YUYINODS_DEVICE_NAME}.local` after yuyinods-proxy + mDNS are enabled; tunnels should use their public URL. |
| `YUYINODS_DEVICE_NAME` | `ods` | The mDNS hostname segment. Drives `${...}.local` and is reused by future remote-access integrations. |
...
1. **Override `/static/favicon.png`, `/static/splash.png`, `/static/logo.png`** — Open WebUI serves these from `/app/backend/static/`. Mounting a YuyinODS-branded set via a volume mount in the compose service is the cleanest path:
   ```yaml
   volumes:
...
1. Browse to the reachable chat URL: `http://localhost:3000` for traditional local usage, or `http://chat.ods.local` after enabling yuyinods-proxy + mDNS.
