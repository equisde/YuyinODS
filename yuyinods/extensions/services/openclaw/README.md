...
> **`OPENCLAW_TOKEN` is required.** The installer sets a random value in `.env`. If it is missing, the container will refuse to start. Verify it is set: `grep OPENCLAW_TOKEN yuyinods/.env`
...
```bash
# Check token is set
grep OPENCLAW_TOKEN yuyinods/.env

# Set a token value if missing
echo "OPENCLAW_TOKEN=$(openssl rand -hex 24)" >> yuyinods/.env
...
```bash
docker compose ps yuyinods-searxng
docker compose logs yuyinods-searxng
```

**Cannot access web UI on port 7860:**
```bash
docker compose ps yuyinods-openclaw
docker compose logs yuyinods-openclaw
```
...
ls yuyinods/config/openclaw/
docker compose logs yuyinods-openclaw | grep -i token
```
