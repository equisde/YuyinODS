"""Tests for yuyinods-host-agent.py — _parse_mem_value and _iso_now."""
...
_agent_path = Path(__file__).resolve().parents[4] / "bin" / "yuyinods-host-agent.py"
...
class TestHostAgentShutdown:
    ...
    def test_signal_shutdown_runs_from_helper_thread(self, monkeypatch):
        ...
        assert ("thread", "yuyinods-host-agent-shutdown", True) in calls
...
class TestResolveAgentBindAddr:
    ...
    def test_explicit_bind_wins(self):
        assert _resolve_agent_bind_addr({"YUYINODS_AGENT_BIND": "0.0.0.0"}, "Linux") == "0.0.0.0"
        assert _resolve_agent_bind_addr({"YUYINODS_AGENT_BIND": "192.168.1.10"}, "Linux") == "192.168.1.10"
...
class TestResolveComposeFlags:
    ...
    def test_windows_passes_host_python_to_bash_resolver(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("YUYINODS_PYTHON_CMD", "python3")
        ...
        env = calls[0][1]["env"]
        assert env["YUYINODS_PYTHON_CMD"] == (
            "/c/Users/yuyinodser/AppData/Local/Programs/Python/Python313/python.exe"
        )
...
class TestToBashPath:
    ...
    def test_leaves_posix_paths_unchanged(self, monkeypatch):
        monkeypatch.setattr(_mod.platform, "system", lambda: "Linux")
        assert _to_bash_path(PurePosixPath("/opt/yuyinods")) == "/opt/yuyinods"

    def test_converts_windows_drive_path(self, monkeypatch):
        monkeypatch.setattr(_mod.platform, "system", lambda: "Windows")
        assert _to_bash_path(Path(r"C:\Users\Gabriel\ods")) == "/c/Users/Gabriel/yuyinods"
...
class TestComposeCacheInvalidationWire:
    ...
    def test_client_posts_to_host_agent_and_unlinks_cache(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setattr(ext_router, "YUYINODS_AGENT_KEY", "wire-test-secret")
            ...
            monkeypatch.setattr(ext_router, "YUYINODS_AGENT_KEY", "wrong-secret")
            ext_router._call_agent_invalidate_compose_cache()
...
class TestInstallHookEnvAllowlist:
    ...
    def test_setup_hook_env_excludes_host_agent_secrets(self):
        ...
        for src in (self._hook_helper_source(), self._install_source()):
            for secret in ("AGENT_API_KEY", "YUYINODS_AGENT_KEY", "DASHBOARD_API_KEY"):
    ...
    def test_setup_hook_env_contains_allowlist_keys(self):
        ...
        for key in (
            ...
            "SERVICE_DATA_DIR", "YUYINODS_VERSION", "GPU_BACKEND", "HOOK_NAME",
        ):
            assert f'"{key}"' in src, (
...
class TestTailscaleStatus:
    ...
    def test_falls_back_to_native_tailscale_when_container_absent(self, monkeypatch):
        payload = {
            ...
            "Self": {
                "HostName": "yuyinods-win",
                "DNSName": "yuyinods-win.tail-example.ts.net.",
        ...
        def fake_run(cmd, *args, **kwargs):
            if cmd[:2] == ["docker", "exec"]:
                return subprocess.CompletedProcess(cmd, 1, "", "No such container: yuyinods-tailscale")
        ...
        assert body["self"]["dns_name"] == "yuyinods-win.tail-example.ts.net"
...
class TestServiceRestart:
    ...
    def test_valid_restart_calls_docker_restart(self, tmp_path, monkeypatch):
        ...
        self._write_manifest(
            ...
            "ape",
            "  name: APE\n"
            "  container_name: yuyinods-ape\n",
        )
        _mod._service_locks.pop("ape", None)
        monkeypatch.setattr(_mod, "_resolve_container_name", lambda sid: "yuyinods-ape")
        ...
        assert calls == [["docker", "restart", "yuyinods-ape"]]
    ...
    def test_delayed_restart_returns_accepted_and_releases_lock(self, tmp_path, monkeypatch):
        ...
        self._write_manifest(
            ...
            "dashboard-api",
            "  name: Dashboard API\n"
            "  container_name: yuyinods-dashboard-api\n",
        )
        _mod._service_locks.pop("dashboard-api", None)
        monkeypatch.setattr(_mod, "_resolve_container_name", lambda sid: "yuyinods-dashboard-api")
        ...
        assert calls == [["docker", "restart", "yuyinods-dashboard-api"]]
...
class TestHandleEnvUpdate:
    ...
    def test_happy_path_writes_file_and_returns_backup(self, env_update_env):
        ...
        body = _make_body("YUYINODS_AGENT_KEY=newvalue\nGGUF_FILE=/models/foo.gguf\n")
        ...
        assert "YUYINODS_AGENT_KEY=newvalue" in env_text
    ...
    def test_accepts_unknown_key_with_warning(self, env_update_env):
        ...
        install_dir, data_dir = env_update_env
        (install_dir / ".env").write_text("YUYINODS_AGENT_KEY=old\n", encoding="utf-8")
        body = _make_body("YUYINODS_AGENT_KEY=old\nNOT_IN_SCHEMA=foo\n")
        handler = _FakeHandler(body)
    ...
    def test_400_control_char_in_value(self, env_update_env):
        body = _make_body("YUYINODS_AGENT_KEY=foo\x00bar\n")
        handler = _FakeHandler(body)
    ...
    def test_400_control_char_escape_sequence(self, env_update_env):
        ...
        body = _make_body("YUYINODS_AGENT_KEY=foo\x1b[31mbar\n")
        handler = _FakeHandler(body)
    ...
    def test_tab_in_value_is_allowed(self, env_update_env):
        ...
        body = _make_body("YUYINODS_AGENT_KEY=foo\tbar\n")
        handler = _FakeHandler(body)
...
class TestEnableRetryEdgeCases:
    ...
    def test_no_hook_retry_completes_with_started_progress(
        ...
        def fake_run(cmd, *a, **k):
            ...
            # '{{.State.Status}}|{{.State.Error}}' yuyinods-<svc>` after the
