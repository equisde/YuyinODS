...
def magic_link_module(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("YUYINODS_DATA_DIR", str(tmp_path))
    monkeypatch.setenv("YUYINODS_SESSION_SECRET", "test-secret-for-magic-link-tests")
    monkeypatch.delenv("YUYINODS_PUBLIC_URL", raising=False)
    monkeypatch.delenv("WEBUI_URL", raising=False)
    monkeypatch.delenv("YUYINODS_TRUST_FORWARDED", raising=False)
    monkeypatch.delenv("YUYINODS_COOKIE_DOMAIN", raising=False)

    # session_signer reads YUYINODS_SESSION_SECRET at import time, so any
...
def test_generate_respects_public_url_env(magic_link_client, monkeypatch, magic_link_module):
    """YUYINODS_PUBLIC_URL override (for Tailscale tunnels / custom domains)
    ...
    monkeypatch.setenv("YUYINODS_PUBLIC_URL", "http://yuyinods.local:3002")
    resp = magic_link_client.post(
        "/api/auth/magic-link/generate",
    ...
    assert resp.json()["url"].startswith("http://yuyinods.local:3002/auth/magic-link/")


def test_generate_url_defaults_to_auth_subdomain(magic_link_client):
    """With no YUYINODS_PUBLIC_URL set, the URL builds off the auth subdomain
    (http://auth.<YUYINODS_DEVICE_NAME>.local) — that's where yuyinods-proxy
...
def test_generate_url_uses_configured_device_name(magic_link_client, monkeypatch, magic_link_module):
    """YUYINODS_DEVICE_NAME feeds into the auth subdomain."""
    monkeypatch.setenv("YUYINODS_DEVICE_NAME", "kitchen")
    resp = magic_link_client.post(
        "/api/auth/magic-link/generate",
        json={"target_username": "alice"},
        headers=magic_link_client.auth_headers,
...
def test_generate_strips_trailing_slash_from_public_url(magic_link_client, monkeypatch, magic_link_module):
    """An operator who sets YUYINODS_PUBLIC_URL=http://x/ must not produce a
    double-slash in the magic-link URL."""
    monkeypatch.setenv("YUYINODS_PUBLIC_URL", "http://yuyinods.local/")
    resp = magic_link_client.post(
        "/api/auth/magic-link/generate",
    ...
    assert url.startswith("http://yuyinods.local/auth/magic-link/")
...
def test_generate_owner_token_defaults_to_revoke_only_hermes_lan(magic_link_client, monkeypatch):
    monkeypatch.setenv("YUYINODS_PUBLIC_URL", "https://yuyinods.example")
    resp = magic_link_client.post(
        "/api/auth/magic-link/generate",
...
def test_generate_owner_lan_requires_ods_proxy(magic_link_client, magic_link_module, monkeypatch):
    monkeypatch.setattr(
        ...
        lambda: (False, "ODS Talk owner cards require yuyinods-proxy"),
    )
    ...
    assert "yuyinods-proxy" in resp.json()["detail"]
...
def test_owner_public_url_mode_uses_public_url_when_requested(magic_link_client, monkeypatch):
    ...
    assert data["url"].startswith("https://yuyinods.example/auth/magic-link/")
...
def test_owner_public_url_mode_does_not_require_ods_proxy(
    ...
    assert resp.status_code == 200, resp.text
    assert resp.json()["url"].startswith("https://yuyinods.example/auth/magic-link/")
...
def test_owner_card_status_reports_proxy_state(
    ...
    monkeypatch.setattr(
        magic_link_module,
        "_ods_proxy_lan_ready",
        lambda: (False, "yuyinods-proxy is not enabled"),
    )
    ...
    assert resp.json() == {
        "ready": False,
        "requires": "yuyinods-proxy",
        "reason": "yuyinods-proxy is not enabled",
    }


def test_ods_proxy_service_refreshes_stale_manifest_cache(magic_link_module, monkeypatch):
    """Owner-card readiness should see yuyinods-proxy after CLI enable.

    The dashboard API imports SERVICES at startup. When `ods enable
    yuyinods-proxy` renames compose.yaml.disabled to compose.yaml, the running
    ...
    assert service == refreshed["yuyinods-proxy"]
    assert magic_link_module.SERVICES["yuyinods-proxy"] == refreshed["yuyinods-proxy"]
...
def test_ods_proxy_service_keeps_disabled_state(magic_link_module, monkeypatch):
    ...
    assert "yuyinods-proxy" not in magic_link_module.SERVICES
...
def test_public_url_mode_requires_public_url(magic_link_client):
    ...
    assert "YUYINODS_PUBLIC_URL" in resp.json()["detail"]
...
def test_redeem_sets_cookie_and_redirects(magic_link_client, magic_link_module):
    ...
    assert b"yuyinods-session=" in cookie_blob
    ...
    assert b"yuyinods-target-user=alice" in cookie_blob


def test_redeem_issues_signed_cookie_that_verifies(magic_link_client, magic_link_module):
    """The yuyinods-session cookie set by redemption must round-trip through
    session_signer.verify() — that's what yuyinods-proxy's forward_auth will
    ...
    cookie = cookies["yuyinods-session"].value if "yuyinods-session" in cookies else None
    assert cookie, "redemption did not set yuyinods-session cookie"
...
def test_redeem_redirects_to_chat_subdomain(magic_link_client, magic_link_module, monkeypatch):
    """Successful redemption 302s to chat.<device>.local — the yuyinods-proxy
    ...
    monkeypatch.setenv("YUYINODS_DEVICE_NAME", "kitchen")
    monkeypatch.setenv("YUYINODS_COOKIE_DOMAIN", "kitchen.local")
...
def test_owner_token_can_be_redeemed_repeatedly_and_revoked(
    ...
    monkeypatch.setenv("YUYINODS_DEVICE_NAME", "studio")
...
def test_redeem_sets_cookie_with_configured_domain(magic_link_client, magic_link_module, monkeypatch):
    """When YUYINODS_COOKIE_DOMAIN is set, the cookie carries that Domain
...
def test_redeem_defaults_cookie_domain_to_device_domain(magic_link_client, magic_link_module, monkeypatch):
    """With YUYINODS_COOKIE_DOMAIN unset in the default host-based layout, derive
    <YUYINODS_DEVICE_NAME>.local so redemption on auth.<device>.local carries to
    chat.<device>.local."""
    monkeypatch.setenv("YUYINODS_DEVICE_NAME", "kitchen")
    monkeypatch.delenv("YUYINODS_COOKIE_DOMAIN", raising=False)
    gen = magic_link_client.post(
...
def test_redeem_omits_cookie_domain_with_public_url_override(magic_link_client, magic_link_module, monkeypatch):
    """YUYINODS_PUBLIC_URL means the operator chose a single custom origin/path
    layout, so omit Domain and let the cookie stay host-only."""
    monkeypatch.setenv("YUYINODS_PUBLIC_URL", "http://yuyinods.example")
    monkeypatch.delenv("YUYINODS_COOKIE_DOMAIN", raising=False)
    gen = magic_link_client.post(
        "/api/auth/magic-link/generate",
        json={"target_username": "alice"},
...
def test_redeem_refuses_when_signing_unconfigured(magic_link_client, magic_link_module):
    """If YUYINODS_SESSION_SECRET isn't configured, redemption returns 503
...
def test_redeem_back_compat_auth_prefix_route_still_works(magic_link_client, magic_link_module):
    ...
    YUYINODS_PUBLIC_URL overrides and any in-flight QR codes from before
...
def test_qr_endpoint_returns_data_url_when_qrcode_installed(magic_link_client):
    ...
    resp = magic_link_client.get(
        "/api/auth/magic-link/qr?url=http://yuyinods.local:3002/auth/magic-link/abc",
        headers=magic_link_client.auth_headers,
    )
