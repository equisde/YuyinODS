...
def signed_talk_cookie(monkeypatch):
    ...
    monkeypatch.setenv("YUYINODS_SESSION_SECRET", "test-secret-for-talk")
...
def talk_client(test_client, signed_talk_cookie):
    test_client.cookies.set("yuyinods-session", signed_talk_cookie)
    return test_client
...
def test_talk_speak_streams_audio(talk_client, monkeypatch):
    ...
    # Critical for streaming through nginx / yuyinods-proxy: without this
...
def test_talk_vision_url_defaults_to_openai_v1(monkeypatch):
    ...
    monkeypatch.delenv("YUYINODS_TALK_VISION_URL", raising=False)
...
def test_talk_vision_url_does_not_duplicate_v1(monkeypatch):
    ...
    monkeypatch.setenv("YUYINODS_TALK_VISION_URL", "http://vision.local:8080/v1")
    monkeypatch.setenv("LLM_API_BASE_PATH", "/v1")
...
def test_talk_vision_url_preserves_lemonade_api_v1(monkeypatch):
    ...
    monkeypatch.setenv("YUYINODS_TALK_VISION_URL", "http://host.docker.internal:8080/api/v1")
    monkeypatch.setenv("LLM_API_BASE_PATH", "/v1")
...
def test_ods_talk_hermes_timeout_is_env_configurable(monkeypatch):
    ...
    monkeypatch.setenv("YUYINODS_TALK_HERMES_TIMEOUT", "900")
    assert hermes_bridge._request_timeout() == 900

    monkeypatch.setenv("YUYINODS_TALK_HERMES_TIMEOUT", "5")
