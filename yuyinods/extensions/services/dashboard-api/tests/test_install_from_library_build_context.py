...
def test_rewrites_relative_dot_context(tmp_path, caplog):
    ...
    final_dir = Path("/var/lib/yuyinods/user-extensions/audiocraft")
    _write(compose, {
        "services": {
            "audiocraft": {
...
def test_rewrites_other_relative_context(tmp_path):
    compose = tmp_path / "compose.yaml"
    final_dir = Path("/var/lib/yuyinods/user-extensions/foo")
    _write(compose, {
        "services": {
            "foo": {
...
def test_multiple_services_mixed(tmp_path):
    compose = tmp_path / "compose.yaml"
    final_dir = Path("/var/lib/yuyinods/user-extensions/multi")
    _write(compose, {
        "services": {
