...
def _write_test_script(install_root: Path, body: str) -> Path:
    """Write a bash script to install_root/scripts/yuyinods-test-functional.sh."""
    scripts = install_root / "scripts"
    scripts.mkdir(parents=True, exist_ok=True)
    path = scripts / "yuyinods-test-functional.sh"
...
def setup_install_dir(tmp_path, monkeypatch):
    ...
    ``Path(INSTALL_DIR) / "scripts" / "yuyinods-test-functional.sh"``; if it's
    not there it falls back to ``Path(os.getcwd()) / "yuyinods-test-functional.sh"``.
