...
def test_update_dry_run_with_env_and_version(test_client, tmp_path, monkeypatch):
    ...
    assert "YUYINODS_VERSION" in data["env_keys"]
...
def test_update_dry_run_version_from_version_file(test_client, tmp_path, monkeypatch):
    """GET /api/update/dry-run falls back to .version file when .env has no YUYINODS_VERSION."""
    import routers.updates as updates_mod
...
def test_update_dry_run_reads_compose_images(test_client, tmp_path, monkeypatch):
    ...
    compose_content = (
        "services:\n"
        "  app:\n"
        "    image: ghcr.io/yuyinods/server:latest\n"
        "  db:\n"
        "    image: postgres:16\n"
    )
    ...
    assert resp.status_code == 200
    data = resp.json()
    assert "ghcr.io/yuyinods/server:latest" in data["images"] or " ghcr.io/yuyinods/server" in str(data["images"])
