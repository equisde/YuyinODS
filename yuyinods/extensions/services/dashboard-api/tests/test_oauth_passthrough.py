...
def oauth_client(monkeypatch):
    ...
    tmp = tempfile.mkdtemp(prefix="yuyinods-oauth-test-")
    monkeypatch.setenv("YUYINODS_PERSONA_DIR", tmp)
...
def test_oauth_providers_reports_credential_status(oauth_client, monkeypatch):
    ...
    monkeypatch.setenv("YUYINODS_OAUTH_PROVIDERS_FILE", str(registry))
    monkeypatch.setenv("YUYINODS_DATA_DIR", str(data_dir))
