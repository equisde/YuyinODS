...
INSTALL_DIR = os.environ.get("YUYINODS_INSTALL_DIR", os.path.expanduser("~/yuyinods"))
DATA_DIR = os.environ.get("YUYINODS_DATA_DIR", os.path.expanduser("~/.ods"))
EXTENSIONS_DIR = Path(
    os.environ.get(
        "YUYINODS_EXTENSIONS_DIR",
        ...
        try:
            ...
            if isinstance(service, dict):
                ...
                services[service_id] = {
                    ...
                    "container_name": service.get("container_name", f"yuyinods-{service_id}"),
...
def _detect_container_default_gateway(route_path: str = "/proc/net/route") -> str:
    ...
    Why this matters: dashboard-api runs on `yuyinods-network` (a custom bridge,
    e.g. 172.18.0.0/16). On Linux, the yuyinods-host-agent binds to that network's
...
def _resolve_agent_host() -> str:
    """Pick the host name/IP to use for the yuyinods-host-agent.

    Priority:
      1. YUYINODS_AGENT_HOST env (explicit operator override)
    ...
    """
    explicit = os.environ.get("YUYINODS_AGENT_HOST", "").strip()
    if explicit:
        return explicit
    if _running_inside_container():
        gw = _detect_container_default_gateway()
        if gw:
            logger.info("Resolved YUYINODS_AGENT_HOST=%s via /proc/net/route", gw)
            return gw
    logger.warning(
        "Could not detect container default gateway; falling back to "
        "host.docker.internal. If host-agent calls time out, set "
        "YUYINODS_AGENT_HOST=<host-ip> in dashboard-api's environment."
    )
    return "host.docker.internal"


AGENT_HOST = _resolve_agent_host()
AGENT_PORT = int(os.environ.get("YUYINODS_AGENT_PORT", "7710"))
...
YUYINODS_AGENT_KEY = os.environ.get("YUYINODS_AGENT_KEY", "") or DASHBOARD_API_KEY
