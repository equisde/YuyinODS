...
def _callback_dir() -> Path:
    ...
    base = Path(os.environ.get("YUYINODS_PERSONA_DIR", "/data/persona"))
    base.mkdir(parents=True, exist_ok=True)
    return base


def _install_dir() -> Path:
    return Path(os.environ.get("YUYINODS_INSTALL_DIR", "/yuyinods"))


def _data_dir() -> Path:
    return Path(os.environ.get("YUYINODS_DATA_DIR", "/data"))


def _providers_file() -> Path:
    override = os.environ.get("YUYINODS_OAUTH_PROVIDERS_FILE", "").strip()
    if override:
...
def _credential_roots() -> list[Path]:
    override = os.environ.get("YUYINODS_OAUTH_CREDENTIAL_DIRS", "").strip()
    if override:
