...
from config import AGENT_URL, YUYINODS_AGENT_KEY, INSTALL_DIR
    ...
    if env_file.exists():
        try:
            for line in _read_utf8(env_file).splitlines():
                if line.startswith("YUYINODS_VERSION="):
...
def _call_update_agent(endpoint_action: str, payload: dict, timeout: int) -> dict:
    ...
    headers = {
        ...
        "Authorization": f"Bearer {YUYINODS_AGENT_KEY}",
    }
...
def _get_update_agent_status(timeout: int = 5) -> dict:
    ...
    headers = {"Authorization": f"Bearer {YUYINODS_AGENT_KEY}"}
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            response = await client.get(
                "https://api.github.com/repos/equisde/YuyinODS/releases/latest",
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            resp = await client.get(
                "https://api.github.com/repos/equisde/YuyinODS/releases?per_page=5",
                headers=_GITHUB_HEADERS,
            )
        releases = resp.json()
        ...
        current = await asyncio.to_thread(_read_current_version)
        return {
            "releases": [{"version": current, "date": datetime.now(timezone.utc).isoformat() + "Z", "title": f"ODS {current}", "changelog": "Release information unavailable. Check GitHub directly.", "url": "https://github.com/equisde/YuyinODS/releases", "prerelease": False}],
            "checked_at": datetime.now(timezone.utc).isoformat() + "Z",
...
_UPDATE_ENV_KEYS = {
    "YUYINODS_VERSION", "TIER", "LLM_MODEL", "GGUF_FILE",
