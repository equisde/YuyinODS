...
Two endpoints today:
  ...
  * ``GET /api/auth/verify-session`` — validates the HMAC-signed
    ``yuyinods-session`` cookie. Used by Caddy reverse proxies (e.g., the
  ...
  * ``POST /api/auth/admin-session`` — mints a signed ``yuyinods-session``
...
Security:
  ...
  * ``admin-session`` IS gated by the API key. Only callers that already
    hold the admin secret (the dashboard, yuyinods-cli, the host agent)
...
def _cookie_domain() -> Optional[str]:
    ...
    YUYINODS_COOKIE_DOMAIN is set by the installer to ``<YUYINODS_DEVICE_NAME>.local``
    when yuyinods-proxy + mDNS are wired so a single redemption authenticates
...
def verify_session(request: Request) -> dict:
    """Validate the yuyinods-session cookie. Returns 200 if valid, 401 if not.
    ...
    the yuyinods-session cookie, hand it to session_signer.verify(), and
...
def admin_session(response: Response, request: Request) -> dict:
    """Mint a signed ``yuyinods-session`` cookie for the install owner.
    ...
    ``yuyinods-session`` cookie identical to one a magic-link redemption
    would issue.
    ...
    Used by:
      ...
      * The host agent / yuyinods-cli — when running setup flows that
        need to leave a cookie in a browser session for follow-up.

    The cookie is identical in shape to a magic-link-issued one:
    HMAC-SHA256 signed against ``YUYINODS_SESSION_SECRET``, ``HttpOnly``,
    ``SameSite=Lax``, ``Secure`` when reached over HTTPS, with the
    operator's ``YUYINODS_COOKIE_DOMAIN`` (if set) so it travels across
    proxy subdomains.

    Returns 503 if ``YUYINODS_SESSION_SECRET`` is not configured — same
    ...
    if not session_signer.is_configured():
        logger.error(
            "admin-session refused: YUYINODS_SESSION_SECRET is not configured. "
