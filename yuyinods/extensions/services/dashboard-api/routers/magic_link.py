...
DATA_DIR = Path(os.environ.get("YUYINODS_DATA_DIR", "/data"))
AUTH_DIR = DATA_DIR / "auth"
TOKEN_STORE_PATH = AUTH_DIR / "magic-links.json"
SESSION_COOKIE_NAME = "yuyinods-session"
...
class GenerateRequest(BaseModel):
    ...
    url_mode: str = Field(
        ...
        description="auto uses the normal URL policy; lan forces .local URLs; public uses YUYINODS_PUBLIC_URL when configured.",
    )
...
def _device_name() -> str:
    """The YUYINODS_DEVICE_NAME segment, with a sane fallback.

    The mDNS announcer and the yuyinods-proxy use the same name to build
    ...
    """
    raw = (os.environ.get("YUYINODS_DEVICE_NAME") or _DEVICE_NAME_DEFAULT).strip()
...
def _public_base() -> str:
    ...
    Set YUYINODS_PUBLIC_URL to an absolute URL (e.g., a Tailscale tunnel
    or custom domain) and the magic-link / redirect URLs build off
    that instead of the per-subdomain default. Empty → use
    `_auth_url` / `_chat_url` built from YUYINODS_DEVICE_NAME.
    """
    return (os.environ.get("YUYINODS_PUBLIC_URL") or "").rstrip("/")
...
def _auth_url(url_mode: str = "auto") -> str:
    ...
    Default: http://auth.<YUYINODS_DEVICE_NAME>.local — the yuyinods-proxy
    on :80 with Host: auth.<name>.local routes to dashboard-api.
    YUYINODS_PUBLIC_URL overrides for tunneled / non-mDNS deployments.
    """
...
def _chat_url(url_mode: str = "auto") -> str:
    ...
    Default: http://chat.<YUYINODS_DEVICE_NAME>.local. Cookie set by
    redemption uses Domain=<YUYINODS_DEVICE_NAME>.local so it carries
    across to the chat subdomain — that's how single-redemption SSO
    works without identity claims in the cookie.
    """
    if _use_public_url(url_mode):
        # When YUYINODS_PUBLIC_URL is overridden, assume /chat is the chat
...
def _magic_link_url(token: str, url_mode: str = "auto") -> str:
    ...
    Default: http://auth.<YUYINODS_DEVICE_NAME>.local/magic-link/<token>.
    The cookie set by the redemption handler uses
    Domain=<YUYINODS_DEVICE_NAME>.local so it's also sent to
    chat.<name>.local, hermes.<name>.local, etc. (subdomain SSO).
    """
    base = _auth_url(url_mode).rstrip("/")
    # When using the override (YUYINODS_PUBLIC_URL), preserve the /auth/
    # prefix that the yuyinods-proxy historically routed to dashboard-api.
...
def _cookie_domain(url_mode: str = "auto") -> Optional[str]:
    ...
    YUYINODS_COOKIE_DOMAIN can override the parent domain used for subdomain SSO.
    In the default yuyinods-proxy layout, derive <YUYINODS_DEVICE_NAME>.local so a
    redemption on auth.<name>.local authenticates requests to chat.<name>.local.
    If YUYINODS_PUBLIC_URL is set, keep the cookie host-only because the operator
    is explicitly using a single custom origin/path layout.
    """
    raw = (os.environ.get("YUYINODS_COOKIE_DOMAIN") or "").strip()
    if raw:
        return raw
    if _use_public_url(url_mode):
        return None
...
def _ods_proxy_service() -> Optional[dict]:
    """Return yuyinods-proxy service config, refreshing once for post-enable state.

    dashboard-api loads extension manifests at process startup, but `ods
    enable yuyinods-proxy` flips compose.yaml.disabled to compose.yaml while this
    ...
    """
    service = SERVICES.get("yuyinods-proxy")
    if service:
        return service

    refreshed_services, _, errors = load_extension_manifests(EXTENSIONS_DIR, GPU_BACKEND)
    if errors:
        logger.debug("Manifest refresh while checking yuyinods-proxy reported %d errors", len(errors))
    service = refreshed_services.get("yuyinods-proxy")
    if service:
        SERVICES["yuyinods-proxy"] = service
    return service
...
def _ods_proxy_lan_ready() -> tuple[bool, str]:
    ...
    if not service:
        return (
            False,
            "ODS Talk owner cards require yuyinods-proxy. Enable LAN web access before generating an owner card.",
        )

    host = service.get("host") or "yuyinods-proxy"
    ...
    try:
        with urllib.request.urlopen(url, timeout=1.0) as resp:
            ...
            if 200 <= status < 400:
                return True, ""
            return False, f"yuyinods-proxy health returned HTTP {status}"
    except (OSError, TimeoutError, urllib.error.URLError) as exc:
        return False, f"yuyinods-proxy is enabled but not reachable: {exc}"
...
def generate_magic_link(payload: GenerateRequest, request: Request) -> GenerateResponse:
    ...
    if payload.url_mode == "public" and not _public_base():
        raise HTTPException(
            status_code=400,
            detail="url_mode=public requires YUYINODS_PUBLIC_URL to be configured",
        )
...
# YUYINODS_PUBLIC_URL override case where dashboard-api is reached via a
...
def redeem_magic_link(token: str, request: Request, response: Response) -> RedirectResponse:
    ...
    # Pre-flight: if YUYINODS_SESSION_SECRET isn't configured, refuse the
    ...
    if not session_signer.is_configured():
        logger.error(
            "magic-link redemption refused: YUYINODS_SESSION_SECRET is not "
...
def _client_ip(request: Request) -> str:
    ...
    proxy. If you put a real reverse proxy in front, set YUYINODS_TRUST_FORWARDED=1
    ...
    """
    if os.environ.get("YUYINODS_TRUST_FORWARDED") == "1":
