...
from config import (
    ...
    YUYINODS_AGENT_KEY, EXTENSION_CATALOG, EXTENSIONS_DIR,
...
def _sync_extension_config(service_id: str) -> bool:
    ...
    The dashboard-api container has /yuyinods/config bind-mounted
...
async def extension_logs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=f"Host agent unavailable. Use 'docker logs yuyinods-{service_id}' in terminal.",
        )
...
def enable_extension(
    ...
    if enabled_compose.exists():
        ...
        _write_initial_progress(service_id)
        # Invalidate .compose-flags cache so yuyinods-cli picks up this extension
    ...
    with _extensions_lock():
        ...
        # Invalidate .compose-flags cache so yuyinods-cli picks up the new enabled set
        if enabled_services:
            _call_agent_invalidate_compose_cache()
