...
from config import AGENT_URL, DATA_DIR, YUYINODS_AGENT_KEY, GPU_BACKEND, SERVICES
...
async def service_resources(api_key: str = Depends(verify_api_key)):
    ...
    # This correctly handles non-standard names (yuyinods-webui -> open-webui)
    # when container_name is populated in SERVICES (by PR E's config.py change).
    # Falls back to yuyinods-{sid} convention when container_name is missing.
    container_to_service = {}
    for sid, svc in SERVICES.items():
        cname = svc.get("container_name", f"yuyinods-{sid}")
