...
from config import (
    ...
    AGENT_HOST, AGENT_PORT, AGENT_URL, YUYINODS_AGENT_KEY,
...
async def host_agent_diagnostics():
    ...
    return {
        "configured": {
            ...
            "host": AGENT_HOST,
            "port": AGENT_PORT,
            "ods_agent_key_configured": bool(YUYINODS_AGENT_KEY),
            "ods_agent_host_explicit": bool(os.environ.get("YUYINODS_AGENT_HOST", "").strip()),
        },
        "container": {
...
async def service_tokens():
    ...
    def _read_tokens():
        ...
        if not oc_token:
            for path in [Path("/data/openclaw/home/gateway-token"), Path("/yuyinods/.env")]:
                try:
                    if path.suffix == ".env":
