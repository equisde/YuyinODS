...
_SETTINGS_APPLY_ALLOWED_SERVICES = frozenset({
    ...
    "ape", "privacy-shield", "yuyinods-proxy",
})
_LLAMA_APPLY_KEYS = {
...
_MANUAL_RESTART_KEYS = {
    "BIND_ADDRESS",
    "DASHBOARD_API_KEY", "YUYINODS_AGENT_KEY", "DASHBOARD_PORT",
    "DASHBOARD_API_PORT", "YUYINODS_AGENT_PORT", "YUYINODS_AGENT_HOST",
}
...
def _match_apply_service(key: str) -> Optional[str]:
    ...
    if key.startswith("N8N_"):
        return "n8n"
    if key == "YUYINODS_AUTH_UPSTREAM" or key.startswith("HERMES_PROXY_"):
        return "hermes-proxy"
    if key.startswith("HERMES_") or key.startswith("WHATSAPP_"):
        return "hermes"
    if key.startswith("YUYINODS_PROXY_"):
        return "yuyinods-proxy"
...
def _compute_env_apply_plan(previous_values: dict[str, str], next_values: dict[str, str]) -> dict[str, Any]:
    ...
    for key in changed_keys:
        ...
        if service and service in _SETTINGS_APPLY_ALLOWED_SERVICES:
            ...
            continue
        if key in _MANUAL_RESTART_KEYS or key.startswith("YUYINODS_AGENT_"):
