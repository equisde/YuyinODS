...
DEFAULT_HERMES_URL = "http://yuyinods-hermes:9119"
...
_IDLE_EXPIRY_SECONDS = _env_int("YUYINODS_TALK_IDLE_EXPIRY", 300)  # 5 min default
...
def _request_timeout() -> int:
    raw = os.environ.get("YUYINODS_TALK_HERMES_TIMEOUT", "")
    if raw.isdigit():
...
def talk_session_key(cookie_value: str) -> str:
    """Stable opaque key for the lifetime of a yuyinods-session cookie."""
...
async def _create_session_on_ws(ws: aiohttp.ClientWebSocketResponse, *, timeout: float = 30) -> str:
    ...
    request_id = "yuyinods-talk-create"
...
async def _submit_on_connection(
    ...
    """
    request_id = f"yuyinods-talk-prompt-{int(time.monotonic() * 1000)}"
    try:
...
async def stream_prompt(session_key: str, text: str) -> AsyncIterator[dict[str, Any]]:
    ...
    YUYINODS_TALK_IDLE_EXPIRY seconds (default 300s = 5 min) so a fleet of
