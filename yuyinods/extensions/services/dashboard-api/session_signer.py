"""HMAC-signed session cookies for ODS's yuyinods-session.
...
Where:
  ...
  * signature is `HMAC-SHA256(YUYINODS_SESSION_SECRET, "<random-id>.<expiry>")`
    base64-url-encoded (no padding)

Why this shape:
  * Stateless validation — the verifier only needs YUYINODS_SESSION_SECRET,
  ...
  * Revocation is bounded by expiry — if a cookie leaks, the operator
    rotates YUYINODS_SESSION_SECRET (which invalidates every issued cookie)
  ...
  * No identity in the cookie — the random-id is opaque. The magic-link
    ...
    `yuyinods-target-user` cookie or server-side audit log). Putting the
...
# HMAC check.
_SECRET: bytes = (os.environ.get("YUYINODS_SESSION_SECRET", "")).encode("utf-8")


def is_configured() -> bool:
    """Return True iff YUYINODS_SESSION_SECRET was provided.
...
def issue(ttl_seconds: int = 12 * 3600) -> str:
    ...
    Raises ``RuntimeError`` if YUYINODS_SESSION_SECRET is not configured —
    we refuse to issue cookies that can't be verified.
    """
    if not _SECRET:
        raise RuntimeError(
            "YUYINODS_SESSION_SECRET is not configured; refusing to issue an "
...
def verify(cookie_value: str) -> Tuple[bool, str]:
    ...
    Reasons (when ok is False):
      * ``"no-secret"`` — YUYINODS_SESSION_SECRET not configured server-side
