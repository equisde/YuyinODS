...
The active mode is controlled by `YUYINODS_MODE` in `.env`. The corresponding config file is loaded automatically from `config/litellm/`.
...
|----------|---------|-------------|
| `LITELLM_KEY` | *(required)* | Master API key — generate with `echo "sk-yuyinods-$(openssl rand -hex 16)"` |
| `LITELLM_PORT` | `4000` | External + internal port |
| `YUYINODS_MODE` | `local` | Operating mode: `local`, `cloud`, or `hybrid` |
...
Your App / n8n / Scripts
         ...
         ▼
    LiteLLM (:4000)
    config/litellm/${YUYINODS_MODE}.yaml
         │
    ┌────┴─────────────────┐
    │                       │
...
- Verify the config file is correct: `cat yuyinods/config/litellm/local.yaml`
