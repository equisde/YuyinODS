...
/**
 * Ensure the dashboard user has a valid `yuyinods-session` cookie.
 *
 * Why this exists
 * ---------------
 ...
 * yuyinods-session cookie to the owner-card help page. Without this
 ...
 * set YUYINODS_SESSION_SECRET, but don't block the dashboard.
 */
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        // 503 = YUYINODS_SESSION_SECRET not configured. The dashboard still
        // works; cookie-gated services won't. Surface the hint quietly.
        if (mint.status === 503) {
          const body = await mint.json().catch(() => ({}))
          console.warn(
            '[yuyinods-session] could not mint admin session:',
            body.detail || 'server misconfigured',
            '— set YUYINODS_SESSION_SECRET in .env and restart dashboard-api',
          )
          return
        }
        ...
        console.warn('[yuyinods-session] admin-session returned', mint.status)
      } catch (err) {
        console.warn('[yuyinods-session] bootstrap failed:', err)
      }
    }
