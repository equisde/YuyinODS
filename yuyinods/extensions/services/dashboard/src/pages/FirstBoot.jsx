...
const PROGRESS_KEY = 'yuyinods-firstboot-progress'
...
export default function FirstBoot({ onComplete }) {
  ...
  const finish = async () => {
    ...
    try {
      ...
      // Mint an admin yuyinods-session cookie for the operator who just
      ...
      try {
        ...
        if (!adminResp.ok && adminResp.status !== 503) {
          ...
          console.warn('[yuyinods-session] admin-session returned', adminResp.status)
        }
      } catch (err) {
        // eslint-disable-next-line no-console
        console.warn('[yuyinods-session] admin-session network failure:', err)
      }
    ...
    <div>
      ...
      <label className="block mb-6">
        ...
        <span className="text-xs text-theme-text-muted mt-2 block">
          ...
          change <code className="text-theme-accent">YUYINODS_DEVICE_NAME</code> in Settings before expecting
