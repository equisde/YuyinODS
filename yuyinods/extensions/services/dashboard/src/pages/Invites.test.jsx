...
const ownerCardReady = { ready: true, requires: 'yuyinods-proxy', reason: '' }
...
describe('Invites', () => {
  ...
  test('warns and disables owner card creation when yuyinods-proxy is unavailable', async () => {
    const fetchMock = vi.fn(async (url) => {
      if (url === '/api/auth/magic-link/list') return response({ tokens: [] })
      if (url === '/api/auth/magic-link/owner-card/status') {
        return response({
          ...
          reason: 'ODS Talk owner cards require yuyinods-proxy.',
        })
      }
      throw new Error(`unexpected request: ${url}`)
    })
    ...
    expect(await screen.findByText(/ODS Talk owner cards require yuyinods-proxy/i)).toBeInTheDocument()
