...
export default function YuyinODSTalk() {
  ...
  const [spokenReplies, setSpokenReplies] = useState(() => {
    try {
      return globalThis.localStorage?.getItem('yuyinods-talk-spoken-replies') === '1'
    } catch {
      return false
    }
  })
  ...
  useEffect(() => {
    try {
      globalThis.localStorage?.setItem('yuyinods-talk-spoken-replies', spokenReplies ? '1' : '0')
    } catch {
            ...
            if (!started) {
              ...
              audio.play().catch(err => {
                if (err?.name === 'NotAllowedError') {
                  ...
                  console.warn('[yuyinods-talk] audio.play() blocked by browser policy:', err.message)
                }
              })
            }
          }
  ...
  const sendAudioFile = useCallback(async (file) => {
    ...
    try {
      const body = new FormData()
      body.set('file', file, file.name || 'yuyinods-talk-audio.webm')
