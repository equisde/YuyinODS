...
import YuyinODSTalk from './YuyinODSTalk' // eslint-disable-line no-unused-vars
...
describe('YuyinODSTalk', () => {
  ...
  test('renders a mobile text portal and sends a message', async () => {
    const fetchMock = vi.fn(async (url, options = {}) => {
      ...
      if (url === '/api/talk/message/stream' && options.method === 'POST') {
        ...
        return sseResponse([
          ...
          { type: 'delta', text: ' from this YuyinODS.' },
          { type: 'complete', session_id: 'sid', text: 'I can help from this YuyinODS.', status: 'ok' },
          { type: 'done' },
        ])
      }
    ...
    expect(await screen.findByText('I can help from this YuyinODS.')).toBeInTheDocument()
  })
  ...
  test('renders markdown in assistant bubbles (bold/lists), not raw asterisks', async () => {
    ...
    const { container } = render(<YuyinODSTalk />)
    expect(await screen.findByText('Ready')).toBeInTheDocument()
    fireEvent.change(screen.getByPlaceholderText('Message ODS'), {
      target: { value: 'choose' },
    })
