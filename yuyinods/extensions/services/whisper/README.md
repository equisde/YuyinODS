...
The `docker-entrypoint.sh` script applies a VAD tuning patch to the upstream speaches STT router at container startup. The patch is idempotent — it detects an existing `YUYINODS_PATCHED` marker and skips re-application on container restart.
...
- Check logs for `[yuyinods-whisper] WARNING: Target pattern not found`
