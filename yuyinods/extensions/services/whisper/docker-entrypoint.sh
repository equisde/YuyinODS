...
apply_vad_patch() {
    local stt_file="$1"
    echo "[yuyinods-whisper] Applying VAD patch to $stt_file"

    if [ ! -f "$stt_file" ]; then
        echo "[yuyinods-whisper] Warning: STT file not found: $stt_file"
        return 1
    fi
...
PYTHON_EOF
    ...
    if "$PYTHON_CMD" /tmp/vad_patcher.py "$stt_file"; then
        echo "[yuyinods-whisper] VAD patch applied successfully"
        rm -f /tmp/vad_patcher.py
        return 0
    else
        echo "[yuyinods-whisper] VAD patch failed, continuing with defaults"
        rm -f /tmp/vad_patcher.py
        return 1
    fi
}
...
if [ -n "$STT_FILE" ]; then
    ...
    echo "[yuyinods-whisper] Could not locate STT module, skipping VAD patch"
fi
