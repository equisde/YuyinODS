...
| `container_name`      | string  | no       | Docker container name (e.g. `yuyinods-webui`). |
...
- **Running validation:** From the repo root: `bash scripts/validate-manifests.sh`. From an install: `./yuyinods-cli config validate` (runs both env and manifest validation).
