...
The `yuyinods-update.sh` script automatically runs migrations during updates:

```bash
# In yuyinods-update.sh
cd ods
./scripts/migrate-config.sh migrate || {
...
**"Migration failed"**
- Check the backup in `~/.yuyinods/backups/`
...
- `../scripts/yuyinods-update.sh` — Update system that calls migrations
