#!/bin/bash
# yuyinods-backup.sh - ODS Backup Utility
...
YUYINODS_DIR="${YUYINODS_DIR:-$SCRIPT_DIR}"
BACKUP_ROOT="${YUYINODS_DIR}/.backups"
...
estimate_backup_bytes() {
    ...
    if [[ "$backup_type" == "full" || "$backup_type" == "user-data" ]]; then
        ...
        for p in "${user_data_paths[@]}"; do
            if [[ -d "$YUYINODS_DIR/$p" ]]; then
                local b
                b=$(du -sk "$YUYINODS_DIR/$p" 2>/dev/null | awk '{print $1 * 1024}')
                total=$(( total + ${b:-0} ))
            fi
        done
    fi
    ...
    if [[ "$backup_type" == "full" || "$backup_type" == "config" ]]; then
        if [[ -d "$YUYINODS_DIR/config" ]]; then
            local b
            b=$(du -sk "$YUYINODS_DIR/config" 2>/dev/null | awk '{print $1 * 1024}')
            total=$(( total + ${b:-0} ))
        fi
        for f in "$YUYINODS_DIR"/.env "$YUYINODS_DIR"/.version "$YUYINODS_DIR"/docker-compose*.y*ml "$YUYINODS_DIR"/yuyinods-preflight.sh "$YUYINODS_DIR"/yuyinods-update.sh; do
            if [[ -f "$f" ]]; then
                local s
    ...
    if [[ "$backup_type" == "full" ]]; then
        if [[ -d "$YUYINODS_DIR/models" ]]; then
            local b
            b=$(du -sk "$YUYINODS_DIR/models" 2>/dev/null | awk '{print $1 * 1024}')
            total=$(( total + ${b:-0} ))
        fi
...
create_manifest() {
    ...
    local description="${3:-}"
    local version
    # .version is a JSON file written by yuyinods-update.sh; extract the version
    # string rather than embedding the entire JSON blob (see get_current_version
    # in yuyinods-update.sh for the canonical reader).
    version=$(jq -r '.version // "unknown"' "$YUYINODS_DIR/.version" 2>/dev/null || echo "unknown")
...
backup_user_data() {
    ...
    for path in "${user_data_paths[@]}"; do
        local full_path="$YUYINODS_DIR/$path"
        if [[ -d "$full_path" ]]; then
...
backup_config() {
    ...
    for file in "$YUYINODS_DIR"/.env "$YUYINODS_DIR"/.version "$YUYINODS_DIR"/docker-compose*.y*ml "$YUYINODS_DIR"/yuyinods-preflight.sh "$YUYINODS_DIR"/yuyinods-update.sh; do
        if [[ -f "$file" ]]; then
            cp "$file" "$backup_dir/"
    ...
    # Config directory
    if [[ -d "$YUYINODS_DIR/config" ]]; then
        rsync_with_progress "$YUYINODS_DIR/config" "$backup_dir/" "Backing up config/"
...
backup_cache() {
    ...
    if [[ -d "$YUYINODS_DIR/models" ]]; then
        rsync_with_progress "$YUYINODS_DIR/models" "$backup_dir/" "Backing up models/"
    ...
    for path in "${cache_paths[@]}"; do
        if [[ -d "$YUYINODS_DIR/$path" ]]; then
            local dest_dir="$backup_dir/$(dirname "$path")"
            mkdir -p "$dest_dir"
            rsync_with_progress "$YUYINODS_DIR/$path" "$dest_dir/" "Backing up $path"
            log_success "Backed up: $path"
        fi
    done
}
...
do_backup() {
    ...
    echo "  yuyinods-restore.sh $backup_id"
}
...
main() {
    ...
    for f in "$YUYINODS_DIR"/docker-compose*.y*ml; do
        [[ -f "$f" ]] && has_compose=true && break
    done
    if [[ "$has_compose" == "false" && ! -d "$YUYINODS_DIR/data" ]]; then
