...
Frequently asked questions about installing, running, and troubleshooting YuyinODS.
...
### What is YuyinODS?
YuyinODS is a turnkey local AI stack that runs entirely on your own hardware. It includes:
...
**After setup:** No. YuyinODS is designed for offline/air-gapped operation. All models run locally.
...
YuyinODS is **free and open source** (Apache 2.0 license). You only pay for:
...
```
The model will load in ~30-120 seconds depending on size. If it fails, YuyinODS automatically rolls back to the previous model.

On Lemonade installs, load the model through YuyinODS rather than only
opening it in the Lemonade app. The Lemonade app can load the file for direct
testing, but Open WebUI uses YuyinODS's persisted LiteLLM route and may
...
```bash
cd ~/yuyinods
./yuyinods-uninstall.sh --force
```

This uses YuyinODS's saved `.compose-flags` stack, removes the matching containers and volumes, and then removes the install directory. Use `--keep-data` or `--keep-models` if you want to preserve local state.

If you need to run Docker Compose manually, do not use bare `docker compose down`: YuyinODS does not use a top-level `docker-compose.yml`. Use the saved flags instead:
...
### What's n8n?
n8n is the workflow engine built into YuyinODS. It provides:
...
For production deployments, use a reverse proxy (nginx, Caddy, Traefik) in front of YuyinODS:
...
**Configs and data:**
```bash
tar -czf yuyinods-backup.tar.gz .env data/
```
...
### Documentation
- Main README: `yuyinods/README.md`
- Installer Architecture: `docs/INSTALLER-ARCHITECTURE.md`
- Security: `SECURITY.md`

### Community
- GitHub Issues: https://github.com/equisde/YuyinODS/issues
