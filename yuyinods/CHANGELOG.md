...
### Fixed
- Owner-card readiness now notices `yuyinods-proxy` after `ods enable
  yuyinods-proxy` and `ods start yuyinods-proxy` without requiring a manual
...
- The new `yuyinods-proxy-owner-card-readiness-1474` regression fixture passed on
...
- `YUYINODS_MODE=lemonade` for AMD installs — routes all services through LiteLLM proxy
...
- `.env.schema.json` updated for `YUYINODS_MODE=lemonade`, `TARGET_API_KEY`, `LLM_BACKEND`, `LLM_API_BASE_PATH`
